...
class FileOutput{
 ...
 public:
   ...
   */
  FileOutput(){
    this->out.precision(_PREC_OUTPUT_);
...
//  int read_file_one(string, vector< float > &); // For files with one column
   ...
   *@brief   Write output
   */
  void write_array(string fname, vector<WebType>&out);
  //////////////////////////////////////////////////////////
  /**
   *@brief   Write output
   */
...
//////////////////////////////////////////////////////////
  /**
   * template<read_array_t>
   *@brief Write as many columns as we need
   */
...
template<typename T> void write_to_file(const std::string& fname,const std::vector<T>& columns)
{
    ofstream out(fname, std::ios::out);
    out.precision(_PREC_OUTPUT_); 
    out.setf(ios::showpoint); 
