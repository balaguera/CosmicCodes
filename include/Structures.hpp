...
# include <fstream>
# include <vector>
# include "def.hpp"  // Do not define it after gnuplot or python
using namespace std;
...
/** 
 * @brief Function to clear and shrink:to_fit all members if the structure.
*/
     ...
     auto clean_vec = [](auto& v)
      {
        ...
        clean_vec(number_sub_structures);
        clean_vec(identity);
        clean_vec(observed);
...
*/
  ...
  /**
   *@brief  Identify whether this has been observed or avoided (used in different processes inside the code)  */
  vector<bool> observed_secondary;
  // -----------------------------------
  /**
   *@brief   Weight (if any)
*/
...
struct s_data_structure_direct_sum{
   ...
   *@brief  Mean density of catalog*/
  Catalogue &cat;
  s_data_structure_direct_sum(Catalogue& c) : cat(c) {}
