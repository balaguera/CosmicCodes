...
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    def plot_angular_power_spectrum_measurement(self):
        ...
        X_min=0
        X_max=self.data["lmax"]
        X_max_max=X_max
        tick_x=50  # this sets the label every 0.05 in x
        min_locator_x=5
        if(self.data["statistics"]=="Cl"):
            Y_min=1e-5
            Y_max=1e-2
        else:
            Y_min=1e-9
        ...
        colors=["red", "blue", "green", "orange","cyan", "black", "red"]
        lines_s=["solid", "dotted", "dashed", "dashdot","solid", "dotted", "dashed", "dashdot","solid", "dotted", "dashed", "dashdot","solid", "dotted", "dashed", "dashdot"]
        labels=[r"$\Delta z_{0}$", r"$\Delta z_{1}$",r"$\Delta z_{2}$",r"\Delta z_{3}$",r"$P_{dm}$",r"$P_{dm,hf}$"]

        xtit, ytit=get_title_coords((X_min), (X_max), np.log10(Y_min), np.log10(Y_max), 0.15, 0.2)
        plt.text(xtit, pow(10,ytit),str(self.data["sample"]), fontsize=11)
        xtit, ytit=get_title_coords((X_min), (X_max), np.log10(Y_min), np.log10(Y_max), 0.15, 0.10)
        plt.text(xtit, pow(10,ytit),"Estimator : "+str(self.data["statistics"]), fontsize=11)

        number_of_tomographic_bins=self.data["number_of_zbins"]

        for i in range(0,number_of_tomographic_bins):

            file_name=self.data["output_file_"+str(i)]
            print("Reading file", file_name)
            if os.path.isfile(file_name):
                data=np.genfromtxt(file_name)     
                x=(data[:,0])
                ...
                epsc=data[:,4]
                xpc=data[:,2]-data[:,1]
                plt.scatter(x,psc,color=colors[i],alpha=0.4, label=labels[i])
                plt.errorbar(x,psc,yerr=epsc, xerr= 0.5*xpc, fmt="none", ecolor=colors[i], color=colors[i], alpha=0.6, linewidth=0.9,capsize=2)    
...
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    def plot_mixing_matrix_angular_power(self):
        ...
        lines_s=["solid", "dotted", "dashed", "dashdot","solid", "dotted", "dashed", "dashdot","solid", "dotted", "dashed", "dashdot","solid", "dotted", "dashed", "dashdot"]
        cmap = plt.cm.hsv  # puedes cambiar a 'viridis', 'turbo', etc.
        number_of_l_bins=self.data["number_of_lbins"]

        colors = cmap(np.linspace(0, 1, number_of_l_bins))

        xtit, ytit=get_title_coords((X_min), (X_max), Y_min, Y_max, 0.15, 0.8)
        plt.text(xtit, ytit,str(self.data["sample"]), fontsize=11)
        xtit, ytit=get_title_coords((X_min), (X_max), (Y_min), (Y_max), 0.15, 0.75)
        plt.text(xtit, ytit,"Estimator : "+str(self.data["statistics"]), fontsize=11)

        plt.title("Mixing matrix elements")
        for i in range(0,number_of_l_bins):

            file_name=self.data["output_file_mixing_matrix_lbin"+str(i)]
            print("Reading file", file_name)
            if os.path.isfile(file_name):
                data=np.genfromtxt(file_name)     
                x=(data[:,0])
                psc=data[:,1]
                plt.plot(x,psc,color=colors[i],alpha=0.4)
        ...
        plt.xscale('linear')
        plt.yscale('linear')
        plt.ylabel(r"$W(\delta \ell,\delta \ell')$", fontsize=12)
        plt.xlabel(r"$\ell$", fontsize=12)
        plt.grid(alpha=0.2)
...
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    def plot_tomographic_maps(self):
        ...
        number_of_tomographic_bins=self.data["number_of_zbins"]
        fsky=self.data["sky_fraction"]
        eps=1e-5
        for i in range(0,number_of_tomographic_bins): #one extra for the mask
            print("Reading file ", "output_file_map"+str(i))
            map = hp.read_map(self.data["output_file_map"+str(i)])
            map2=hp.pixelfunc.remove_monopole(map,copy=True ,fitval=True)
            monopole=map2[1]/fsky
            map_delta=(map/monopole)
            if(self.data["statistics"]=="Cl"):
                map_delta=np.log(map_delta+eps)

            minimo=np.min(map_delta)
            if(self.data["statistics"]=="Cl"):
                hp.visufunc.mollview(map_delta,title=self.data["sample"]+","+self.data["statistics"], unit=r'log(1+$\delta$)',min=minimo)
            else:
                hp.visufunc.mollview(map_delta,title=self.data["sample"]+","+self.data["statistics"], unit=r'$\delta$',min=minimo)
...
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    ...
    def plot_halo_mass_plane(self):
        ...
        plt.xlabel(r"Mass $\log M$", fontsize=10)
...
if plotCosmo.data.get("angular_power_spectrum_measurement", False):
    print("Showing measurements of angular power spectrum")
    plotCosmo.plot_angular_power_spectrum_measurement()

if plotCosmo.data.get("show_tomographic_maps", False):
    print("Showing maps")
    plotCosmo.plot_tomographic_maps()
