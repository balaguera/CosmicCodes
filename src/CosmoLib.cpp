...
void CosmoLib::get_hmodel(){
  ...
  if(this->params._use_file_power())
  {
...
#pragma omp parallel for
#endif
    for(size_t i=0; i<v_mass.size();i++)
...
#pragma omp parallel for
  ...
  for(size_t i=0; i<v_mass.size();i++) // DO not parallelize this loop.
...
#pragma omp parallel for
  ...
  #pragma omp parallel for
  #endif
      for(size_t i=0;i<v_k_ps.size();++i)
...
#pragma omp parallel for
  ...
  if(this->params._compute_non_linear_correlation_function_halofit() || this->params._compute_non_linear_correlation_function_pt())
    {
...
#pragma omp parallel for 
  ...
  if(this->params._compute_density_profile())
  {
...
#pragma omp parallel for 
  ...
  // ***********************************************************************************************
  if(this->params._compute_halo_model_galaxy_power_spectrum())
    {
...
// Para calcular esto es obligatorio calcular el espectro
  if(this->params._compute_halo_model_galaxy_correlation_function())
    {
