...

#include "../include/PowerSpectrumF.hpp"
using namespace std;
...
void PowerSpectrumF::measure_marked_correlation_function()
  {
    ...
    So.message_warning("This method still uses the AoS data model. If it's going to be used, please shift it to the SoA data model asap.");
...
#pragma omp parallel for reduction(+:mean_mass)
    ...
    mean_mass+=this->tracer_cat.catalogue.mass_at(i);
...
#endif
      for (ULONG i=0;i<this->N_galaxy; ++i)
    mean_vmax+=this->tracer_cat.catalogue.vmax_at(i);
...
#pragma omp for
      for (ULONG i=0;i<this->N_galaxy;++i)
    {
      ...
      real_prec x=this->tracer_cat.catalogue.coord1_at(i)+rsd_x*conversion_factor*vx;
      real_prec y=this->tracer_cat.catalogue.coord2_at(i)+rsd_y*conversion_factor*vy;
      real_prec z=this->tracer_cat.catalogue.coord3_at(i)+rsd_z*conversion_factor*vz;
#else
      real_prec x=this->tracer_cat.catalogue.coord1_at(i);
      real_prec y=this->tracer_cat.catalogue.coord2_at(i);
      real_prec z=this->tracer_cat.catalogue.coord3_at(i);
#endif
      real_prec mass=this->tracer_cat.catalogue.mass_at(i);
      real_prec sigma=this->tracer_cat.catalogue.vmax_at(i);
      for (ULONG j=i+1;j<this->N_galaxy;++j)
        {
          ...
          real_prec dx=x-(this->tracer_cat.catalogue.coord1[j]+rsd_x*conversion_factor*vxp);
          real_prec dy=y-(this->tracer_cat.catalogue.coord2[j]+rsd_y*conversion_factor*vyp);
          real_prec dz=z-(this->tracer_cat.catalogue.coord3[j]+rsd_z*conversion_factor*vzp);
#else
          real_prec dx=x-this->tracer_cat.catalogue.coord1[j];
          real_prec dy=y-this->tracer_cat.catalogue.coord2[j];
          real_prec dz=z-this->tracer_cat.catalogue.coord3[j];
#endif
          real_prec pmass=this->tracer_cat.catalogue.mass_at(j);
          real_prec psigma=this->tracer_cat.catalogue.vmax_at(j);
...
#pragma omp for
    ...
    this->tracer_cat.catalogue.clear_mem();
    So.DONE();
...
#endif
     for(ULONG i=0;i<this->params._NGRID();++i)
      fftw_functions.set_data_g(i,(static_cast<real_prec>(Y[i])/static_cast<real_prec>(nmean))-1.);
...
#endif
      for(ULONG i=0;i<this->params._NGRID();++i)
          fftw_functions.set_data_gp(i,(static_cast<real_prec>(X[i])/static_cast<real_prec>(nmean))-1.);
      }
...

void PowerSpectrumF::measure_power_spectrum_data()
{
  bool verbose=true;
  ...
  if(this->params._statistics()=="Pk_fkp")
    ...
    So.welcome_message_yama();

  string file_pow=this->file_power;      // Output file, keep roor in case intervals are used
 
  if(CoordinateSystem::EQZ==this->params._sys_of_coord_g())
 ...
 // Tabulate r-z relation if the radial coordinate is redshift
  ...
  Catalogue random(this->params, "RANDOM");                   //Define a random Catalogue like object
  s_data_structure_direct_sum s_data_struct_r_ds(random); // this is defiend here for the fkp error bars need the info from the randoms. So far at this point it is only defiend but not filled

#ifdef _USE_SEVERAL_RANDOM_FILES_
  for(int ir=0;ir<this->params._NRANDOMfiles();++ir)
    {
      random.read_catalog_new(this->params._Input_dir_cat()+ this->params._RANDOMfile(ir)); //Read it.
#else
     random.read_catalog_new(this->params._Input_dir_cat()+ this->file_random);            //Read it.
#endif

     HaloTools htools_ran(this->params, random);
  
// STEO 2
     if(this->params._sys_of_coord_r()!=CoordinateSystem::CART) // If using spherical coordinates
       {
         // The new Lbox is updated in this method into the instance params belonging to the class random_cat
         if(!this->params._nbar_tabulated() && !this->params._use_file_nbar())// extra security check
          {
             measure_dndz=true;
#ifdef _USE_SEVERAL_RANDOM_FILES_
            if(0==ir)// resize only when reading the first file, For the others, we just keep on filling it.
              ...
              dndz_v.resize(z_v.size(),0);
#endif
               // First second argument of the following method is alpha. Below the dndz_v will be rescaled by the ratio Ngal/Nran once Ngal is read
             htools_ran.get_mean_number_density(1.0,vzz,vrc,z_v,dndz_v); //Compute mean number denisty from random catalogue
          ...
          // Read the nbar file provided from the parameter file, done only for the first "ir"
                vector<real_prec> nb;
                ULONG nzl=this->File.read_file(this->params._file_nbar(),nb, 1);  // Read external filw with tabulated number density
            ...
            else if(params._nbar_tabulated())
              {
#ifdef _USE_SEVERAL_RANDOM_FILES_
                if(0==ir)// resize only when reading the first file, For the others, we just keep on filling it.
                 {
            ...
            // 3
#ifdef _USE_SEVERAL_RANDOM_FILES_
            htools.ang_to_cart_coordinates(random, &s_data_struct_prop,ir); // Transform the coordinates of random to cartessian coordinates
#else
            htools_ran.ang_to_cart_coordinates(&s_data_struct_prop);    // Transform the coordinates of random to cartessian coordinates
#endif
            // 4: Interpolate on the mesh
            if(this->params._statistics()=="Pk_y_ds")
              {
                s_data_struct_r_ds.mean_density = mean_density;
                s_data_struct_r_ds.compute_dndz = measure_dndz;
                s_data_struct_r_ds.zz_v = z_v;
                s_data_struct_r_ds.dndz_v = dndz_v;
                s_data_struct_r_ds.zz_c = vzz;
                s_data_struct_r_ds.rr_c = vrc;
                fftw_functions.get_power_moments_fourier_grid_ds_yam(&s_data_struct_r_ds); //Yamamoto direct sum 
              }
            else
             {
#ifdef _USE_SEVERAL_RANDOM_FILES_
              htools.get_interpolated_density_field(random,false,"any",ir);  // Interpolate random catalog on a mesh
#else
            htools_ran.get_interpolated_density_field(false,"any");  // Interpolate random catalog on a mesh
#endif
            }
// 5 Update
#ifdef _USE_SEVERAL_RANDOM_FILES_
            ...
            if(this->params._get_window_matrix())
              {
                ...
                this->get_window_matrix_multipole(random);
              }
#endif
            // 7 Release memmory
            random.clear_mem();
...
// *****************************************************************************************
       ...
       Catalogue galaxy(this->params, this->params._type_of_object());   //Define a galaxy Catalogue like object
       galaxy.read_catalog_new(this->params._Input_dir_cat()+ this->file_data);   //Read it.
       HaloTools htools_gal(this->params, galaxy);
...
#else
          htools_gal.ang_to_cart_coordinates(&s_data_struct_prop);
#endif
        }
      // Do direct sum or interpolate on a mesh to do fftw
        if(this->params._statistics()=="Pk_y_ds")//will be deprecated
          {
            s_data_structure_direct_sum s_data_struct_g_ds(galaxy);
            s_data_struct_g_ds.mean_density = mean_density;
            s_data_struct_g_ds.compute_dndz = measure_dndz;
            s_data_struct_g_ds.zz_v = z_v;
            s_data_struct_g_ds.dndz_v = dndz_v;
            s_data_struct_g_ds.zz_c = vzz;
            s_data_struct_g_ds.rr_c = vrc;
           ...
           htools_gal.get_interpolated_density_field(false,"any",0); // Interpolate tracer catalog on a mesh
#else
          htools_gal.get_interpolated_density_field(false,"any"); // Interpolate tracer catalog on a mesh
#endif
        ...
        this->fftw_functions.set_n_gal(htools_gal._n_gal());
        this->fftw_functions.set_w_g(htools_gal._w_g());
        ...
        if(this->params._use_random_catalog())
          {
            this->fftw_functions.set_n_ran(htools_ran._n_gal());
            this->fftw_functions.set_w_r(htools_ran._w_g());
            this->fftw_functions.set_s_r(htools_ran._s_g());
            this->fftw_functions.set_sr1(htools_ran._sg1());
            this->fftw_functions.set_sr2(htools_ran._sg2());
            this->fftw_functions.set_normal_p(htools_ran._normal_p());
            this->fftw_functions.set_normal_b(htools_ran._normal_b());
          }
...
#endif
        for(ULONG i=0;i<this->params._NGRID();++i)
           this->fftw_functions.set_data_g(i,htools_gal.field_external[i]);// Set rho_r to its container in fftw_functions

        if(!this->params._use_random_catalog())
          this->fftw_functions.raw_sampling(pow(this->params._Lbox(),3));//This set normalization and SN for box
        else  if(this->params._use_random_catalog())
          {
...
#pragma omp parallel for
#endif
          for(ULONG i=0;i<this->params._NGRID();++i)
            this->fftw_functions.set_data_r(i,htools_ran.field_external[i]); // Set rho_r to its container in fftw_functions
        ...
        if(this->params._Get_tracer_relative_bias())
          {
            ...
            htools_gal.select_random_subsample_v(this->params._fraction_dilute()); // Esto generara un .observed=1 si lo tomamos o 0 si no
            this->object_by_object_rbias(galaxy, htools_gal, htools_ran); // Assign and print downsampled catalog
            galaxy.clear_mem();
          }

         // Free the memmory used for the interpolation of tracer and randoms
        htools_gal.field_external.clear();
        htools_gal.field_external.shrink_to_fit();

        htools_ran.field_external.clear();
        htools_ran.field_external.shrink_to_fit();
        ...
        if(this->params._statistics()=="Pk_fkp" || this->params._statistics()=="Pk_ys" || this->params._statistics()=="Pk_yb"  || this->params._statistics()=="Pk_ybc"   || this->params._statistics()=="Pk_ysc" ||  this->params._statistics()=="Pk_y_ds")
          ...
          else if(this->params._statistics()=="Bk_fkp")  // Estimates of Bispectrum. Using the DFT already done for P(k)
           {
...

void PowerSpectrumF::measure_power_spectrum_box()
{
  bool verbose=true;
  So.enter(__PRETTY_FUNCTION__);
      ...
      if(this->params._statistics()=="Pk_fkp")
        So.welcome_message();
      if(this->params._statistics()=="Pk_ys" || this->params._statistics()=="Pk_yb" || this->params._statistics()=="Pk_ybc" || this->params._statistics()=="Pk_ysc" || this->params._statistics()=="Pk_y_ds")
        So.welcome_message_yama();
#endif
...
// Define some structures here
  s_data_structure s_data_struct_r;                 // This is defiend here for the fkp error bars need the info from the randoms. So far at this point it is only defiend but not filled
    ...
    if(CoordinateSystem::CART==this->params._sys_of_coord_g() )
...
// *****************************************************************************************
    ...
    // Read
     ...
     HaloTools htools_gal(params, galaxy);

     htools_gal.get_interpolated_density_field(false,"any"); // Interpolate tracer catalog on a mesh

    // Free memmory if necessary. If not, released below
     size_t ngal_new = galaxy._NOBJS();
     galaxy.clear_mem();
...
#pragma omp parallel for 
      ...
      this->fftw_functions.raw_sampling(vol);//This set normalization and SN for box
      real_prec nmean=static_cast<double>(ngal_new)/static_cast<double>(this->params._NGRID());
      /**********/
      #ifdef _USE_OMP_
#pragma omp parallel for
#endif
       for(ULONG i=0;i<this->params._NGRID();++i)
         this->fftw_functions.set_data_g(i,static_cast<real_prec>(htools_gal.field_external[i]));

      fftw_functions.get_fluctuation();
  
      htools_gal.field_external.clear();         // Free the memmory used for the interpolation of tracer and randoms
      htools_gal.field_external.shrink_to_fit();
            ...
            if(this->params._statistics()=="Pk_fkp")
              {
                ...
                if(true==params._FKP_error_bars())
                  {
                    So.message("This option has been disabled");
//                    this->fftw_functions.get_fkp_error_bars(&s_data_struct_r_ds, kvector_data, this->pk0, this->modes_g, this->sigma_fkp);
                  }
              }
              else if(this->params._statistics()=="Pk_yb" || this->params._statistics()=="Pk_ybc" || this->params._statistics()=="Pk_ys" || this->params._statistics()=="Pk_y_ds" || this->params._statistics()=="Pk_ysc" )
...
void PowerSpectrumF::measure_window_function(){
    ...
    string file_pow=this->file_power;
      ...
      if(this->params._statistics()=="Pk_fkp")
        So.welcome_message();
      if(this->params._statistics()=="Bk_fkp")
        So.welcome_message_bispectrum();
       if(this->params._statistics()=="Bk_fkp_fast")
        So.welcome_message_bispectrum_fast();
      if(this->params._statistics()=="Pk_ys" || this->params._statistics()=="Pk_yb" || this->params._statistics()=="Pk_ybc" || this->params._statistics()=="Pk_ysc" || this->params._statistics()=="Pk_y_ds")
        So.welcome_message_yama();
#endif
        if(CoordinateSystem::EQR==this->params._sys_of_coord_g())
          So.write_cosmo_parameters((void *)&this->params.s_cosmo_pars);
        ...
        Catalogue random(params, "RANDOM");
        random.read_catalog_new(this->params._Input_dir_cat_random()+this->file_random);

        HaloTools htools(params, random);
        ...
        if(this->params._sys_of_coord_g()==CoordinateSystem::EQR ||this->params._sys_of_coord_g()==CoordinateSystem::EQZ )
        ...
        if(this->params._use_random_catalog())
          alpha_0 = static_cast<real_prec>(this->N_galaxy)/static_cast<real_prec>(this->N_random);
        ...
        if(true==this->params._use_random_catalog() && false==params._nbar_tabulated())
          {
            ...
            vector<gsl_real> dndz_v;
            dndz_v.resize(z_v.size(),0);
            htools.get_mean_number_density(alpha_0,vzz,vrc,z_v,dndz_v);
          }
    ...
    // Transforming to cartessian coord.- and searching for box side lenght.
        if( this->params._sys_of_coord_r()!=CoordinateSystem::CART)
          {
            ...
            this->random_cat.ang_to_cart_coordinates(random, &s_data_struct_r,0);
#else
            htools.ang_to_cart_coordinates(&s_data_struct_r);
#endif
            if(true==this->params._new_Lbox())//if this is true, the params need to be updated
            {
              this->set_params(htools.params); // Update the instance parms in this class from the instance params in htools
              this->fftw_functions.set_params(htools.params); //Update the instance parms  in the class fftw_functions from the instance params in random_cat
            }
       }
...

void PowerSpectrumF::measure_power_spectrum(bool verbose, Catalogue&  tracer_cat)
  {

   if(tracer_cat._NOBJS()==0)
    {
    ...
    HaloTools htools(params, tracer_cat);
      ...
      if( this->params._sys_of_coord_g()==CoordinateSystem::EQZ)
        So.write_cosmo_parameters((void *)&this->params.s_cosmo_pars);

        // Define some structures here
      s_parameters_box s_p_box;
      s_data_structure_direct_sum s_data_struct_r_ds(tracer_cat); // this is defiend here for the fkp error bars need the info from the randoms. So far at this point it is only defiend but not filled


      // Add random and galaxy catalogues

      this->N_galaxy=tracer_cat._NOBJS();
...
#ifdef _USE_SEVERAL_RANDOM_FILES_
      htools.get_interpolated_density_field(false, "any",0);
#else
      htools.get_interpolated_density_field(false, "any");
#endif
     // Here we need to pass the fields from catalog to fftw:
...
#endif
     for(ULONG i=0;i<this->params._NGRID();++i)
          this->fftw_functions.set_data_g(i,htools.field_external[i]);

      htools.field_external.clear();htools.field_external.shrink_to_fit();
...

 void PowerSpectrumF::measure_power_spectrum(Catalogue& tracer_cat, string space_p,string property)
 {
  ...
  this->So.enter(__PRETTY_FUNCTION__);
   ...
   for(int im=0; im< N_intervals;++im)
     ...
     this->File.input_type=this->params._input_type();
     ULONG in_new=0;
     Catalogue final_cat(params, "TRACER"); //* Define a catalog structure 

     if(property==_MASS_)
       {
        if(space_p=="redshift")
         {
          for(ULONG i=0;i<tracer_cat._NOBJS();++i)
           if(tracer_cat.mass_at(i)>=pow(10,this->params._MASSbins_min(im)) && tracer_cat.mass_at(i)<pow(10,this->params._MASSbins_max(im)))
           {
            final_cat.push_mass(tracer_cat.mass_at(i));
            final_cat.push_coord1(tracer_cat.coord1_at(i));
            final_cat.push_coord2(tracer_cat.coord2_at(i));
            final_cat.push_coord3(tracer_cat.coord3_at(i));
            final_cat.push_vel1(tracer_cat.vel1_at(i));
            final_cat.push_vel2(tracer_cat.vel2_at(i));
            final_cat.push_vel3(tracer_cat.vel3_at(i));
         }
       }
       ...
       else if (property==_VMAX_)
     {
       if(space_p=="redshift")
         {
           for(ULONG i=0;i<tracer_cat._NOBJS();++i)
           if(log10(tracer_cat.vmax_at(i))>=this->params._VMAXbins_min(im) && log10(tracer_cat.vmax_at(i))< this->params._VMAXbins_max(im))
            {
            final_cat.push_vmax(tracer_cat.vmax_at(i));
            final_cat.push_coord1(tracer_cat.coord1_at(i));
            final_cat.push_coord2(tracer_cat.coord2_at(i));
            final_cat.push_coord3(tracer_cat.coord3_at(i));
            final_cat.push_vel1(tracer_cat.vel1_at(i));
     ...
     HaloTools htools_new(params, final_cat);
     ...
     if(space_p=="real")
       htools_new.get_interpolated_density_field_real_space(false,"any");
     else if (space_p=="redshift")
       htools_new.get_interpolated_density_field(false,"any");

     So.message_screen("Number of tracers interpolated",final_cat._NOBJS());
     this->fftw_functions.resize_fftw_vectors();
     this->fftw_functions.write_fftw_parameters();
     this->fftw_functions.set_n_gal(htools_new._n_gal());
     this->fftw_functions.set_w_g(htools_new._w_g());
     this->fftw_functions.set_s_g(htools_new._s_g());
     this->fftw_functions.set_sg1(htools_new._sg1());
     this->fftw_functions.set_sg2(htools_new._sg2());
...
#pragma omp parallel for
     ...
     for(ULONG i=0;i<this->params._NGRID();++i)// Here we need to pass the fields from catalog to fftw:
       this->fftw_functions.set_data_g(i,htools.field_external[i]);// Set rho_r to its container in fftw_functions
...
void PowerSpectrumF::halo_bias_analysis(string space_p)
 {
   ...
   // **************************************************************************************************
   Catalogue tracer(params);
   tracer.read_catalog_new(params._Input_dir_cat()+params._file_catalogue());
   this->set_params(tracer.params); // params has been updated inside the catalog class.
   HaloTools htools(params, tracers);
   htools.Get_SO_tracer();
   // **************************************************************************************************
...
#pragma omp parallel for
   ...
   if(true==this->params._Get_tracer_bias())
     {
       ...
       if(true==this->params._Get_tracer_relative_bias())
       {
         ...
         this->object_by_object_bias(tracer.catalogue,DM_DEN_FIELD,TR_DEN_FIELD);
       }
       else
        this->object_by_object_bias(tracer,DM_DEN_FIELD);
   ...
   if(true==this->params._Get_tracer_quadratic_bias())
       {
         ...
         this->object_by_object_qbias(tracer,DM_DEN_FIELD);
       }
    }
   // **************************************************************************************************
#ifdef _USE_PEAK_HEIGHT_AS_PRIMARY_PROPERTY_HBIAS_
   ...
   hatools.get_peak_height_at_tracer();
#else
   if(true==this->params._Get_peak_height_at_halo())
     {
       htools.get_peak_height_at_tracer();
   ...
   tracer.resize_gal_cwt(tracer._NOBJS());
    #pragma omp parallel for
   for(ULONG i=0;i<tracer._NOBJS();++i)
      tracer.set_gal_cwt(this->cwclass.CWClass[tracer.GridID_at(i)],i); //esto asigna 1, 2, 3, 4
   ...
   if(true==this->params._Get_tidal_anisotropy_at_halo())
     {
       ...
       tracer.resize_tidal_anisotropy(tracer._NOBJS());
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
       for(ULONG i=0;i<this->params._NGRID() ;++i)
...
#pragma omp parallel for
   ...
   for(ULONG i=0;i<tracer._NOBJS() ;++i) //loop over the "observed obejcts", i.e, with cuts already set
    {
       tracer.catalogue.lambda1[i] =this->cwclass.lambda1[tracer.catalogue.GridID[i]];
       tracer.catalogue.lambda2[i] =this->cwclass.lambda2[tracer.catalogue.GridID[i]];
       tracer.catalogue.lambda3[i] =this->cwclass.lambda3[tracer.catalogue.GridID[i]];
    }
...
#pragma omp parallel for
    ...
    for(ULONG i=0; i< tracer._NOBJS(); ++i)
      tracer.catalogue.mean_number_density[i] = static_cast<real_prec>(gsl_spline_eval (spline, tracer.catalogue.mass_at(i), acc));
  }
 ...
 #pragma omp parallel for
    for(ULONG i=0;i<tracer._NOBJS();++i)
       tracer.catalogue.local_dm_at(i)=log10(1+DM_OVERDENSITY_FIELD[tracer.catalogue.GridID[i]]);
...
/*
    ...
    tracer_randomize.Halo=tracer.catalogue;
   ...
   for(int icw=0;icw<=Ncwt;icw++)  // loop over nuber of cosmic-web types. ALL=0 knots. 1 = filaments: 2=sheets; 3 =voids
       ...
       for(int im_primary=0; im_primary< Nprimary_bins;++im_primary)
     {
...
#endif
       HaloTools catalogue_aux(this->params);
       ...
       if(icw==0)// THis selects all objects regardless their CW clasiffication.
         ...
         {
#ifdef _USE_PEAK_HEIGHT_AS_PRIMARY_PROPERTY_HBIAS_
         if(tracer.catalogue.peak_height[i]>= this->params._PHbins_min(im_primary) && tracer.catalogue.peak_height[i]< this->params._PHbins_max(im_primary))
#elif defined  _USE_MASS_AS_PRIMARY_PROPERTY_HBIAS_
           if(tracer.catalogue.mass_at(i)>= pow(10,this->params._MASSbins_min(im_primary)) && tracer.catalogue.mass_at(i)< pow(10,this->params._MASSbins_max(im_primary)))
#elif defined  _USE_VMAX_AS_PRIMARY_PROPERTY_HBIAS_
             if(tracer.catalogue.vmax_at(i)>= pow(10,this->params._VMAXbins_min(im_primary)) && tracer.catalogue.vmax_at(i)< pow(10,this->params._VMAXbins_max(im_primary)))
#endif
               {
                  catalogue_aux.GridID.push_back(tracer.catalogue.GridID[i]);
                  catalogue_aux.coord1.push_back(tracer.catalogue.coord1_at(i));
                  catalogue_aux.coord2.push_back(tracer.catalogue.coord2_at(i));
                  catalogue_aux.coord3.push_back(tracer.catalogue.coord3_at(i));
                  catalogue_aux.vel1.push_back(tracer.catalogue.vel1_at(i));
                  catalogue_aux.vel2.push_back(tracer.catalogue.vel2_at(i));
                  catalogue_aux.vel3.push_back(tracer.catalogue.vel3_at(i));
                  catalogue_aux.mass.push_back(tracer.catalogue.mass_at(i));
                  catalogue_aux.vmax.push_back(tracer.catalogue.vmax_at(i));
                  catalogue_aux.rs.push_back(tracer.catalogue.rs[i]);
                  catalogue_aux.rvir.push_back(tracer.catalogue.rvir[i]);
                  catalogue_aux.concentration.push_back(tracer.catalogue.concentration_at(i));
                  catalogue_aux.spin.push_back(tracer.catalogue.spin_at(i));
                  catalogue_aux.spin_bullock.push_back(tracer.catalogue.spin_bullock[i]);
                  catalogue_aux.vrms.push_back(tracer.catalogue.vrms(i));
                  catalogue_aux.virial.push_back(tracer.catalogue.virial[i]);
                  catalogue_aux.b_to_a.push_back(tracer.catalogue.b_to_a[i]);
                  catalogue_aux.c_to_a.push_back(tracer.catalogue.c_to_a[i]);
                  catalogue_aux.mach_number.push_back(tracer.catalogue.mach_number_at(i));
                  catalogue_aux.local_overdensity.push_back(tracer.catalogue.local_overdensity[i]);
                  catalogue_aux.bias.push_back(tracer.catalogue.bias_at(i));
                  catalogue_aux.tidal_anisotropy.push_back(tracer.catalogue.tidal_anisotropy_at(i));
                  catalogue_aux.peak_height.push_back(tracer.catalogue.peak_height[i]);
                  catalogue_aux.dach_number.push_back(tracer.catalogue.dach_number[i]);
                  catalogue_aux.local_dm.push_back(tracer.catalogue.local_dm_at(i));              }//closes if
         } // closes loop
         }//closes if cwc
#ifdef _USE_CWC_HALO_ANALYSIS_
       else if(icw>0)
         {
           for(ULONG i=0;i<tracer._NOBJS();++i)
         {
           if(tracer.catalogue.gal_cwt[i]==static_cast<WebType>(ict))
             {
#ifdef _USE_PEAK_HEIGHT_AS_PRIMARY_PROPERTY_HBIAS_
               if(tracer.catalogue[i].peak_height>= this->params._PHbins_min(im_primary) && tracer.catalogue[i].peak_height< this->params._PHbins_max(im_primary))
#elif defined  _USE_MASS_AS_PRIMARY_PROPERTY_HBIAS_
             if(tracer.catalogue.mass_at(i)>= pow(10,this->params._MASSbins_min(im_primary)) && tracer.catalogue.mass_at(i)< pow(10,this->params._MASSbins_max(im_primary)))
#elif defined  _USE_VMAX_AS_PRIMARY_PROPERTY_HBIAS_
               if(tracer.catalogue.vmax_at(i)>= pow(10,this->params._VMAXbins_min(im_primary)) && tracer.catalogue.vmax_at(i)< pow(10,this->params._VMAXbins_max(im_primary)))
#endif
                 {
                  ...
                  catalogue_aux.peak_height.push_back(tracer.catalogue.peak_height[i]);
                  catalogue_aux.dach_number.push_back(tracer.catalogue.dach_number[i]);
                  catalogue_aux.local_dm.push_back(tracer.catalogue.local_dm_at(i));
                  } // closes if
             } // closes if
         } // closes loop
         }// closes else if
...
#endif
       catalogue_aux.set_NOBJS(counter_m);
       // ************************************************************************************************************
      ...
      So.message_screen("Measuring marked power spectrum:");
       ...
       this->N_galaxy=catalogue_aux.coord1.size();
       real_prec mean_density=static_cast<real_prec>(catalogue_aux.coord1.size())/pow(this->params._Lbox(),3);
       s_data_struct_g_marked.mean_density=mean_density;
       fftw_functions.set_n_gal(catalogue_aux.coord1.size());
       ...
       first_sec_property_used=0;//the first secondary property used the mass, labled 0
        ...
        for(int Nbb=0;Nbb<Number_of_properties;++Nbb) // Loop over the  properties to be used as marks, in bins of the primary poroperty
         ...
         {
           pair<real_prec,real_prec>stats=catalogue_aux.get_variance(prop_name[Nbb],false);// false means that it gives <prop²> istead of <(prop-mean)²>
           ...
           real_prec nmean=static_cast<real_prec>(catalogue_aux.coord1.size())/static_cast<real_prec>(this->params._NGRID()); // mean number of tracers in cells
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
...
#pragma omp parallel for
       ...
       if(true==this->params._set_bins_equal_number_tracers() && tracer._Number_of_tracers_in_ph_bins(im_primary)>0)// if not, read those from the para files as have been provided
        ...
        if(tracer._Number_of_tracers_in_mass_bins(im_primary)/(this->params._Number_of_bins_equal_number_tracers()) >= 1 )
        {
         ...
         if(true==bmass)
           {
             binn_l<<"#Mass:"<<endl;
             catalogue_aux.get_intervals_equal_number_aux("_MASS_");
             So.message_screen("Writing bining scheme in file ", file_bins );
             for(int i=0;i<catalogue_aux.params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._MASSbins_min(i)<<"  "<<catalogue_aux.params._MASSbins_max(i)<<"\t"<< endl;
           }
#endif
         ...
         if(true==bvmax)
           {
             ...
             binn_l<<"#Vmax:"<<endl;
             catalogue_aux.get_intervals_equal_number_aux("_VMAX_");
             for(int i=0;i<catalogue_aux.params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._VMAXbins_min(i)<<"  "<<catalogue_aux.params._VMAXbins_max(i)<<"\t"<< endl;
             for(int i=0;i<catalogue_aux.params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._VMAXbins_min(i)<<"  "<<catalogue_aux.params._VMAXbins_max(i)<<"\t"<< endl;
            binn.close();
         }
#endif
         if(true==used_prop[label_rs])
           {
             ...
             binn.open(file_bins.c_str());
         binn_l<<"#RS:"<<endl;
         catalogue_aux.get_intervals_equal_number_aux("_RS_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._RSbins_min(i)<<"  "<<catalogue_aux.params._RSbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._RSbins_min(i)<<"  "<<catalogue_aux.params._RSbins_max(i)<<endl;
         binn.close();
        }
         if(true==used_prop[label_rvir])
           {
             ...
             binn.open(file_bins.c_str());
         binn_l<<"#RVIR:"<<endl;
         catalogue_aux.get_intervals_equal_number_aux("_RVIR_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._RVIRbins_min(i)<<"  "<<catalogue_aux.params._RVIRbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._RVIRbins_min(i)<<"  "<<catalogue_aux.params._RVIRbins_max(i)<<endl;
            binn.close();
         }
         ...
         catalogue_aux.get_intervals_equal_number_aux("_CONCENTRATION_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._CONCENTRATIONbins_min(i)<<"  "<<catalogue_aux.params._CONCENTRATIONbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._CONCENTRATIONbins_min(i)<<"  "<<catalogue_aux.params._CONCENTRATIONbins_max(i)<<endl;
         binn.close();
        }
         if(true==used_prop[label_spin])
           {
             ...
             binn.open(file_bins.c_str());
         binn_l<<"#SPIN:"<<endl;
         catalogue_aux.get_intervals_equal_number_aux("_SPIN_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._SPINbins_min(i)<<"  "<<catalogue_aux.params._SPINbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._SPINbins_min(i)<<"  "<<catalogue_aux.params._SPINbins_max(i)<<endl;
         binn.close();
    }
         if(true==used_prop[label_spin_bullock])
           {
             ...
             binn.open(file_bins.c_str());
             So.message_screen("Writing bining scheme in file ", file_bins );
          catalogue_aux.get_intervals_equal_number_aux("_SPIN_BULLOCK_");
          for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
            binn_l<<i<<"  "<<catalogue_aux.params._SPINBULLOCKbins_min(i)<<"  "<<catalogue_aux.params._SPINBULLOCKbins_max(i)<<endl;
          for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
            binn<<i<<"  "<<catalogue_aux.params._SPINBULLOCKbins_min(i)<<"  "<<catalogue_aux.params._SPINBULLOCKbins_max(i)<<endl;
          binn.close();
         }
         ...
         binn.open(file_bins.c_str());
         So.message_screen("Writing bining scheme in file ", file_bins );
         catalogue_aux.get_intervals_equal_number_aux("_VRMS_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._VRMSbins_min(i)<<"  "<<catalogue_aux.params._VRMSbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._VRMSbins_min(i)<<"  "<<catalogue_aux.params._VRMSbins_max(i)<<endl;
         binn.close();
        }
         ...
         binn.open(file_bins.c_str());
         So.message_screen("Writing bining scheme in file ", file_bins );
         catalogue_aux.get_intervals_equal_number_aux("_VIRIAL_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._VIRIALbins_min(i)<<"  "<<catalogue_aux.params._VIRIALbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._VIRIALbins_min(i)<<"  "<<catalogue_aux.params._VIRIALbins_max(i)<<endl;
         binn.close();
        }
         ...
         binn.open(file_bins.c_str());
         So.message_screen("Writing bining scheme in file ", file_bins );
         catalogue_aux.get_intervals_equal_number_aux("_BTOA_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._BTOAbins_min(i)<<"  "<<catalogue_aux.params._BTOAbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._BTOAbins_min(i)<<"  "<<catalogue_aux.params._BTOAbins_max(i)<<endl;
         binn.close();
         }
         ...
         binn.open(file_bins.c_str());
         So.message_screen("Writing bining scheme in file ", file_bins );
         catalogue_aux.get_intervals_equal_number_aux("_CTOA_");
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn_l<<i<<"  "<<catalogue_aux.params._CTOAbins_min(i)<<"  "<<catalogue_aux.params._CTOAbins_max(i)<<endl;
         for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
           binn<<i<<"  "<<catalogue_aux.params._CTOAbins_min(i)<<"  "<<catalogue_aux.params._CTOAbins_max(i)<<endl;
         binn.close();
            }
         if(true==mach)
           {
             ...
             So.message_screen("Writing bining scheme in file ", file_bins );
             binn.open(file_bins.c_str());
             catalogue_aux.get_intervals_equal_number_aux("_MACH_");
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._MACHbins_min(i)<<"  "<<catalogue_aux.params._MACHbins_max(i)<<endl;
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._MACHbins_min(i)<<"  "<<catalogue_aux.params._MACHbins_max(i)<<endl;
             binn.close();
           }
         if(true==tbias)
           {
             ...
             So.message_screen("Writing bining scheme in file ", file_bins );
             binn.open(file_bins.c_str());
             catalogue_aux.get_intervals_equal_number_aux("_BIAS_");
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._BIASbins_min(i)<<"  "<<catalogue_aux.params._BIASbins_max(i)<<endl;
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._BIASbins_min(i)<<"  "<<catalogue_aux.params._BIASbins_max(i)<<endl;
             binn.close();
           }
         if(true==tlc)
           {
             ...
             So.message_screen("Writing bining scheme in file ", file_bins );
             binn.open(file_bins.c_str());
             catalogue_aux.get_intervals_equal_number_aux("_LOCAL_OVERDENSITY_");
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._LCbins_min(i)<<"  "<<catalogue_aux.params._LCbins_max(i)<<endl;
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._LCbins_min(i)<<"  "<<catalogue_aux.params._LCbins_max(i)<<endl;
             binn.close();
           }
         if(true==bta)
           {
             ...
             So.message_screen("Writing bining scheme in file ", file_bins );
             binn.open(file_bins.c_str());
             catalogue_aux.get_intervals_equal_number_aux("_TIDAL_ANISOTROPY_");
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._TAbins_min(i)<<"  "<<catalogue_aux.params._TAbins_max(i)<<endl;
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._TAbins_min(i)<<"  "<<catalogue_aux.params._TAbins_max(i)<<endl;
             binn.close();
           }
         ...
         if(true==bph)
           {
             ...
             So.message_screen("Writing bining scheme in file ", file_bins );
             binn.open(file_bins.c_str());
             catalogue_aux.get_intervals_equal_number_aux("_PEAK_HEIGHT_");
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._PHbins_min(i)<<"  "<<catalogue_aux.params._PHbins_max(i)<<endl;
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._PHbins_min(i)<<"  "<<catalogue_aux.params._PHbins_max(i)<<endl;
             binn.close();
           }
#endif
         ...
         if(true==dach)
           {
             ...
             So.message_screen("Writing bining scheme in file ", file_bins );
             binn.open(file_bins.c_str());
             catalogue_aux.get_intervals_equal_number_aux("_DACH_");
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._DACHbins_min(i)<<"  "<<catalogue_aux.params._DACHbins_max(i)<<endl;
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._DACHbins_min(i)<<"  "<<catalogue_aux.params._DACHbins_max(i)<<endl;
             binn.close();
           }
         if(true==dm_local)
           {
             ...
             So.message_screen("Writing bining scheme in file ", file_bins );
             binn.open(file_bins.c_str());
             catalogue_aux.get_intervals_equal_number_aux("_LOCALDM_");
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn_l<<i<<"  "<<catalogue_aux.params._LOCALDMbins_min(i)<<"  "<<catalogue_aux.params._LOCALDMbins_max(i)<<endl;
             for(int i=0;i<params._Number_of_bins_equal_number_tracers()+1;++i)
               binn<<i<<"  "<<catalogue_aux.params._LOCALDMbins_min(i)<<"  "<<catalogue_aux.params._LOCALDMbins_max(i)<<endl;
             binn.close();
           }
         binn_l.close();
         So.DONE();
    ...
    int imass=0;int ir=0;int iv=0;int irv=0;int icon=0; int is=0; int isb=0;int irm=0;int ivir=0; int iba=0; int ica=0; int ima=0; int ibi=0; int ilc=0; int ita=0; int ildm=0; int ida=0;int iph=0;
                                         ...
                                         HaloTools tracer_final(this->params);
                                         ULONG in_new=0;  // counter for sub sample
                                         real_prec bias_from_objects=0;
                                         vector<real_prec>bias_ob;
                                         // We select now properties in bins of secondary properties. Selection in CWT and Mass has been already imposed in the catalogue_aux catalog.
                                         for(ULONG i=0;i<catalogue_aux.coord1.size();++i)
                                         {
#ifndef _USE_MASS_AS_PRIMARY_PROPERTY_HBIAS_
                                           if(log10(catalogue_aux.mass_at(i))>= catalogue_aux.params._MASSbins_min(mass_index) && log10(catalogue_aux.mass_at(i)) < catalogue_aux.params._MASSbins_max(mass_index))
#endif
#ifndef _USE_VMAX_AS_PRIMARY_PROPERTY_HBIAS_
                                         if(log10(catalogue_aux.vmax_at(i))>= catalogue_aux.params._VMAXbins_min(vmax_index) && log10(catalogue_aux.vmax_at(i))< catalogue_aux.params._VMAXbins_max(vmax_index))
#endif
                                           if(catalogue_aux.rs[i]>= catalogue_aux.params._RSbins_min(ir) && catalogue_aux.rs[i]< catalogue_aux.params._RSbins_max(ir))
                                               if(catalogue_aux.rvir[i]>= catalogue_aux.params._RVIRbins_min(irv) && catalogue_aux.rvir[i]< catalogue_aux.params._RVIRbins_max(irv))
                                                   if(catalogue_aux.concentration_at(i) >= catalogue_aux.params._CONCENTRATIONbins_min(icon) && catalogue_aux.concentration_at(i)< catalogue_aux.params._CONCENTRATIONbins_max(icon))
                                               if(catalogue_aux.spin_at(i)>= catalogue_aux.params._SPINbins_min(is) && catalogue_aux.spin_at(i)< catalogue_aux.params._SPINbins_max(is))
                                                   if(catalogue_aux.spin_bullock[i]>= catalogue_aux.params._SPINBULLOCKbins_min(isb) && catalogue_aux.spin_bullock[i]< catalogue_aux.params._SPINBULLOCKbins_max(isb))
                                               if(catalogue_aux.vrms(i)>= catalogue_aux.params._VRMSbins_min(irm) && catalogue_aux.vrms(i)< catalogue_aux.params._VRMSbins_max(irm))
                                             if(catalogue_aux.virial[i]>= catalogue_aux.params._VIRIALbins_min(ivir) && catalogue_aux.virial[i]< catalogue_aux.params._VIRIALbins_max(ivir))
                                               if(catalogue_aux.b_to_a[i]>= catalogue_aux.params._BTOAbins_min(iba) && catalogue_aux.b_to_a[i]< catalogue_aux.params._BTOAbins_max(iba))
                                                 if(catalogue_aux.c_to_a[i]>= catalogue_aux.params._CTOAbins_min(ica) && catalogue_aux.c_to_a[i]< catalogue_aux.params._CTOAbins_max(ica))
                                                   if(catalogue_aux.mach_number_at(i)>= catalogue_aux.params._MACHbins_min(ima) && catalogue_aux.mach_number_at(i)< catalogue_aux.params._MACHbins_max(ima))
                                                 if(catalogue_aux.bias_at(i)>= catalogue_aux.params._BIASbins_min(ibi) && catalogue_aux.bias_at(i)< catalogue_aux.params._BIASbins_max(ibi))
                                                   if(catalogue_aux.local_overdensity[i]>= catalogue_aux.params._LCbins_min(ilc) && catalogue_aux.local_overdensity[i] < catalogue_aux.params._LCbins_max(ilc))
                                                     if(catalogue_aux.tidal_anisotropy_at(i)>= catalogue_aux.params._TAbins_min(ita) && catalogue_aux.tidal_anisotropy_at(i)< catalogue_aux.params._TAbins_max(ita))
#ifndef _USE_PEAK_HEIGHT_AS_PRIMARY_PROPERTY_HBIAS_
                                                       if(catalogue_aux.peak_height[i]>= catalogue_aux.params._PHbins_min(ph_index) && catalogue_aux.peak_height[i]< catalogue_aux.params._PHbins_max(ph_index))
#endif
                                                     if(catalogue_aux.dach_number[i]>= catalogue_aux.params._DACHbins_min(ida) && catalogue_aux.dach_number[i]< catalogue_aux.params._DACHbins_max(ida))
                                                         if(catalogue_aux.local_dm_at(i)>= catalogue_aux.params._LOCALDMbins_min(ida) && catalogue_aux.local_dm_at(i)< catalogue_aux.params._LOCALDMbins_max(ida))
                                                       {
                                                          tracer_final.catalogue.coord1.push_back(catalogue_aux.coord1_at(i));
                                                          tracer_final.catalogue.coord2.push_back(catalogue_aux.coord2_at(i));
                                                          tracer_final.catalogue.coord3.push_back(catalogue_aux.coord3_at(i));
                                                          tracer_final.catalogue.mass.push_back(catalogue_aux.mass_at(i));
                                                          tracer_final.catalogue.vmax.push_back(catalogue_aux.vmax_at(i));
                                                          tracer_final.catalogue.rs.push_back(catalogue_aux.rs[i]);
                                                          tracer_final.catalogue.rvir.push_back(catalogue_aux.rvir[i]);
                                                          tracer_final.catalogue.concentration.push_back(catalogue_aux.concentration_at(i));
                                                          tracer_final.catalogue.spin.push_back(catalogue_aux.spin_at(i));
                                                          tracer_final.catalogue.spin_bullock.push_back(catalogue_aux.spin_bullock[i]);
                                                          tracer_final.catalogue.vrms.push_back(catalogue_aux.vrms(i));
                                                          tracer_final.catalogue.virial.push_back(catalogue_aux.virial[i]);
                                                          tracer_final.catalogue.b_to_a.push_back(catalogue_aux.b_to_a[i]);
                                                          tracer_final.catalogue.c_to_a.push_back(catalogue_aux.c_to_a[i]);
                                                          tracer_final.catalogue.mach_number.push_back(catalogue_aux.mach_number_at(i));
                                                          tracer_final.catalogue.bias.push_back(catalogue_aux.bias_at(i));
                                                          tracer_final.catalogue.local_overdensity.push_back(catalogue_aux.local_overdensity[i]);
                                                          tracer_final.catalogue.tidal_anisotropy.push_back(catalogue_aux.tidal_anisotropy_at(i));
                                                          tracer_final.catalogue.peak_height.push_back(catalogue_aux.peak_height[i]);
                                                          tracer_final.catalogue.dach_number.push_back(catalogue_aux.dach_number[i]);
                                                          tracer_final.catalogue.vel1.push_back(catalogue_aux.vel1_at(i));
                                                          tracer_final.catalogue.vel2.push_back(catalogue_aux.vel2_at(i));
                                                          tracer_final.catalogue.vel3.push_back(catalogue_aux.vel3_at(i));
                                                          tracer_final.catalogue.local_dm.push_back(catalogue_aux.local_dm_at(i));
                                                        }
                                             }
                                         cout<<endl;
                             ...
                             //            ntr.close();
                             //            So.DONE();
                                         tracer_final.set_NOBJS(in_new);// IMPORTANT. OTHERWISE METHODS OF catalogue_aux WILL SEE DEFAULT VALUE NOBS=0
                                         cout<<endl;
                                         if(in_new>0)
                                           {
                                         // ************************************************************************************************ //
                                          ...
                                          if(true==this->params._Get_spearman_coefficient())
                                           {
                                              ...
                                              for(int ka=0;ka<prop_name.size();++ka)
                                               ...
                                               spearman_b.close();
                                               b_p.clear();b_p.shrink_to_fit();
                                               tracer_final.catalogue_ranked.clear_mem();
                                          }
                                          // ************************************************************************************************ //
                                        if(this->params._Get_PCA())
                                             {
...
#endif
   for(ULONG i=0;i<this->params._NGRID();++i)
     fftw_functions.set_data_g(i, static_cast<real_prec>(data_in_MW[i])-static_cast<real_prec>(data_in[i])/static_cast<real_prec>(nmean));
...
#pragma omp parallel for
 ...
 
void PowerSpectrumF::get_window_matrix_multipole(Catalogue &random)
{
    this->So.enter(__PRETTY_FUNCTION__);
    if(this->params._FKP_weight() && this->params._nbar_tabulated())
#ifdef _USE_OMP_
#pragma omp parlalel for
#endif
    for (ULONG i=0; i< random._NOBJS();++i)
      random.set_weight1(1./(1+this->params._Pest()*random.mean_density_at(i)), i);
  else
#ifdef _USE_OMP_
#pragma omp parlalel for
#endif
    for (ULONG i=0; i< random._NOBJS();++i)
      random.set_weight1(1,i);
...
#endif
  vector<real_prec>distances_cat(random._NOBJS(),0);
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
  for (ULONG i=0; i< random._NOBJS();++i)
    distances_cat[i]=pow(random.coord1_at(i),2)+pow(random.coord2_at(i),2)+pow(random.coord3_at(i),2);
  So.DONE();
  this->kvector_data.clear();
  this->kvector_data.shrink_to_fit();
  for(int i=0;i<this->params._d_Nnp_data();i++)
  ...
  ULONG counter=0;
  for (ULONG i=0; i<  random._NOBJS();++i)
    for (ULONG j=i+1; j< random._NOBJS();++j)
      {
...
#pragma omp parallel for
  ...
  for (ULONG ip=0; ip<counter;++ip)
  {
      ...
      real_prec rp=sqrt(pow(random.coord1_at(i)+random.coord1_at(j),2)+pow(random.coord2_at(i)+random.coord2_at(j),2)+pow(random.coord3_at(i)+random.coord3_at(j),2));
   	  real_prec rm=sqrt(pow(random.coord1_at(i)-random.coord1_at(j),2)+pow(random.coord2_at(i)-random.coord2_at(j),2)+pow(random.coord3_at(i)-random.coord3_at(j),2));
      ...
      if(abs(deltaR)<=1)
        ...
        pairs[ip].DeltaR=0;
      pairs[ip].weight_pair=random.weight1_at(i)*random.weight1_at(j);
    }
  So.DONE();
#ifdef _FULL_VERBOSE_
...
#endif
        tracer_cat.set_bias(hb,itr);
#ifndef _TNG_GAL_
        tracer_cat.set_bias_rs(hbs,itr);
        tracer_cat.set_rs_factor(gama, itr);
#endif
        lss_bias_halo+=hb;
   }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 void PowerSpectrumF::object_by_object_bias_lm(Catalogue& tracer_cat, vector<real_prec>& dm_field, int lmax){
   this->So.enter(__PRETTY_FUNCTION__);
#ifdef _FULL_VERBOSE_
   ...
   for(ULONG itr=0;itr<tracer_cat._NOBJS();++itr)
     {
       ...
       for(int ell=0; ell<=lmax;++ell) // loop over l
       {
         ...
         for(int mm=-ell; mm<=ell; ++mm) // loop over m
        {
          ...
          tracer_cat.set_bias_multipole(hll,itr, ell);
        }   //closes loop over l
      } // loop over tracers
...
#endif
        for(ULONG itr=0;itr<tracer_cat._NOBJS();++itr)
          bll+=tracer_cat.bias_multipole_at(itr,ell);
        bmean[ell]=bll/static_cast<double>(tracer_cat._NOBJS());
...
#endif
        for(ULONG itr=0;itr<tracer_cat._NOBJS();++itr)
          bll+=pow(tracer_cat.bias_multipole_at(itr,ell)-bmean[ell],2)/(static_cast<double>(tracer_cat._NOBJS())-1);
        bvar[ell]=sqrt(bll)/static_cast<double>(tracer_cat._NOBJS()); // error of the mean
...
// this method is likely to be called from another method in this class, once the catalog has been alloacated in a member-type object Catalog
 void PowerSpectrumF::object_by_object_rbias(Catalogue &galaxy, HaloTools & htools_galaxy, HaloTools & htools_random){
   this->So.enter(__PRETTY_FUNCTION__);
#ifdef _FULL_VERBOSE_
...
#endif
     for(ULONG i=0;i<this->params._NGRID();++i)
        over_field[i]=htools_galaxy.field_external[i]-this->fftw_functions._alpha()*htools_random.field_external[i];
...
#endif
     for(ULONG i=0;i<this->params._NGRID();++i)
        over_field[i]=this->fftw_functions._alpha()*htools_random.field_external[i];
...
#endif
   for(ULONG itr=0;itr<galaxy._NOBJS();++itr)
    if(galaxy.observed_at(itr)==1)
    {
        real_prec we_fkp=1.0;
        if(true==this->params._FKP_weight())
          we_fkp=1.0/(1.0+this->params._Pest()*galaxy.mean_density_at(itr));
        w_g_reduced+=we_fkp;
        Ntr++;
    }
...
#pragma omp parallel for reduction(+:lss_rbias_halo)
   ...
   for(ULONG itr=0;itr<galaxy._NOBJS();++itr)
   {
     if(galaxy.observed_at(itr)==1)
       {
        real_prec xtracer=galaxy.coord1_at(itr);
        real_prec ytracer=galaxy.coord2_at(itr);
        real_prec ztracer=galaxy.coord3_at(itr);
        real_prec we_fkp=1.0;
        if(true==this->params._FKP_weight())
          we_fkp=1.0/(1.0+this->params._Pest()*galaxy.mean_density_at(itr));
...
//         real_prec hb=(vol)*(power_hm/p_dm)/(vol/static_cast<real_prec>(this->params._NGRID())); THIS WAS THE ORIGINAL SHAPE, below I just rearrange terms
//        real_prec hbtr=(power_hmt/p_t)*(this->params._NGRID())*(static_cast<real_prec>(this->tracer_cat.Halo.size())/static_cast<real_prec>(Ntr));
        ...
        galaxy.set_relative_bias(hbtr,itr);
        lss_rbias_halo+=hbtr;
       } // closes if
   ...
   HaloTools tools(params, galaxy);
   tools.write_catalog(this->params._Output_directory()+"reduced_gal_cat.txt");
   So.DONE();
 }

void PowerSpectrumF::object_by_object_bias(Catalogue& tracer_cat, vector<real_prec>& dm_field, vector<real_prec>&tracer_field){
   this->So.enter(__PRETTY_FUNCTION__);
#ifdef _FULL_VERBOSE_
   this->So.message_screen("Getting object-to-object bias");
#endif
...
#pragma omp parallel for reduction(+:lss_bias_halo)
   ...
   for(ULONG itr=0;itr<tracer_cat._NOBJS();++itr)
     {
       ...
       for(ULONG i=initial_mode; i< new_Nft/2;++i)
        ...
        gama= (vol)*(gama/p_dm)/(vol/static_cast<real_prec>(this->params._NGRID()));
#endif
        tracer_cat.set_bias(hb,itr);
        tracer_cat.set_relative_bias(hb,itr);
#ifndef _TNG_GAL_
        tracer_cat.set_bias_rs(hbs,itr);
        tracer_cat.set_rs_factor(gama,itr);
#endif
   ...
   lss_bias_halo/=static_cast<real_prec>(tracer_cat._NOBJS());
   lss_rbias_halo/=static_cast<real_prec>(tracer_cat._NOBJS());
   So.message_screen("\tMean large-scale bias from individual bias =", lss_bias_halo);
   So.message_screen("\tMean large-scale relative bias from individual bias =", lss_rbias_halo);
#ifdef _TNG_GAL_
   vector<real_prec>baux(tracer_cat._NOBJS(),0);
   ...
   vector<real_prec>zaux(tracer_cat._NOBJS(),0);
    for(ULONG i=0;i<xaux.size(); ++i)
    {
        baux[i]=tracer_cat.bias_at(i);
        xaux[i]=tracer_cat.coord1_at(i);
        yaux[i]=tracer_cat.coord2_at(i);
        zaux[i]=tracer_cat.coord3_at(i);
    }
    string file_bias=this->params._Output_directory()+"Bias_gal1";
    this->File.write_array(file_bias, baux);
    vector<real_prec>bfaux(this->params._NGRID(),0);
...

void PowerSpectrumF::object_by_object_bias_squared(Catalogue& tracer_cat, vector<real_prec>& dm_field, vector<real_prec>&tracer_field){
   this->So.enter(__PRETTY_FUNCTION__);
#ifdef _FULL_VERBOSE_
...
//                     power_cross[kbin]+=(cos(k_dot_r)*Delta_tr[lp][REAL]-sin(k_dot_r)*Delta_tr[lp][IMAG]+cos(k_dot_r)*Delta_tr[lp][IMAG]-sin(k_dot_r)*Delta_tr[lp][REAL]);
        ...
        real_prec hb=(power_hm/p_dm)*(this->params._NGRID());
        tracer_cat.set_bias_squared(hb,itr);
        lss_bias_halo+=hb;
   }
   lss_bias_halo/=static_cast<real_prec>(tracer_cat._NOBJS());
...
void PowerSpectrumF::object_by_object_qbias(Catalogue & tracer_cat, vector<real_prec>& dm_field){
   this->So.enter(__PRETTY_FUNCTION__);
#ifdef _FULL_VERBOSE_
   this->So.message_screen("Getting object-to-object quadratic bias");
#endif
   ...
   for(ULONG itr=0;itr<tracer_cat._NOBJS();++itr)
     {
       ...
       for(ULONG i=Kmin_bin; i< new_Nft/2;++i)
        ...
        for(ULONG i=Kmin_bin; i< power_cross.size();++i)
          {
              power_hm+=(power_cross[i]*this->params._NGRID() - tracer_cat.bias_at(itr)*power_dmat[i]);
              p_dm+=Apower[i];
            }
        real_prec hb=(power_hm/p_dm)/static_cast<real_prec>(this->params._NGRID());
        tracer_cat.set_qbias(hb,itr);
        lss_bias_halo+=hb;
   }
   lss_bias_halo/=static_cast<real_prec>(tracer_cat._NOBJS());
   So.DONE();
...
#pragma omp atomic
    ...
    if(true==this->params._Generate_FA())
     ...
     if(ir==0)
        ...
        std::ofstream btea; btea.open(this->params._Output_directory()+"power_grf_real"+to_string(ir)+".txt");
        btea.precision(6);
...
void PowerSpectrumF::GetSuperClusters(string realm, string tbias){
   ...
   Catalogue cat(params);
   cat.read_catalog_new(this->params._Input_dir_cat()+this->params._file_catalogue());
   
   HaloTools htools(this->params, cat);
   ...
   for(int i=0;i<this->params._d_Nnp_data();i++)
     ...
     this->object_by_object_bias(cat.catalogue,DM_DEN_FIELD,TR_DEN_FIELD);
   ...
   htools.get_superclusters(realm, tbias); // Here we selec tracers with bias above the fourth quartile. This gives catalogue_aux
   
   ULONG size_cat=catalogue_aux.coord1.size();
   ...
   std::vector<str::string>names_p={"_MASS_", tbias, "_SPIN_","_CONCENTRATION_","_MACH_", "_BTOA_","_CTOA_" ,"_LOCALDM_", "_TIDAL_ANISOTROPY_"};

  for (const auto& name : names_p) {
     htools.Get_Ranked_Props(name);      
    }

    So.DONE();
   //******************************************************
   //   catalogue_ranked and catalogue_aux have the same dimension, 
  ...
  Catalogue catalogue_aux = std::move(cat);
  catalogue_aux.set_params(params);
  HaloTools htools_aux(params, catalogue_aux);
   ...
   for(ULONG rr=0;rr<=this->params.d_NGRID_JK();++rr) // Loop over each of the sub-samples JK. The 0th item is the full sample
    ...
    if(rr==0)
    {
       ...
       {
         xc.push_back(catalogue_aux.coord1_at(i));
         yc.push_back(catalogue_aux.coord2_at(i));
         zc.push_back(catalogue_aux.coord3_at(i));
         mc.push_back(htools.catalogue_ranked.mass_at(i));
         if(tbias=="_BIAS_")
             bc.push_back(htools.catalogue_ranked.bias_at(i));
         if(tbias=="_RELATIVE_BIAS_")
             bc.push_back(htools.catalogue_ranked.relative_bias_at(i));
         sc.push_back(htools.catalogue_ranked.spin_at(i));
         mach.push_back(htools.catalogue_ranked.mach_number_at(i));
         cc.push_back(htools.catalogue_ranked.concentration_at(i));
         tidalc.push_back(htools.catalogue_ranked.tidal_anisotropy_at(i));
         btoac.push_back(htools.catalogue_ranked.b_to_a_at(i));
         ctoac.push_back(htools.catalogue_ranked.c_to_a_at(i));
         localdmc.push_back(htools.catalogue_ranked.local_dm_at(i));
         mean_mark_m+=catalogue_ranked.mass_at(i);
         if(tbias=="_BIAS_")
            mean_mark_b+=catalogue_ranked.bias_at(i);
         if(tbias=="_RELATIVE_BIAS_")
            mean_mark_b+=catalogue_ranked.relative_bias_at(i);
         mean_mark_s+=catalogue_ranked.spin_at(i);
         mean_mark_mach+=catalogue_ranked.mach_number_at(i);
         mean_mark_concentration+=catalogue_ranked.concentration_at(i);
         mean_mark_btoa+=catalogue_ranked.b_to_a_at(i);
         mean_mark_ctoa+=catalogue_ranked.c_to_a_at(i);
         mean_mark_lcdm+=catalogue_ranked.local_dm_at(i);
         mean_mark_ta+=catalogue_ranked.tidal_anisotropy_at(i);
       }
    }
      else{
       for(ULONG i=0;i<size_cat;++i)// Loop over the catalog
    	 if(catalogue_aux.GridID_n[i] != rr)
    	   {
    	     xc.push_back(catalogue_aux.coord1_at(i));
    	     yc.push_back(catalogue_aux.coord2_at(i));
    	     zc.push_back(catalogue_aux.coord3_at(i));
...
#pragma omp critical
  ...
  for(int rr=0;rr<=Nruns;++rr)
   ...
   So.message_screen("  Shuffling properties for run ", rr);
      ...
      for(ULONG i=0;i<size_cat;++i)
       {
        catalogue_ranked.set_mass(catalogue_ranked.mass_at(aux_p[i],i));
        if (tbias == "_BIAS_")
          catalogue_ranked.set_bias(
              catalogue_ranked.bias_at(aux_p[i]), i);

        if (tbias == "_RELATIVE_BIAS_")
            catalogue_ranked.set_relative_bias(
                catalogue_ranked.relative_bias_at(aux_p[i]), i);

        catalogue_ranked.set_spin(
            catalogue_ranked.spin_at(aux_p[i]), i);

        catalogue_ranked.set_mach_number(
            catalogue_ranked.mach_number_at(aux_p[i]), i);

        catalogue_ranked.set_concentration(
            catalogue_ranked.concentration_at(aux_p[i]), i);

        catalogue_ranked.set_c_to_a(
            catalogue_ranked.c_to_a_at(aux_p[i]), i);

        catalogue_ranked.set_b_to_a(
            catalogue_ranked.b_to_a_at(aux_p[i]), i);

        catalogue_ranked.set_local_dm(
            catalogue_ranked.local_dm_at(aux_p[i]), i);

        catalogue_ranked.set_tidal_anisotropy(
            catalogue_ranked.tidal_anisotropy_at(aux_p[i]), i);
       }
       So.DONE();
    }
   real_prec mean_mark_m=0;
...
#pragma omp for 
     for(ULONG i=0;i<size_cat;++i)
       for(ULONG j=i+1;j<size_cat;++j)
    	 {
    	   real_prec dx=catalogue_aux.coord1_at(i)-catalogue_aux.coord1[j];
    	   real_prec dy=catalogue_aux.coord2_at(i)-catalogue_aux.coord2[j];
    	   real_prec dz=catalogue_aux.coord3_at(i)-catalogue_aux.coord3[j];
    	   real_prec dist_squared=dx*dx+dy*dy+dz*dz;
           if (dist_squared>=rmin_squared && dist_squared<rmax_squared)
           {
        	   real_prec ldist=0.5*log10(dist_squared); // this is log (sqrt(dist_squared))
    	       ...
    	       {
        	       gcount_priv[ibin]++;  // marking with property=mass
                   mcount_priv[ibin]+=catalogue_ranked.mass_at(i)*catalogue_ranked.mass_at(j);
                   if(tbias=="_BIAS_")
            	       bcount_priv[ibin]+=catalogue_ranked.bias_at(i)*catalogue_ranked.bias_at(j); // marking with bias
                   if(tbias=="_RELATIVE_BIAS_")
                       bcount_priv[ibin]+=catalogue_ranked.relative_bias_at(i)*catalogue_ranked.relative_bias_at(j); // marking with bias
        	          scount_priv[ibin]+=catalogue_ranked.spin_at(i)*catalogue_ranked.spin_at(j);
                   machcount_priv[ibin]+=catalogue_ranked.mach_number_at(i)*catalogue_ranked.mach_number_at(j); // marking with bias       
                   ccount_priv[ibin]+=catalogue_ranked.concentration_at(i)*catalogue_ranked.concentration_at(j);
                   btoacount_priv[ibin]+=catalogue_ranked.b_to_a_at(i)*catalogue_ranked.b_to_a_at(j);
                   ctoacount_priv[ibin]+=catalogue_ranked.c_to_a_at(i)*catalogue_ranked.c_to_a_at(j);
                   lcdmcount_priv[ibin]+=catalogue_ranked.local_dm_at(i)*catalogue_ranked.local_dm_at(j);
                   ctoacount_priv[ibin]+=catalogue_ranked.tidal_anisotropy_at(i)*catalogue_ranked.tidal_anisotropy_at(j);
    	       }
    	    }
         }
#pragma omp critical
 ...
 void PowerSpectrumF::GetSuperClusters(string prop){
   ...
   cat.get_superclusters("over", prop); // Here we selec tracers with bias above the fourth quartile. This gives catalogue_aux
   ULONG size_cat=catalogue_aux.coord1.size();
   int Nrbins=this->params._Nbins_cf();
   real_prec rmax=this->params._rmax_cf();
   real_prec rmin=this->params._rmin_cf();
   ...
   // here we shuffle properties
   So.message_screen("  Ranking properties");
   catalogue_ranked .Get_Ranked_Props(prop);
   So.DONE();
   //******************************************************
   //   catalogue_ranked and catalogue_aux have the same dimension, 
   // same coordinates in the i-label, only properties have been ranked
  ...
  vector<real_prec>rbias_tr(size_cat,0);
   ...
   Catalogue catalogue_ranked(params); 
   HaloTools htools_ranked(this->params, catalogue_ranked);
...
#pragma omp parallel for 
#endif
  for(ULONG i=0;i<size_cat;++i)
    rbias_tr[i]=catalogue_ranked.relative_bias_at(i); // keep this for JK
  ...
  for(ULONG i=0;i<size_cat;++i)
    ...
    //            catalogue_ranked.relative_bias_at(i)=cat.catalogue_ranked[aux_p[i]].relative_bias;
                catalogue_ranked.relative_bias_at(i)=rbias_tr[aux_p[i]];
            So.DONE();
...
#pragma omp parallel for reduction(+:mean_mark_m)
   ...
   for(ULONG i=0;i<size_cat;++i)
     {
      if(prop=="_RELATIVE_BIAS_")
           mean_mark_m+=catalogue_ranked.relative_bias_at(i);
     }
...
#pragma omp for
     for(ULONG i=0;i<size_cat;++i)
       for(ULONG j=i+1;j<size_cat;++j)
         {
           real_prec dx=cat.catalogue.coord1_at(i)-catalogue_aux.coord1_at(i);
           real_prec dy=catalogue_aux.coord2_at(i)-catalogue_aux.coord2_at(i);
           real_prec dz=catalogue_aux.coord3_at(i)-catalogue_aux.coord3_at(i);
           real_prec dist_squared=dx*dx+dy*dy+dz*dz;
           if (dist_squared>=rmin_squared && dist_squared<rmax_squared)
           {
               real_prec ldist=0.5*log10(dist_squared); // this is log (sqrt(dist_squared))
               ...
               if(ibin<Nrbins)
               {
                   gcount_priv[ibin]++;  // marking with property=mass
                   if(prop=="_RELATIVE_BIAS_")
                     mcount_priv[ibin]+=catalogue_ranked.relative_bias_at(i)*catalogue_ranked.relative_bias_at(j);
               }
            }
         }
#pragma omp critical
...
#pragma omp parallel for
   for(ULONG i=0;i<size_cat;++i) 
        catalogue_aux.GridID_n[i] = grid_ID(&cat.box_JK, catalogue_aux.coord1_at(i),catalogue_aux.coord2_at(i),catalogue_aux.coord3_at(i));
   ...
   for(ULONG rr=0;rr<=this->params.d_NGRID_JK();++rr) // Loop over each of the sub-samples JK. The 0th item is the full sample
    ...
    if(rr==0)
    {
       for(ULONG i=0;i<size_cat;++i)// Loop over the catalog
       {
         ...
         if(prop=="_RELATIVE_BIAS_")
         {
//             mc.push_back(htools.catalogue_ranked.relative_bias_at(i));
 //           mean_mark_m+=catalogue_ranked.relative_bias_at(i);
