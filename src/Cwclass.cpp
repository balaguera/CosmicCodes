...
#pragma omp parallel for
  ...
  this->CWClass.resize(this->params._NGRID(),WebType::Full);
#endif
...
#pragma omp parallel for reduction(+:nknots,nfilaments,nsheets,nvoids,nrest)
  ...
  for (ULONG index=0;index<this->params._NGRID();++index)
	...
	{
	  this->CWClass[index]=WebType::Sheet;
	  nsheets++;
	}
      
      else if ((lambda1[index]<this->params._lambdath() && lambda2[index]>=this->params._lambdath() && lambda3[index]>=this->params._lambdath()) || (lambda1[index]>=this->params._lambdath() && lambda2[index]>this->params._lambdath() && lambda3[index]<this->params._lambdath()) || (lambda1[index]>this->params._lambdath() && lambda2[index]<this->params._lambdath() && lambda3[index]>=this->params._lambdath()))
	{
	  this->CWClass[index]=WebType::Filament;
	  nfilaments++;
	}
      else
	nrest ++;
...
/*
  ...
  for (ULONG index=0;index<this->params._NGRID();++index)
   {
     if(WebType::Void==this->CWClass[index])
      {
  ...
  for (ULONG index=0;index<this->params._NGRID();++index)
   {
     if(WebType::Sheet==this->CWClass[index])
      {
         l1.push_back(this->Invariant_TF_I[index]);
         l2.push_back(this->Invariant_TF_II[index]);
         l3.push_back(this->Invariant_TF_III[index]);
  ...
  for (ULONG index=0;index<this->params._NGRID();++index)
   {
     if(WebType::Filament==this->CWClass[index])
      {
         l1.push_back(this->Invariant_TF_I[index]);
         l2.push_back(this->Invariant_TF_II[index]);
         l3.push_back(this->Invariant_TF_III[index]);
  ...
  for (ULONG index=0;index<this->params._NGRID();++index)
   {
     if(WebType::Knot==this->CWClass[index])
      {
         l1.push_back(this->Invariant_TF_I[index]);
         l2.push_back(this->Invariant_TF_II[index]);
         l3.push_back(this->Invariant_TF_III[index]);
  ...
  for (ULONG index=0;index<this->params._NGRID();++index)
    {
      if (this->lambda1_vs[index]>this->params._lambdath()_v && this->lambda2_vs[index]>this->params._lambdath()_v && this->lambda3_vs[index]>this->params._lambdath()_v)
        {
          this->CWClass_V[index]=WebType::Knot;
          nknots++;
        }
        ...
        {
          this->CWClass_V[index]=WebType::Void;
          nvoids++;
        }
        ...
        {
          this->CWClass_V[index]=WebType::Sheet;
          nsheets++;
        }

      else if ((lambda1_vs[index]<this->params._lambdath()_v && lambda2_vs[index]>this->params._lambdath()_v && lambda3_vs[index]>this->params._lambdath()_v) || (lambda1_vs[index]>this->params._lambdath()_v && lambda2_vs[index]>this->params._lambdath()_v && lambda3_vs[index]<this->params._lambdath()_v) || (lambda1_vs[index]>this->params._lambdath()_v && lambda2_vs[index]<this->params._lambdath()_v && lambda3_vs[index]>this->params._lambdath()_v))
        {
          this->CWClass_V[index]=WebType::Filament;
          nfilaments++;
        }
      else
        nrest ++;
...
#pragma omp parallel for reduction(+:aux)
#endif
    for(ULONG i=0;i<this->params._NGRID();++i)
      if(this->CWClass[i]==WebType::Knot)
	aux++;
  ...
  for(ULONG i=0;i < this->params._NGRID(); ++i)
    {
      if(this->CWClass[i]==WebType::Knot && rho[i]>0)  // If cell is classified as knot, proceed. This must be the classification of the REFERENCE density  field. The >0 indicates that there might be some cells classified as knots but empty
	{
	  if(this->SKNOT_M_info[i] == -1) // if this cell has not been already used in the fof, proceed
              ...
              for(int j=0;j<n;++j)  // j runs over the neighbour cells of the current one, up to n, when n is increased inside the "if" when a neighbour knot is found
		{
		  if( (k= knot[j]-1)>=0 && k < this->params._NGRID())
                    if( (this->CWClass[k]==WebType::Knot && rho[k]>0) && this->SKNOT_M_info[k]== -1)   // If density is greater than zero and cell not yet used
		      {
			knot[n] = k;   
		  ...
		  if( (k = knot[j] + 1) >= 0 && k < this->params._NGRID())
                    if( (this->CWClass[k]==WebType::Knot && rho[k]>0) && this->SKNOT_M_info[k] == -1)
		      {
			knot[n] = k;
...
void Cwclass::get_collapsing_regions(ULONG &Ntsk,vector<real_prec>&sk_index)
{
  ...
  for(ULONG i=0;i < this->params._NGRID(); ++i)
    {
      if(this->CWClass[i]>WebType::Full)  // If cell is classified as knot, proceed. This must be the classification of the REFERENCE density  field. The >0 indicates that there might be some cells classified as knots but empty
	{
	  if(this->SKNOT_M_info[i] == -1) // if has not been already used in the fof, proceed
	    {
		  ...
		  if( (k= knot[j]-1)>=0 && k < this->params._NGRID()) 		  // I(i,j,k-1) = I-1
		    {
		      if( this->CWClass[k]>WebType::Full && this->SKNOT_M_info[k]== -1)   // If density is greater than zero and cell not yet used
			{
			  knot[n] = k;   
			  this->SKNOT_M_info[k] = 1; //mark this cell as already visited
			  n++;
		  ...
		  if( (k = knot[j] + 1) >= 0 && k < this->params._NGRID())  // I(i,j,k+1) = I+1
		    {
		      if( this->CWClass[k]>WebType::Full && this->SKNOT_M_info[k] == -1)
			{
			  knot[n] = k;
			  this->SKNOT_M_info[k] = 1;
			  n++;
...
#pragma omp parallel for reduction(+:aux)
    for(ULONG i=0;i<this->params._NGRID();++i)
      if(this->CWClass_V[i]==WebType::Knot)
        aux++;
  ...
  for(ULONG i=0;i < this->params._NGRID(); ++i)
    {
      if(this->CWClass_V[i]==WebType::Knot && rho[i]>0)  // If cell is classified as knot, proceed. This must be the classification of the REFERENCE density  field. The >0 indicates that there might be some cells classified as knots but empty
        {
          if(this->VDISP_KNOT_info[i] == -1) // if has not been already used in the fof, proceed
            {
              ...
              for(int j=0;j<n;++j)  // j runs over the neighbour cells of the current one, up to n, when n is increased inside if a neirbour knot is found
                {
                  if( (k= knot[j]-1)>=0 && k < this->params._NGRID())
                    if( (this->CWClass_V[k]==WebType::Knot && rho[k]>0) && this->VDISP_KNOT_info[k]== -1)   // If density is greater than zero and cell not yet used
                      {
                        knot[n] = k;
                  ...
                  if( (k = knot[j] + 1) >= 0 && k < this->params._NGRID())
                    if( (this->CWClass_V[k]==WebType::Knot && rho[k]>0) && this->VDISP_KNOT_info[k] == -1)
                      {
                        knot[n] = k;
 ...
 So.message_screen("Min and Max of Kinetic from FoF regions: ");
  ...
  for(ULONG i = 0; i < this->params._NGRID(); i++)
    {
      if(this->CWClass_V[i]==WebType::Knot && rho[i]>0)
        {
...
bool Cwclass::get_cell_classified(int sua, ULONG ig)
{
  ...
  if(this->cwt_used.size()==1 && this->cwt_used[0]==0)
	...
	{
	  if(static_cast<int>(this->CWClass[ig])==classi)
	    res=true;
	}
      else         // This referes to combinations
	if(classi==234)
	  {
            if(this->CWClass[ig]==WebType::Filament || this->CWClass[ig]==WebType::Sheet ||this->CWClass[ig]==WebType::Void)
	      res=true;
	  }
	else if(classi==12)
	  {
            if(this->CWClass[ig]==WebType::Knot || this->CWClass[ig]==WebType::Filament)
	      res=true;
	  }
	else if(classi==23)
	  {
            if(this->CWClass[ig]==WebType::Filament || this->CWClass[ig]==WebType::Sheet)
	      res=true;
	  }
	else if(classi==34)
	  {
            if(this->CWClass[ig]==WebType::Sheet || this->CWClass[ig]==WebType::Void)
	      res= true;
	  }
	else if(classi==123)
	  {
            if(this->CWClass[ig]==WebType::Knot || this->CWClass[ig]==WebType::Filament || this->CWClass[ig]==WebType::Sheet)
	      res= true;
	  }
    }
  return res;
...
WebType Cwclass::get_Tclassification(ULONG ig)
{

  int ians;
  if(this->cwt_used.size()==1)
    ians=this->cwt_used[0];
  else
    {
      for(int ic=0;ic<this->cwt_used.size();++ic)// loop over the number of cwt required from the par file
	{
	  ...
	  if(cwc<5)
	    {
	      if(static_cast<int>(this->CWClass[ig])==cwc)
		ians=ic;
	    }
	  else{
	    if(cwc==234)
  ...
  return static_cast<WebType>(ians);
}
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
...
WebType Cwclass::get_Vclassification(int ig)
{
      ...
      for(int ic=0;ic<this->cwv_used.size();++ic)// loop over the number of cwt required from the par file
        {
          ...
          if(cwc<5)
            {
              if(static_cast<int>(this->CWClass_V[ig])==cwc)
                ans=ic;
            }
          else{
            if(cwc==234)
              {
                if(this->CWClass_V[ig]==WebType::Filament || this->CWClass_V[ig]==WebType::Sheet || this->CWClass_V[ig]==WebType::Void)
                  ans=ic;
              }
            else if(cwc==12)
              {
                if(this->CWClass_V[ig]==WebType::Knot || this->CWClass_V[ig]==WebType::Filament)
                  ans=ic;
              }
            else if(cwc==23)
              {
                if(this->CWClass_V[ig]==WebType::Filament || this->CWClass_V[ig]==WebType::Sheet)
                  ans=ic;
              }
            else if(cwc==34)
              {
                if(this->CWClass_V[ig]==WebType::Sheet || this->CWClass_V[ig]==WebType::Void)
                  ans=ic;
              }

            else if(cwc==123)
              {
                if(this->CWClass_V[ig]==WebType::Knot ||  this->CWClass_V[ig]==WebType::Filament || this->CWClass_V[ig]==WebType::Sheet)
                  ans=ic;
              }
 ...
 return static_cast<WebType>(ans);
}
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
