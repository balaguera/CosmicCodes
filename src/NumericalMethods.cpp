...
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
# include "../include/NumericalMethods.hpp"
using namespace std;
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
long double factorial(int n){
    if (n < 0) {
        std::cerr << "Factorial of negative number!\n";
        std::abort();
    }
    return tgammal(n + 1.0L);
}
...
void Cholesky_decomposition(vector< vector<gsl_real> > &G, vector< vector<gsl_real> > &y){
  int n=G[0].size();
  gsl_matrix *A  = gsl_matrix_alloc(n,n);
  for (int i=0;i<n;++i)
    for(int j=0;j<n;++j)
...
void gradfindif(ULONG N1,ULONG N2,ULONG N3,real_prec L1,real_prec L2,real_prec L3,vector<real_prec>in,vector<real_prec>&out,int dim)
{
    ...
    //    real_prec factor=static_cast<real_prec>(N1)/static_cast<real_prec>(2.0*L1);
  switch(dim){
  case(1):
#ifdef _USE_OMP_
...
void low_pass_filter(ULONG Nft_HR, ULONG Nft_LR, int imas, bool correct, vector<real_prec>&HR_field, vector<real_prec>&LR_field, real_prec Lbox)
{
  ...
  if(NTT_HR<NTT_LR)
    {
      throw std::runtime_error("High resolution mesh must have larger number of Nft cells than low resolution.");
      exit(0);
    }
...
size_t index_nd(const std::vector<size_t>& idx,
...
{
  if (params->Nft == 0) {
    std::cerr << "ERROR: Nft == 0\n";
    std::exit(1);
  }
  long i = static_cast<long>(floor((x-params->min1)/params->d1)+ BIN_SHIFT); // indices of the cell of the particle
  long j = static_cast<long>(floor((y-params->min2)/params->d2)+ BIN_SHIFT);
  long k = static_cast<long>(floor((z-params->min3)/params->d3)+ BIN_SHIFT);
  ...
  i = i % params->Nft;
//  i = static_cast<ULONG>(fmod(static_cast<real_prec>(i),static_cast<real_prec>(params->Nft)));
  j = j % params->Nft;
...
#pragma omp parallel for
  for(ULONG i=0;i<N;i++)    
    dummy2[i]*=2.0*out[i];//this term appears twice in the determinant
  fname=string("aux2");
  get_scalar(fname,out,N1,N2,N3);
...
////////////////////////////////////////////////////////////////////////////
int npix_ring(int nside_h, int ir){
  // Number of pixels contained in a ring labled with ir. with 0<ir< 4nside-1 rings
  ULONG Npixels_ring=0;
  if(ir<nside_h-1)
    Npixels_ring=4*(ir+1);
  else if(ir>=nside_h-1 && ir<3*nside_h)
    Npixels_ring=4*nside_h;
  else if(ir>=3*nside_h)
    Npixels_ring=4*(4*nside_h-ir-1);
  return Npixels_ring;
}
