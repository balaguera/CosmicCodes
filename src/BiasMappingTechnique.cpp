...
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
#include "../include/BiasMappingTechnique.hpp"
...
#endif
  size_t NTT=(this->params._Nft())*(this->params._Nft())*(this->params._Nft()/2+1);
...
#endif
      for(size_t i=0;i<this->Power_DM_REF.size(); ++i)
...
#pragma omp parallel for
#endif
          for(size_t i=0;i<this->Power_DM_KONV.size(); ++i)
...
#pragma omp parallel for
#endif
      for(size_t i=0;i<this->Power_NEW.size(); ++i)
...
#pragma omp parallel for
#endif
      for(size_t i=0;i<this->params._Nft()/2/this->params._ndel_data();++i)
...
void BiasMT::GetKernel(bool rejection, 
                      real_prec exponent)
{
  this->So.enter(__PRETTY_FUNCTION__);
...
#endif
  if(!this->use_iteration_ini) // initialized as false in bamrunner if iteration_ini >0
    {
      size_t nmodes_f=this->kvec.size();
...
#pragma omp parallel for collapse(3)
      for(size_t i=0;i< this->params._Nft(); ++i)
        for(size_t j=0;j< this->params._Nft(); ++j)
          for(size_t k=0;k< this->params._Nft()/2+1; ++k)
            {
              size_t ind=index_3d(i,j,k, this->params._Nft(), this->params._Nft()/2+1);
...
// Get the shell-averaged version of the Kernel in FOurier space:
  for(size_t i=0;i< kernel_updated.size(); ++i)
...
// ****************************************************************************************************************************
#pragma omp parallel for
    for(size_t ind=0;ind< this->NTT ;++ind)
      {
...
void  BiasMT::get_min_max_X_Y()
{
  ...
  // The filds delta X and delta Y ahave been already, if requiested, converted to overdensities
   this->So.enter(__PRETTY_FUNCTION__);
   if(this->params._Redefine_limits())
    {
      real_prec num_in_log_x = this->params._Convert_Density_to_Delta_X() ? NUM_IN_LOG: 0.;
      real_prec num_in_log_y = this->params._Convert_Density_to_Delta_Y() ? NUM_IN_LOG: 0.;
...
#pragma omp parallel for
#endif
          for(size_t i=0;i<this->delta_X.size();++i)AUX[i]=log10(num_in_log_x+this->delta_X[i]);
...
#pragma omp parallel for
#endif
          for(size_t i=0;i<this->delta_Y.size();++i)
...
#endif
{
  ...
  size_t NGRID_NEW_HR;
#ifdef _EXTRAPOLATE_VOLUME_
  NGRID_NEW=static_cast<size_t>(this->params._Nft_low()*this->params._Nft_low()*this->params._Nft_low());
#else
#ifdef _USE_TRACER_HR_
  NGRID_NEW= this->params._NGRID();
  NGRID_NEW_HR=static_cast<size_t>(this->params._Nft_HR()*this->params._Nft_HR()*this->params._Nft_HR());
#else
  NGRID_NEW= this->params._NGRID();
  ...
  */
  ...
  for(size_t i=0;i <  this->params._NGRID(); ++i)
    {
      if(this->delta_Y_mass_at(i)<1.0)
        empty_cells++;
      this->delta_Y_mass_at(i)=log10(NUM_IN_LOG+ this->delta_Y_mass_at(i)/MASS_SCALE);
    }
...
#pragma omp parallel for
#endif
      for(size_t i=0;i< this->delta_X.size();++i)
...
#pragma omp atomic update
      ...
      if(this->params._Write_PDF_number_counts())
        {
...
// Analyze input file for Y variables
  ...
  NNGRID= static_cast<size_t>(this->params._Nft_low()*this->params._Nft_low()*this->params._Nft_low());
#else
...
#pragma omp parallel for reduction(+:lss_bias, ncount)
  ...
  if(this->params._Convert_Density_to_Delta_X())
    {
...
// Convert the TRACER to delta
    if(this->params._Convert_Density_to_Delta_Y())
      {
#ifdef _FULL_VERBOSE_
...
void BiasMT::get_BiasMT_DM()
{
  ...
  for(size_t i=0;i<this->params._NX(); ++i)
...
void BiasMT::get_scaling_relations_primary_property_two(int ifile)
{
 ...
 if(ifile==0){
  ...
  size_t N_c = N_C_BIN3;
  // ************************RESIZE VECTORS FOR HISTOGRAMS*************************
  size_t N_v= N_CV_BIN1;
#ifdef _USE_TRACERS_IN_CELLS_
  N_v = N_BINS_TRACERS_IN_CELLS;
#endif
 // ************************RESIZE VECTORS FOR HISTOGRAMS*************************
  size_t LENGHT_AB=N_v*N_a* N_b* N_c* N_CV_BIN2*N_CV_BIN3;
  ...
  size_t N_x = this->params._NPROPbins_bam();
  size_t LENGHT_AB_ONE= LENGHT_AB*N_x;
#endif
...
// *********************************************************************************************
  ...
  this->catalogue_ref.set_type_of_object("TRACER_REF");
  ...
  // *********************************************************************************************
  this->catalogue_ref.read_catalog_new(newcat);
  HaloTools htools_ref(params, catalogue_ref);

if(ifile==this->params._Number_of_references()-1)// Measure the abundance of the last refernce,
      this->htools_ref->get_property_function(this->params._Output_directory()+"tracer_ref_abundance.txt");
...
// This aims at assigning the coordinates of ref to mocks, in case we want to reassign prop to the refernece halos
  ...
  this->catalogue_aux.set_params(this->params);
  this->catalogue_aux.set_type_of_object("TRACER_REF");
  string newcat_aux=this->params._Input_dir_cat_new_ref()+this->params._file_catalogue_new_ref();
  this->catalogue_aux.read_catalog_new(newcat_aux);
  Htools htools_aux(params, catalogue_aux);

//     this->catalogue.Halo.resize(this->catalogue_aux._NOBJS()); Hay que resize los vectores
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
     for(size_t i=0; i<this->catalogue._NOBJS(); ++i)
       {
         this->catalogue.set_coord1(this->catalogue_aux.coord1_at(i),i);
         this->catalogue.set_coord2(this->catalogue_aux.coord2_at(i),i);
         this->catalogue.set_coord3(this->catalogue_aux.coord3_at(i),i);
         this->catalogue.set_vel1(this->catalogue_aux.vel1_at(i),i);
         this->catalogue.set_vel2(this->catalogue_aux.vel2_at(i),i);
         this->catalogue.set_vel3(this->catalogue_aux.vel3_at(i),i);
         this->catalogue.set_GridID(this->catalogue_aux.GridID_at(i),i);
       }
     this->catalogue.set_NOBJS(this->catalogue_aux._NOBJS());
#else  // else for  _ASSIGN_PROPERTIES_TO_NEW_REFERENCE_
      // Else assign to the *last reference read*.
     this->catalogue.Halo.resize(this->catalogue_ref._NOBJS());
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
     for(size_t i=0; i<this->catalogue_ref._NOBJS(); ++i)
       {
         this->catalogue.set_coord1(this->catalogue_aux.coord1_at(i),i);
         this->catalogue.set_coord2(this->catalogue_aux.coord2_at(i),i);
         this->catalogue.set_coord3(this->catalogue_aux.coord3_at(i),i);
         this->catalogue.set_GridID(this->catalogue_aux.GridID_at(i),i);
//         this->catalogue.vel1_at(i)=this->catalogue_ref.vel1_at(i);
//         this->catalogue.vel2_at(i)=this->catalogue_ref.vel2_at(i);
//         this->catalogue.vel3_at(i)=this->catalogue_ref.vel3_at(i);
       }
     this->catalogue.set_NOBJS(this->catalogue_ref._NOBJS());
#endif
 }
#endif
  // *********************************************************************************************
  for(size_t i=0;i<this->params._Number_of_MultiLevels();++i)
    this->tracers_multi[i][ifile]=this->catalogue_ref.params.get_Ntracers_MultiLevels(i);
 // *********************************************************************************************
// We need to load all the info of the tracers together in a single class member when using multilevel. WHen we do not se multilevel, all the info of all refs go into the dm_prop container
 ...
 REF_DM=delta_dm_aux_mem;
  ...
  size_t NXn=600;// This is as NX but higher to make pdf
...
#pragma omp parallel for
#endif
  for(size_t i = 0;i <  this->params._NGRID() ;++i)  //TRANSFORM DELTA TO LOG10(NUM_IN_LOG + DELTA)
  ...
  this->catalogue_ref.get_density_field_grid(_MASS_, REF_MASS_FIELD);
  ...
  // ********************************************************************************
#ifdef _USE_NUMBER_OF_NEIGHBOURS_
  this->catalogue_ref.get_neighbour_tracers(this->ncells_info);
#endif
  // ********************************************************************************
#ifdef _GET_DIST_MIN_SEP_REF_
    this->catalogue_ref.get_distribution_min_separations(this->ncells_info);
#endif
// ********************************************************************************
#ifdef _GET_DIST_REDUCED_MASS_
  this->catalogue_ref.get_distribution_reduced_mass_in_cell();
#endif
  // ********************************************************************************
#if defined _USE_MIN_SEPARATIONS_IN_CELLS_
  this->catalogue_ref.get_stats_separation_in_cell();
#if defined _ASSIGN_PROPERTIES_TO_REFERENCE_ || defined (_ASSIGN_PROPERTIES_TO_NEW_REFERENCE_)
  this->min_halo_separation=this->catalogue_ref.min_halo_separation;
#else
  ...
  this->catalogue_ref.get_mean_separation_in_cell();
#if defined _ASSIGN_PROPERTIES_TO_REFERENCE_ || defined (_ASSIGN_PROPERTIES_TO_NEW_REFERENCE_)
  this->min_mean_halo_separation=this->catalogue_ref.min_halo_separation;
#else
...
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
#ifdef _FULL_VERBOSE_
    So.message_screen("Allocating", LENGHT_AB* (sizeof(real_prec)+sizeof(size_t))/(1e9), "Gb for list of Vmax (reference) in theta-bins");
#endif
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
#ifdef _FULL_VERBOSE_
    So.message_screen("Allocating", LENGHT_AB* (sizeof(real_prec)+sizeof(size_t))/(1e9), "Gb for list of masses (reference) in theta-bins");
#endif
#endif
...
#ifdef _USE_STATISTICAL_ASSIGNMENT_
#ifdef _FULL_VERBOSE_
  So.message_screen("Allocating", LENGHT_AB_ONE* (sizeof(size_t))/(1e9), "Gb for probability distribution");
#endif
...
#endif
#ifdef _USE_STATISTICAL_ASSIGNMENT_
  So.message_screen("**Measuring n(M|theta) from the reference  using ", this->catalogue_ref._NOBJS()," objects");
#else
#ifdef _FULL_VERBOSE_
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
    So.message_screen("**Identifying reference masses in bins of {theta}_ref using ", this->catalogue_ref._NOBJS()," objects");
#elif defined _USE_VMAX_AS_PRIMARY_OBSERVABLE_
  So.message_screen("**Identifying reference values of Vmax in bins of {theta}_ref with ", this->catalogue_ref._NOBJS()," objects from the reference catalog");
#endif
#endif
#endif
#ifdef  _SET_THRESHOLD_FOR_PROP_RANDOM_TRACERS_
    size_t counter_vmax=0;
    size_t counter_vmax_r=0;
#endif
...
// Do not parallelize. There are push_backs inside this loop.
  for(size_t ig = 0; ig< this->catalogue_ref._NOBJS() ; ++ig)     // Open loop over the number of reference tracer objects
    {
      size_t I_Y=0;
#ifdef _USE_STATISTICAL_ASSIGNMENT_
...
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
      real_prec halo_prop=log10(this->catalogue_ref.mass_at(ig)/this->params._MASS_units());  // Using the logarithm of Mass
      I_Y= get_bin(halo_prop,lm_min,this->params._NMASSbins_mf(),this->htools_ref->logdeltaM,this->bin_accumulate_borders);
#elif defined _USE_VMAX_AS_PRIMARY_OBSERVABLE_
     real_prec halo_prop=log10(this->catalogue_ref.vmax_at(ig));   //Using the logarithn of Vmax
    ...
    // Get ID of this reference dark matter tracers
      size_t ID=this->catalogue_ref.GridID_at(ig);
    // Get bin of DM ovedensity
#ifdef _USE_DM_DENSITY_AT_HALO_POSITION_
      real_prec xdm=linInterpol(this->params._Nft(),this->params._Lbox(), this->params._d1(),this->catalogue_ref.coord1_at(ig),this->catalogue_ref.coord2_at(ig),this->catalogue_ref.coord3_at(ig),this->delta_dm_aux_mem);
      size_t I_X  = get_bin(xdm,this->s_mins.prop1,this->params._NX(),this->s_deltas.prop1,this->bin_accumulate_borders);
      this->catalogue_ref.local_dm_at(ig)=xdm;
#else
      ...
      this->catalogue_ref.gal_cwt_at(ig)=this->cwclass_ref.CWClass[ID];
#endif
      size_t I_MK=0;
#ifdef _USE_MASS_KNOTS_
      I_MK= (this->cwclass_ref.cwt_used[I_CWT]== WebType::Knot ? this->cwclass_ref.SKNOT_M_info[ID]: 0);
#endif
      size_t I_CWV=0;
#ifdef _USE_CWC_V_
      ...
      I_VK= (this->cwclass_ref.cwv_used[I_CWV]== WebType::Knot ? this->cwclass_ref.VDISP_KNOT_info[ID]: 0);
#endif
      size_t I_C1=0;
#ifdef _USE_TOTAL_MASS_IN_CELL_
      ...
      size_t I_C2=0;
#ifdef _USE_NUMBER_OF_NEIGHBOURS_
      I_C2=this->catalogue_ref.Number_of_neighbours_at(ig);
      ...
      real_prec min_sep = this->catalogue_ref.min_separation_in_cell[ID];
      I_C2 = get_bin(min_sep,  MIN_SEP_IN_CELLS, N_b, delta_min_sep , this->bin_accumulate_borders);
#elif defined _USE_MEAN_SEPARATIONS_IN_CELLS_
      real_prec min_sep = this->catalogue_ref.min_mean_separation_in_cell[ID];
              ...
              if(CV1<= this->s_maxs.prop7 && CV1>= this->s_mins.prop7)
                ...
                if(CV2<= this->s_maxs.prop8 && CV2>= this->s_mins.prop8)
                  ...
                  if(CV3<= this->s_maxs.prop9 && CV3>= this->s_mins.prop9)
                      ...
                      {
#endif
                      std::vector<size_t>idx={I_X, I_CWT, I_MK, I_CWV,I_VK, I_C1, I_C2, I_C3, I_CV1, I_CV2, I_CV3};
                      std::vector<size_t>dims={0,static_cast<size_t>(this->params._n_cwt()), static_cast<size_t>(this->params._n_sknot_massbin()), static_cast<size_t>(this->params._n_cwv()), static_cast<size_t>(this->params._n_vknot_massbin()),N_a, N_b ,N_c, N_v, N_CV_BIN2,N_CV_BIN3};    
                      size_t index=index_nd(idx,dims);
                      // The container ABUNDANCE will be used used to assign property to mock tracers below the threshold Xthreshold
#ifdef _USE_STATISTICAL_ASSIGNMENT_
                      size_t index_prop_ab= index_2d(I_Y,index_ant,LENGHT_AB);  // In case that we assign the the paired-*referennce, we need this.If we assign to the same reference, we do not need to get abundance
                      this->ABUNDANCE[index_prop_ab]++;
#endif
#ifndef _USE_STATISTICAL_ASSIGNMENT_
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
                      this->dm_properties_bins[index_ant].tracer_properties.push_back(this->catalogue_ref.mass_at(ig));
                     ...
                     // If the ordered index vmax_index (ordered from bottom-to-top in vmax) for each reference tracer is higher than Nran, means that the vmax for this tracer wll be assigned to DMparticles at assignment campaign
                      // while the lowest this->catalogue.Ntracers_ran vmax available will be assigned to tracers generated with random coordinates.
#ifdef _BOTTOM_RANDOM_
                      if(this->catalogue_ref.Halo_at(ig).vmax_index >= this->catalogue.Ntracers_ran)// This "if" guarranties that the number of vmax contained here are equal to the number of dm- assigned tracers.
#elif defined (_TOP_RANDOM_)                                                                                     // In the Bottom-random case, the dm particles get the highst Vmax values
                      if(this->catalogue_ref.Halo_at(ig).vmax_index < this->catalogue.Ntracers_dm)  // In the Top_Random case , dm particles get the lowest vmax values
#endif
                       {
                           counter_vmax++;
#endif
                           this->dm_properties_bins[index_ant].tracer_properties.push_back(this->catalogue_ref.vmax_at(ig));
                           this->dm_properties_bins[index_ant].used_property.push_back(false);
#ifdef _ASSIGN_MASS_LINKED_TO_V_FROM_REF_
                           this->dm_properties_bins[index_ant].tracer_properties_secondary.push_back(this->catalogue_ref.mass_at(ig));
                       ...
                       else  // if this->catalogue_ref.Halo_at(ig).vmax_index < this->catalogue.Ntracers_ran, we fill withthe value sof Vmax the vectors of structures for the randoms
                        {
                          this->dm_properties_for_randoms_bins[index_ant].tracer_properties.push_back(this->catalogue_ref.vmax_at(ig));
  ...
  this->So.DONE();
    //  So.message_warning_ini(__LINE__, __PRETTY_FUNCTION__, __FILE__,"catalogue_ref.clear_mem()() is disabled in order to compute correlation between masses");
     // this->catalogue_ref.clear_mem()();
#ifdef _GET_HALO_MASS_PROPERTIES_CORRELATIONS_
  mean_ldm/=static_cast<real_prec>(this->catalogue_ref._NOBJS());
  mean_hmass/=static_cast<real_prec>(this->catalogue_ref._NOBJS());
  m_dm/=static_cast<real_prec>(this->catalogue_ref._NOBJS());
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
...
#endif
#ifdef _USE_TRACERS_IN_CELLS_
  mean_ntr/=static_cast<real_prec>(this->catalogue_ref._NOBJS());
  m_ntr/=static_cast<real_prec>(this->catalogue_ref._NOBJS());
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
#ifdef _FULL_VERBOSE_
...
#pragma omp parallel for reduction(+:aux_a)
  for(size_t ih=0;ih<LENGHT_AB ;++ih)
  ...
  So.message_screen("Expected =", this->catalogue.Ntracers_dm);
#else
#ifdef _FULL_VERBOSE_
...
#pragma omp parallel for reduction(+:aux_b)
  ...
  So.message_screen("Expected =", this->catalogue.Ntracers_ran);
  aux_a+=aux_b;
#endif
  if(aux_a<this->catalogue_ref._NOBJS())
    {
      ...
      std::cout<<aux_a<<"  "<<this->catalogue_ref._NOBJS()<<endl;
      exit(0);
    }
  So.DONE();
#ifdef _USE_STATISTICAL_ASSIGNMENT_
...
#endif
#pragma omp parallel for reduction(+:aux_a)
  for(size_t ih=0;ih<this->ABUNDANCE.size() ;++ih)
     aux_a+=ABUNDANCE[ih];
  if(aux_a<this->catalogue_ref._NOBJS())
    {
...
#pragma omp parallel for
#endif
   for(size_t idm = 0; idm < LENGHT_AB; ++idm)
     {
      long aux_a=-10000;
      long aux_b;
      ...
      for(int tr_j = 0; tr_j < N_x; ++tr_j)
        {
          size_t indexa=index_2d(tr_j,idm,LENGHT_AB);
...
// ========================================================================================
  ...
  size_t ncells_used = static_cast<size_t>(get_nobjects(this->ABUNDANCE));
  if(ncells_used<this->catalogue_ref._NOBJS())
    {
      So.message_warning_ini(__LINE__, __PRETTY_FUNCTION__, __FILE__,"The number of tracers counted in ABUNDANCE(Y,X) is smaller than the input. Perhaps the range for some of the DMF properties *must* be increased to include all cells. CosmiCatlas stops here");
      std::cout<<"Number of cells used = "<<ncells_used<<"  Number expected = "<<this->catalogue_ref._NOBJS()<<endl;
    }
   ...
   this->ABUNDANCE_normalized.resize(LENGHT_AB_ONE, 0);
    ...
    size_t LENGHT_AC  = N_CV_BIN1*N_CV_BIN2*N_CV_BIN3*N_a* N_b* N_c * this->params._n_sknot_massbin() * this->params._n_cwt()*this->params._n_vknot_massbin()*this->params._n_cwv()*this->params._NX();
...
#endif
    for(size_t i=0;i < this->params._NX();++i)// loop sobre bines de dark matter
                    ...
                    for(int lv1 = 0; lv1 < N_CV_BIN1; ++lv1)
                      for(int lv2 = 0; lv2 < N_CV_BIN2; ++lv2)
                        for(int lv3 = 0; lv3 < N_CV_BIN3; ++lv3)
                           {
                                std::vector<size_t>idx={i,sua,k,vua,vk,l1,l2,l3,lv1,lv2,lv3};
                                std::vector<size_t>dims={0,this->params._n_cwt(),this->params._n_sknot_massbin(),this->params._n_cwv(),this->params._n_vknot_massbin(),N_a,N_b,N_c, N_CV_BIN1,N_CV_BIN2,N_CV_BIN3};    
                                size_t index_l=index_nd(idx,dims);
                                
                                long aux_a=-10000;
                                long aux_b;
                                for(int tr_j = 0; tr_j < this->params._NMASSbins_mf(); ++tr_j)
                                  {
                                    std::vector<size_t>idxa={tr_j,i,sua,k,vua,vk,l1,l2,l3,lv1, lv2, lv3};
                                    std::vector<size_t>dimsa={0,this->params._NX(),this->params._n_cwt(),this->params._n_sknot_massbin(),this->params._n_cwv(),this->params._n_vknot_massbin(),N_a,N_b,N_c, N_CV_BIN1,N_CV_BIN2,N_CV_BIN3};    
                                    size_t indexa=index_nd(idxa,dimsa);
                                    long AUX=this->ABUNDANCE[indexa];
                                    aux_b=max(aux_a, AUX);
                                    aux_a=aux_b;
                                    std::vector<size_t>idxb={tr_j,i,sua,k,vua,vk,l1,l2,l3,lv1,lv2,lv3};
                                    std::vector<size_t>dimsb={0,this->params._NX(),this->params._n_cwt(),this->params._n_sknot_massbin(),this->params._n_cwv(),this->params._n_vknot_massbin(),N_a, N_b,N_c, N_CV_BIN1,N_CV_BIN2,N_CV_BIN3};    
                                    size_t indexa=index_nd(idxb,dimsb);
#ifdef _USE_OMP_
#pragma omp atomic update
#endif
...
#endif
  // ******************************************************************************
  size_t N_c=1; // this applies to tidal anisotropy, inv tidal field iv, s2
...
#endif
  // ******************************************************************************
  size_t LENGHT_AB=N_v*N_a* N_b* N_c*N_x*N_bias;
  LENGHT_AB*=this->params._n_sknot_massbin() * this->params._n_cwt() * this->params._n_vknot_massbin() * this->params._n_cwv()*this->params._NX();
  // ******************************************************************************
#ifdef _USE_STATISTICAL_ASSIGNMENT_
  size_t N_x = this->params._NPROPbins_bam();
...
// Get min and max of the differnet properties invovled
  this->get_new_min_max_properties();
  this->catalogue_ref.set_params(this->params);
  this->catalogue_ref.set_type_of_object("TRACER_REF");
  ...
  //read catalog passing as argument the file and the mininum mass requested
     ...
     this->catalogue_ref.read_catalog_new(newcat);   // READ INPUT CATALOG //
#elif defined (_SET_CAT_WITH_VMAX_CUT_)
  this->catalogue_ref.read_catalog(newcat,params._VMAXmin());
#else
  this->catalogue_ref.read_catalog_new(newcat);
#endif
#else
#ifdef _SET_CAT_WITH_MASS_CUT_ to deprecate
  this->catalogue_ref.read_catalog(newcat,pow(10,params._LOGMASSmin())*params._MASS_units(),static_cast<real_prec>(BIG_NUMBER));
#elif defined (_SET_CAT_WITH_VMAX_CUT_)
  this->catalogue_ref.read_catalog(newcat,params._VMAXmin(),static_cast<real_prec>(BIG_NUMBER));
#endif
#endif

     HaloTools* htools_ref = nullptr;
     htools_ref = new HaloTools(params, catalogue_ref);

// *********************************************************************************************
//update_params:
#ifdef _USE_FIXED_MULTISCALE_LEVELS_
  this->params=this->catalogue_ref._params(); // Update params
#endif
  // *********************************************************************************************
  ...
  string newcat_aux=this->params._Input_dir_cat_new_ref()+this->params._file_catalogue_new_ref();
     ...
     this->catalogue_aux.read_catalog_new(newcat_aux);
#elif defined (_SET_CAT_WITH_VMAX_CUT_)
  this->catalogue_aux.read_catalog(newcat_aux,params._VMAXmin());//to move to _new()
#else
  this->catalogue_aux.read_catalog_new(newcat_aux);
#endif
#else
#ifdef _SET_CAT_WITH_MASS_CUT_
  this->catalogue_aux.read_catalog(newcat_aux,pow(10,params._LOGMASSmin())*params._MASS_units(),static_cast<real_prec>(BIG_NUMBER));/to move to _new()
#elif defined (_SET_CAT_WITH_VMAX_CUT_)
  this->catalogue_aux.read_catalog(newcat_aux,params._VMAXmin(),static_cast<real_prec>(BIG_NUMBER));/to move to _new()
#endif
#endif

    this->catalogue.set_params(this->params);

    this->catalogue.resize_positions(this->catalogue_aux._NOBJS());
    this->catalogue.resize_velocities(this->catalogue_aux._NOBJS());
    this->catalogue.resize_GridID(this->catalogue_aux._NOBJS());
    this->catalogue.resize_GridID_n(this->catalogue_aux._NOBJS());
    
    #ifdef _USE_OMP_
#pragma omp parallel for
#endif
     for(size_t i=0; i<this->catalogue_aux._NOBJS(); ++i)
       {
          this->catalogue.set_coord1(
             this->catalogue_aux.coord1_at(i),i);
        
          this->catalogue.set_coord2(
            this->catalogue_aux.coord2_at(i), i);

          this->catalogue.set_coord3(
              this->catalogue_aux.coord3_at(i), i);

          this->catalogue.set_vel1(
              this->catalogue_aux.vel1_at(i), i);

          this->catalogue.set_vel2(
              this->catalogue_aux.vel2_at(i), i);

          this->catalogue.set_vel3(
              this->catalogue_aux.vel3_at(i), i);

          this->catalogue.set_GridID(
              this->catalogue_aux.GridID_at(i), i);

          this->catalogue.set_GridID_n(
              this->catalogue_aux.GridID_n_at(i), i);

        
#ifdef _CHECK_ASSIGN_
        this->catalogue.set_mass(
            this->catalogue_aux.mass_at(i), i);
#endif
       }
    this->catalogue.set_NOBJS(this->catalogue_aux._NOBJS());
...
#pragma omp parallel for
#endif
     for(size_t i=0; i<this->catalogue_ref._NOBJS(); ++i) // Copy the information of the reference catalog to the "new" catalog, except for the properties to be assigned
       {
          this->catalogue.set_coord1(
             this->catalogue_aux.coord1_at(i),i);
          ...
          this->catalogue.set_mass(0, i);
          this->catalogue.set(0,i);
       }
    this->catalogue.set_NOBJS(this->catalogue_ref._NOBJS());
    this->catalogue.set_type_of_object("TRACER_MOCK");
#endif
#endif
  this->catalogue_aux.clear_mem(); 
// ************************************************************************************
  htools_ref->get_property_function(this->params._Output_directory()+"tracer_ref_abundance.txt");
#if defined _USE_NEIGHBOURS_ || defined _GET_DIST_MIN_SEP_REF_ || defined _GET_DIST_MIN_SEP_MOCK_
  So.message_screen("Identifying Neighbouring Cells (this is done once)");
  this->ncells_info.resize( this->params._NGRID());
  get_neighbour_cells(this->params._Nft(), N_CELLS_BACK_FORTH, this->ncells_info);
...
bias_pr:
   if(this->params._Get_tracer_bias())
    { // WE leave the param question separated from the prerpc, as we would want to use bias to plot but not to assign props
     ...
     power_new.object_by_object_bias(this->catalogue_ref, this->delta_dm_aux_mem);
     htools_ref->set_params(this->params);

#ifndef _ASSIGN_PROPERTIES_TO_NEW_REFERENCE_
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
     for(size_t i=0; i<this->catalogue_ref._NOBJS(); ++i)//Copy the bias from the "ref" to the "new" catalog. Since the bias is computed from the coords, both can have the same unless coords change
         this->catalogue.bias[i]=this->catalogue_ref.bias[i];
#endif
     htools_ref->get_min("_BIAS_");
     htools_ref->get_max("_BIAS_");
     So.message_screen("Minimum bias",this->htools_ref->_min_bias());
     So.message_screen("Maximum bias",this->htools_ref->_max_bias());
   }
     // ********************************************************************************
#ifdef _USE_MACH_NUMBER_
#ifndef _READ_BIAS_
#ifdef _USE_CHUNCKS_NC_
    this->catalogue_ref.get_local_mach_number_chuncks(this->params._Scale_mach_number());//here get mach and delta5
#else
    this->catalogue_ref.get_local_mach_number(this->params._Scale_mach_number());//here get mach and delta5
#endif
#endif
...
#pragma omp parallel for
#endif
     for(size_t i=0; i<this->catalogue_ref._NOBJS(); ++i)//Copy the mach_number from the "ref" to the "new" catalog. Since mach is computed from the phase-space coords, both can have the same unless coords change
         this->catalogue.mach_number[i]=this->catalogue_ref.Halo[i].mach_number ;
#endif
     this->catalogue_ref.set_min_mach(this->catalogue_ref.get_min("_MACH_"));
     this->catalogue_ref.set_max_mach(this->catalogue_ref.get_max("_MACH_"));
     So.message_screen("Minimum mach",this->htools_ref->_minmach());
     So.message_screen("Maximum mach",this->htools_ref->_maxmach());
#endif
     // ********************************************************************************
#ifdef _USE_LOCAL_OVERDENSITY_
#ifndef _ASSIGN_PROPERTIES_TO_NEW_REFERENCE_
...
#pragma omp parallel for
#endif
     for(size_t i=0; i<this->catalogue_ref._NOBJS(); ++i)//Copy the local_delta from the "ref" to the "new" catalog. Since local_delta is computed from the phase-space coords, both can have the same unless coords change
         this->catalogue.local_overdensity[i]=this->catalogue_ref.local_overdensity[i];
#endif
     htools_ref->set_min_local_overdensity(this->catalogue_ref.get_min("_LOCAL_OVERDENSITY_"));
     htools_ref->set_max_local_overdensity(this->catalogue_ref.get_max("_LOCAL_OVERDENSITY_"));
     So.message_screen("Minimum local-overd",this->htools_ref->_minlocal_overdensity());
     So.message_screen("Maximum local-overd",this->htools_ref->_maxlocal_overdensity());
#ifdef _WRITE_BIAS_
    ...
    for(size_t i=0;i<this->catalogue_ref._NOBJS();++i)
      bout<<this->catalogue_ref.bias_at(i)<<"\t"<<this->catalogue_ref.mach_number_at(i)<<"\t"<<catalogue_ref.local_overdensity_at(i)<<"\t"<<this->catalogue_ref.number_of_neighbours_at(i)<<endl;
    bout.close();
#endif
#endif
     // ********************************************************************************
...
#pragma omp parallel for
      ...
      htools_ref->get_tracer_tidal_anisotropy(tidal);
      So.message_screen("\tMin TA :",this->htools_ref->_mintidal_anisotropy());
      So.message_screen("\tMax TA :",this->htools_ref->_maxtidal_anisotropy());
...
#pragma omp parallel for
  ...
  vector<real_prec> REF_DEN_FIELD( this->params._NGRID(),0);
  this->File.read_array(this->params._Input_Directory_Y()+this->params._Name_Catalog_Y(),REF_DEN_FIELD);
//  this->catalogue_ref.get_density_field_grid(_COUNTS_,REF_DEN_FIELD);
  int nmax=get_max<real_prec>(REF_DEN_FIELD);
  So.message_screen("Maximum number of tracer in cell", nmax);
#endif
#ifdef _USE_CROSS_CORRELATION_CONF_SPACE_
...
#ifdef _USE_TOTAL_MASS_IN_CELL_
  vector<real_prec> REF_MASS_FIELD( this->params._NGRID(),0);
  this->catalogue_ref.get_density_field_grid(_VMAX_, REF_MASS_FIELD);
  real_prec mmax=get_max<real_prec>(REF_MASS_FIELD);
#ifdef _FULL_VERBOSE_
  ...
  real_prec delta_min_sep = (MAX_SEP_IN_CELLS-MIN_SEP_IN_CELLS)/static_cast<real_prec>(N_BINS_MIN_SEP_IN_CELLS);
#elif defined _USE_MEAN_SEPARATIONS_IN_CELLS_
  this->catalogue_ref.get_stats_separation_in_cell(); // tghis method also computes the men and stdev ojo
#ifdef _ASSIGN_PROPERTIES_TO_REFERENCE_
  this->min_mean_halo_separation= MIN_MEAN_SEP_IN_CELLS;// this->catalogue_ref.min_mean_halo_separation;
#else
  ...
  N_a= N_BINS_STDV_SEP_IN_CELLS ;
    ...
    this->catalogue_ref.get_stats_separation_in_cell(); // this also works to get the mean and stedev
#endif
...
#endif
#ifdef _USE_STATISTICAL_ASSIGNMENT_
  So.message_screen("**Measuring n(Vmax|{Theta}) from the reference using ", this->catalogue_ref._NOBJS()," objects");
#else
#ifdef _FULL_VERBOSE_
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
    So.message_screen("**Identifying reference masses in bins of {theta}_ref using ", this->catalogue_ref._NOBJS()," objects");
...
// Do not parallelize when reading from the reference. There are push_backs inside this loop.
   for(size_t ig = 0; ig< this->catalogue_ref._NOBJS() ; ++ig)
     {
       size_t I_Y=0;
#ifdef _USE_STATISTICAL_ASSIGNMENT_
     // When assigning via reading from the reference values, those values are allocated in the dm_prop structure and then
...
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
      real_prec halo_prop=log10(this->catalogue_ref.mass_at(ig)/this->params._MASS_units());  // Using the logarithm of Mass
      I_Y= get_bin(halo_prop,lm_min,this->params._NMASSbins_mf(),this->htools_ref->logdeltaM,this->bin_accumulate_borders);//ojo con el numero de bies cá, arreglarlo
#elif defined _USE_VMAX_AS_PRIMARY_OBSERVABLE_
     real_prec halo_prop=log10(this->catalogue_ref.vmax_at(ig));   //Using the logarithn of Vmax
      I_Y =get_bin(halo_prop, log10(this->params._VMAXmin()),N_x,log10(this->params._VMAXmax()/this->params._VMAXmin())/static_cast<double>(N_x), true);
#endif
#endif
      size_t ID=this->catalogue_ref.GridID_at(ig); // Get ID of this reference dark matter tracers
#ifdef _USE_DM_DENSITY_AT_HALO_POSITION_
      // Get bin of DM ovedensity
      real_prec xdm=linInterpol(this->params._Nft(),this->params._Lbox(), this->params._d1(),this->catalogue_ref.coord1_at(ig),this->catalogue_ref.coord2_at(ig),this->catalogue_ref.coord3_at(ig),this->delta_dm_aux_mem);
      size_t I_X  = get_bin(xdm,this->s_mins.prop1,this->params._NX(),this->s_deltas.prop1,this->bin_accumulate_borders);
      ...
      I_CWT= static_cast<size_t>(this->cwclass_ref.get_Tclassification(ID));
      this->catalogue_ref.gal_cwt_at(ig)=this->cwclass_ref.CWClass[ID];
#endif
      size_t I_MK=0;
#ifdef _USE_MASS_KNOTS_
      I_MK= static_cast<size_t>(this->cwclass_ref.cwt_used[I_CWT]== WebType::Knot ? this->cwclass_ref.SKNOT_M_info[ID]: 0);
#endif
      size_t I_CWV=0;
#ifdef _USE_CWC_V_
      I_CWV=cwclass_ref.get_Vclassification(ID);
      ...
      I_C1= get_bin(C1, this->s_mins.prop4, N_C_BIN1, s_deltas.prop4,this->bin_accumulate_borders);
       ...
       real_prec sep_std = this->catalogue_ref.stdv_separation_in_cell[ID];
      ...
      I_C2 = get_bin(min_sep,  MIN_SEP_IN_CELLS, N_b, delta_min_sep , this->bin_accumulate_borders);
#elif  defined _USE_MEAN_SEPARATIONS_IN_CELLS_
      real_prec min_sep = this->catalogue_ref.mean_separation_in_cell[ID];
      ...
      real_prec C3  =linInterpol(this->params._Nft(),this->params._Lbox(), this->params._d1(),this->catalogue_ref.coord1_at(ig),this->catalogue_ref.coord2_at(ig),this->catalogue_ref.coord3_at(ig),this->cwclass_ref.Tidal_Anisotropy);
      I_C3= get_bin(C3, this->s_mins.prop6, N_C_BIN3, s_deltas.prop6,this->bin_accumulate_borders);
#else
...
#elif defined _USE_MACH_NUMBER_
      I_C3= get_bin(this->catalogue_ref.mach_number_at(ig),this->htools_ref->_minmach(), N_BINS_MACH, (this->htools_ref->_maxmach()-this->htools_ref->_minmach())/static_cast<real_prec>(N_BINS_MACH), this->bin_accumulate_borders);
#endif
       size_t I_CV1=0;
#ifdef _USE_TRACERS_IN_CELLS_
      I_CV1=get_bin(static_cast<int>(REF_DEN_FIELD[ID]),0,N_BINS_TRACERS_IN_CELLS,DELTA_TRACERS_IN_CELLS,true);
//      I_CV1=static_cast<int>(REF_DEN_FIELD[ID]);
      ...
      I_CV2= get_bin(this->catalogue_ref.local_overdensity_at(ig), this->htools_ref->_minlocal_overdensity(), N_BINS_LO,  (this->htools_ref->_maxlocal_overdensity()-this->htools_ref->_minlocal_overdensity())/static_cast<real_prec>(N_BINS_LO) ,this->bin_accumulate_borders);
#endif
      size_t I_CV3=0;
#ifdef _USE_INVARIANT_SHEAR_VFIELD_III_
      real_prec CV3 = invariant_field_III(cwclass_ref.lambda1_vs[ID],cwclass_ref.lambda2_vs[ID],cwclass_ref.lambda3_vs[ID]);// Not yet assigned to container
      ...
      I_CV3 = get_bin(this->catalogue_ref.Halo_at(ig).bias, this->htools_ref->min_bias(), N_BINS_BIAS,(this->htools_ref->max_bias()-this->htools_ref->min_bias())/static_cast<real_prec>(N_BINS_BIAS),this->bin_accumulate_borders);
#endif
#ifdef _GET_HALO_MASS_PROPERTIES_CORRELATIONS_
      mean_ldm+=xdm;
      mean_hmass+=halo_prop;
              ...
              if(CV1<= this->s_maxs.prop7 && CV1>= this->s_mins.prop7)
                ...
                if(CV2<= this->s_maxs.prop8 && CV2>= this->s_mins.prop8)
                  ...
                  if(CV3<= this->s_maxs.prop9 && CV3>= this->s_mins.prop9)
                     ...
                     {
#endif
                      std::vector<size_t>idxa={I_X, I_CWT, I_MK, I_CWV,I_VK, I_C1, I_C2, I_C3, I_CV1, I_CV2, I_CV3};
                      std::vector<size_t>dimsa={0,static_cast<size_t>(this->params._n_cwt()), static_cast<size_t>(this->params._n_sknot_massbin()), static_cast<size_t>(this->params._n_cwv()), static_cast<size_t>(this->params._n_vknot_massbin()),N_a, N_b ,N_c, N_v, N_x,N_bias};    
                      size_t index_ant=index_nd(idxa,dimsa);
...
#endif // closes _SET_THRESHOLD_FOR_PROP_RANDOM_TRACERS_
  try{
      check_if_positive_number<long>(this->catalogue_ref._NOBJS()-aux_a);
     }
...
#pragma omp parallel for reduction(+:baux_a)
  ...
  for(size_t ih=0;ih<this->ABUNDANCE.size() ;++ih)
     baux_a+=ABUNDANCE[ih];
  if(baux_a<this->catalogue_ref._NOBJS())
    {
      So.message_warning_ini(__LINE__, __PRETTY_FUNCTION__, __FILE__,"The number of tracers counted in ABUNDANCE(Y,X) is smaller than the input. Perhaps the range for some of the DMF properties *must* be increased to include all cells. CosmiCatlas stops here");
      std::cout<<baux_a<<"  "<<this->catalogue_ref._NOBJS()<<endl;
      exit(0);
    }
  So.DONE();
  //* Final check to verify that all tracers have been counted in
  ...
  if(ncells_used<this->catalogue_ref._NOBJS())
    {
      So.message_warning_ini(__LINE__, __PRETTY_FUNCTION__, __FILE__,"The number of tracers counted in ABUNDANCE(Y,X) is smaller than the input. Perhaps the range for some of the DMF properties *must* be increased to include all cells. CosmiCatlas stops here");
      std::cout<<"Number of objects allocated = "<<ncells_used<<"  Number expected = "<<this->catalogue_ref._NOBJS()<<endl;
    }
    else
...
#pragma omp parallel for private(aux_a,aux_b)
  ...
  {
    aux_a=-10000;
    for(size_t tr_j = 0; tr_j < N_x; ++tr_j)
      {
...
#pragma omp parallel for reduction(+:aux_h)
#endif
    for(size_t ih=0;ih<this->ABUNDANCE_normalized.size();++ih)
      aux_h+=this->ABUNDANCE_normalized[ih];
...
//* This function is explicitely meant to be applid to assign masses/vmax after having assigned vmax/masses  *//
{
  ...
  size_t Ntot=1; // Ntot is the total number of bins to use in the P(Y|Xi)
  //-----------------------
  ...
  size_t n_cwt=1;
   ...
   n_cwt= static_cast<size_t>(this->params._n_cwt());
#endif
  Ntot*=n_cwt;
  //-----------------------
  // For X4:
...
#pragma omp for nowait
    ...
    for(size_t ig = 0; ig< this->catalogue_ref._NOBJS() ; ++ig)
     {
       size_t ID=this->catalogue_ref.GridID_at(ig);       // Get ID of this tracer
      // -----------------------------------------------------------------------------------------
      // P(Y|X1,X2,X3,X4,X5) h_property refers to Y
       size_t I_Y=0;//This index takes the place of the variable to assign (e.g. Mvir, Rs), so here it is read form ref
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
       if(h_property==_MASS_)// if we want to assign mass, learn from it
        I_Y  = get_bin(log10(this->catalogue_ref.mass_at(ig)),this->params._LOGMASSmin(),this->params._NPROPbins_bam(), (this->params._LOGMASSmax()-this->params._LOGMASSmin())/static_cast<double>(this->params._NPROPbins_bam()),this->bin_accumulate_borders);
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
       if(h_property==_VMAX_)
        I_Y  = get_bin(log10(this->catalogue_ref.vmax_at(ig)),log10(this->params._VMAXmin()),this->params._NPROPbins_bam(),this->htools_ref->logdeltaVMAX,this->bin_accumulate_borders);
#endif
#ifdef  _USE_RS_AS_DERIVED_OBSERVABLE_  
        else if(h_property==_RS_ )
         I_Y  = get_bin(log10(this->catalogue_ref.Halo_at(ig).rs),log10(this->params._RSmin()),this->params._NPROPbins_bam(),this->htools_ref->logdeltaRS,this->bin_accumulate_borders);
#endif
#ifdef _USE_CONCENTRATION_AS_DERIVED_OBSERVABLE_
        else if( h_property == _CONCENTRATION_)
         I_Y  = get_bin(log10(this->catalogue_ref.concentration_at(ig)),log10(this->params._CONCENTRATIONmin()),this->params._NPROPbins_bam(),log10(this->params._CONCENTRATIONmax()/this->params._CONCENTRATIONmin())/static_cast<double>(this->params._NPROPbins_bam()),this->bin_accumulate_borders);
#endif
#ifdef _USE_SPIN_AS_DERIVED_OBSERVABLE_
        else if(h_property==_SPIN_)
          I_Y  = get_bin(log10(this->catalogue_ref.spin_at(ig)), log10(this->params._SPINmin()),this->params._NPROPbins_bam(),this->htools_ref->logdeltaSPIN,this->bin_accumulate_borders);
        else if( h_property==_SPIN_BULLOCK_)
          I_Y  = get_bin(log10(this->catalogue_ref.spin_bullock_at(ig)), log10(this->params._SPINmin()),this->params._NPROPbins_bam(),this->htools_ref->logdeltaSPIN,this->bin_accumulate_borders);
#endif
      size_t I_X1=0;
      ...
      if(h_property==_RS_ || h_property == _CONCENTRATION_ || h_property==_SPIN_  ||  h_property==_SPIN_BULLOCK_ ) // Using DELTA only allowed so far when assigning mass. FOr Rs and Spin we use so far P(Rs|M,Vmax) or P(S|M,Vmax)
        I_X1 =  get_bin(log10(this->catalogue_ref.mass_at(ig)),this->params._LOGMASSmin(),this->params._NPROPbins_bam(), (this->params._LOGMASSmax()-this->params._LOGMASSmin())/static_cast<double>(this->params._NPROPbins_bam())  ,this->bin_accumulate_borders);
        // Use this if we want to use Vmax (even if primary) to assign seconday, passing over Mvir assigned
//        I_X1 =  get_bin(log10(this->catalogue_ref.vmax_at(ig)),log10(this->params._VMAXmin()),this->params._NPROPbins_bam(), (log10(this->params._VMAXmax())-log10(this->params._VMAXmin()))/static_cast<double>(this->params._NPROPbins_bam())  ,this->bin_accumulate_borders);
#endif
        ...
        if(h_property==_MASS_)
         I_X1=get_bin(log10(this->catalogue_ref.vmax_at(ig)), log10(this->params._VMAXmin()),this->params._NPROPbins_bam(),log10(this->params._VMAXmax()/this->params._VMAXmin())/static_cast<double>(this->params._NPROPbins_bam()), true);
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_ // If mvir is primary  we can now assign vmax from P(Vmax|M...)
       I_X1=get_bin(log10(this->catalogue_ref.mass_at(ig)), this->params._LOGMASSmin(),this->params._NPROPbins_bam(),(this->params._LOGMASSmax()-this->params._LOGMASSmin())/static_cast<double>(this->params._NPROPbins_bam()), true);
#endif
      }
   // Get bin of DM ovedensity
   // We use this bin as I_X ->DM for P(M|DM,Vmax) , or I_X -> Mass for P(Rs (or spin)|M,Vmax). Vmax will take the index I_CV2. If add_dm_density undef, I_X=0 for P(M|Vmax)
      ...
      if(h_property==_MASS_ || h_property==_VMAX_  || h_property==_CONCENTRATION_ || h_property==_RS_) // Using DELTA only allowed so far when assigning mass (or vmax) and concentration. FOr Rs we use so far P(Rs|M,Vmax)
      ...
      else if(h_property==_SPIN_ || h_property==_SPIN_BULLOCK_) // If we want to assign spin, we learn from concentration (already assigned)
        {
          ...
          I_X2  = get_bin(log10(this->catalogue_ref.Halo_at(ig).rs),log10(this->params._RSmin()),this->params._NPROPbins_bam(),this->htools_ref->logdeltaRS,this->bin_accumulate_borders);
#elif defined _USE_CONCENTRATION_AS_DERIVED_OBSERVABLE_
          I_X2  = get_bin(log10(this->catalogue_ref.concentration_at(ig)),log10(this->params._CONCENTRATIONmin()),this->params._NPROPbins_bam(),log10(this->params._CONCENTRATIONmax()/this->params._CONCENTRATIONmin())/static_cast<double>(this->params._NPROPbins_bam()),this->bin_accumulate_borders);
#endif
    ...
    // If assignment is for secondary (Mas has been already assigned), Let IX1 takes the place of halo mass
      size_t I_X3=0;
#ifdef _USE_CWC_mass_
       I_X3=static_cast<int>(this->cwclass_ref.get_Tclassification(ID));
#endif
      size_t I_X4=0;
#ifdef _USE_MACH_NUMBER_
      I_X4= get_bin(this->catalogue_ref.mach_number_at(ig),this->htools_ref->_minmach(), N_BINS_MACH, (this->htools_ref->_maxmach()-this->htools_ref->_minmach())/static_cast<real_prec>(N_BINS_MACH), this->bin_accumulate_borders);
#endif
      size_t I_X5=0;
#ifdef _USE_LOCAL_OVERDENSITY_
      I_X5= get_bin(this->catalogue_ref.local_overdensity_at(ig), this->htools_ref->_minlocal_overdensity(), N_BINS_LO,  (this->htools_ref->_maxlocal_overdensity()-this->htools_ref->_minlocal_overdensity())/static_cast<real_prec>(N_BINS_LO) ,this->bin_accumulate_borders);
#endif
      size_t index_dm=0;
      size_t I_X6=0;     
#ifdef _USE_TIDAL_ANISOTROPY_SEC_PROP_ 
      I_X6= get_bin(this->catalogue_ref.Halo_at(ig).tidal_anisotropy, this->htools_ref->_mintidal_anisotropy(), N_BINS_TA,  (this->htools_ref->_maxtidal_anisotropy()-this->htools_ref->_mintidal_anisotropy())/static_cast<real_prec>(N_BINS_TA) ,this->bin_accumulate_borders);
      std::vector<size_t>idxa={I_X1,I_X2,I_X3,I_X4,I_X5,I_X6};
      std::vector<size_t>dimsa={0,NbinsX2,n_cwt,N_BINS_MACH,N_BINS_LO,N_BINS_TA};    
      index_dm=index_nd(idxa,dimsa);
#else
      std::vector<size_t>idxa={I_X1,I_X2,I_X3,I_X4,I_X5};
      std::vector<size_t>dimsa={0,NbinsX2,n_cwt,N_BINS_MACH,N_BINS_LO};    
      index_dm=index_nd(idxa,dimsa);
#endif
#ifndef _BIN_ACCUMULATE_
       if(xdm <=this->s_maxs.prop1 && xdm >=this->s_mins.prop1)
          {
#endif
            size_t index_prop_b=index_2d(I_Y,index_dm,Ntot_partial); // I_Y is the bin of mass
#ifdef _USE_OMP_
  ...
  {
#pragma omp critical
    for(size_t i=0;i <this->ABUNDANCE.size();++i)
...
// Free memmory only in the last property (spin) has been assigned
   ...
   if(h_property==_SPIN_ || h_property == _SPIN_BULLOCK_)
      this->delta_dm_aux_mem.clear();delta_dm_aux_mem.shrink_to_fit();
#endif
/*  // These lines are commented since there is no strickt rule to free memmory here for the container this->catalogue_ref.Halo, as it willbe used below ehen comparing/writting bias to output files for comparison
{
...
#pragma omp parallel for
#endif
    for(size_t idm = 0; idm < Ntot_partial; ++idm)
     {
     long aux_a=-10000;
     long aux_b=0;
     for(size_t tr_j = 0; tr_j < this->params._NPROPbins_bam()  ; ++tr_j)
       {
         size_t indexa=index_2d(tr_j,idm,Ntot_partial);
         long AUX=this->ABUNDANCE[indexa];
         aux_b=max(aux_a, AUX);
         aux_a=aux_b;
       }
...
#pragma omp parallel for
#endif
      for(size_t i = 0;i <  this->params._NGRID() ;++i)  //TRANSFORM DELTA TO LOG10(NUM_IN_LOG + DELTA). tHIS IS DONE SINCE THE KONVOLUITION ALWAYS GOVES DELTAS, WE HAVE TO TRANSFORMTO LOG(1+DELTA)
...
#endif
   size_t LENGHT_BIAS_NCOUNT=N_CV_BIN1 * N_CV_BIN2* N_CV_BIN3* N_C_BIN1 * N_C_BIN2* N_C_BIN3* this->params._n_sknot_massbin() * this->params._n_cwt() * this->params._n_vknot_massbin() * this->params._n_cwv()* NX_NEW * this->new_nbins_y;
   size_t LENGHT_BIAS_NCOUNT_aux=N_CV_BIN1 * N_CV_BIN2* N_CV_BIN3* N_C_BIN1 * N_C_BIN2* N_C_BIN3* this->params._n_sknot_massbin() * this->params._n_cwt() * this->params._n_vknot_massbin() * this->params._n_cwv()* NX_NEW;
#ifdef _USE_GNUPLOT_
   size_t LENGHT_BIAS_LOCAL_DENSITY=NX_NEW * this->new_nbins_y;
#endif
   int count_arrays=1;
#ifdef _USE_MASS_TRACERS_
   count_arrays++;
...
#ifdef _DO_BiasMT_CALIBRATION_
#ifdef _FULL_VERBOSE_
   So.message_screen("Allocating", LENGHT_BIAS_NCOUNT*count_arrays*(sizeof(real_prec)+sizeof(size_t))/(1e9), "Gb for bias");
#endif
#endif
// This section will be done in parallel only if we have more than two threads
  ...
  this->BIAS_LOCAL_DM.resize(LENGHT_BIAS_NCOUNT, 0);
   ...
   So.message_screen("Reading BIAS from calibration");
    ...
    size_t ncounts_aux = static_cast<size_t>(get_nobjects(this->BIAS_NCOUNTS));
#ifdef _FULL_VERBOSE_
   ...
   if(this->use_iteration_ini) //NOT WORKING YET
     {
...
#pragma omp parallel for
#endif
       for(size_t ig = 0; ig<  this->params._NGRID() ; ++ig)
         {
           // Get number counts or the property catalogue. If displacement, the definition of Delta is on line 12277 of Bam.cpp (bamrunner)
           real_prec halo_prop = this->delta_Y_at(ig);
           int I_Y=0;
           if(_COUNTS_== property)
           ...
           real_prec halo_mass_prop=this->delta_Y_mass_at(ig);
           ...
           size_t I_X=0;
#ifdef _USE_DM_IN_BiasMT_
           real_prec xdm    = this->delta_X_at(ig);
           I_X  = ((this->params._iMAS_X() == 0  && !this->params._Convert_Density_to_Delta_X()) ? static_cast<int>(this->delta_X_at(ig)) : get_bin(xdm,this->s_mins.prop1,this->params._NX(),this->s_deltas.prop1,this->bin_accumulate_borders));
#endif
           size_t I_CWT=0;
#ifdef _USE_CWC_
           I_CWT=static_cast<size_t>(this->cwclass.get_Tclassification(ig));
#endif
           size_t I_MK=0;
#ifdef _USE_MASS_KNOTS_
           I_MK= (this->cwclass.cwt_used[this->cwclass.get_Tclassification(ig)]== WebType::Knot ? this->cwclass.SKNOT_M_info_at(ig): 0);
#endif
           size_t I_CWV=0;
#ifdef _USE_CWC_V_
           I_CWV=static_cast<int>(this->cwclass.get_Vclassification(ig));
#endif
           size_t I_VK=0;
#ifdef _USE_VEL_KNOTS_V_
           I_VK= (this->cwclass.cwv_used[I_CWV]== WebType::Knot ? this->cwclass.VDISP_KNOT_info_at(ig): 0);
#endif
           size_t I_C1=0;
#ifdef _USE_INVARIANT_TIDAL_FIELD_II_
           real_prec C1 = this->cwclass.Invariant_TF_II_at(ig);
           I_C1= get_bin(C1, this->s_mins.prop4, N_C_BIN1, s_deltas.prop4,this->bin_accumulate_borders);
#elif defined _USE_DELTA2_
           real_prec C1 = this->cwclass.DELTA2_at(ig);
           I_C1= get_bin(C1, this->s_mins.prop4, N_C_BIN1, s_deltas.prop4,this->bin_accumulate_borders);
#endif
           size_t I_C2=0;
#ifdef _USE_INVARIANT_TIDAL_FIELD_III_
           real_prec C2 = this->cwclass.Invariant_TF_III_at(ig);
           I_C2= get_bin(C2, this->s_mins.prop5, N_C_BIN2, this->s_deltas.prop5,this->bin_accumulate_borders);
#elif defined _USE_DELTA3_
           real_prec C2 = this->cwclass.DELTA3_at(ig);
           I_C2= get_bin(C2, this->s_mins.prop5, N_C_BIN2, s_deltas.prop5,this->bin_accumulate_borders);
#endif
           size_t I_C3=0;
#ifdef _USE_INVARIANT_TIDAL_FIELD_IV_
           real_prec C3 = this->cwclass.Invariant_TF_IV_at(ig);
           I_C3= get_bin(C3, this->s_mins.prop6, N_C_BIN3, this->s_deltas.prop6,this->bin_accumulate_borders);
#elif defined (_USE_TIDAL_ANISOTROPY_)
#ifdef _USE_ANISOTROPY_AT_TRACER_POSITION_
      real_prec C3  =linInterpol(this->params._Nft(),this->params._Lbox(), this->params._d1(),this->catalogue_ref.coord1_at(ig),this->catalogue_ref.coord2_at(ig),this->catalogue_ref.coord3_at(ig),this->cwclass.Tidal_Anisotropy);
      I_C3= get_bin(C3, this->s_mins.prop6, N_C_BIN3, s_deltas.prop6,this->bin_accumulate_borders);
#else
...
#endif
#elif defined _USE_S2_
           real_prec C3 = this->cwclass.S2_at(ig);             // s²
           I_C3= get_bin(C3, this->s_mins.prop6, N_C_BIN3, s_deltas.prop6,this->bin_accumulate_borders);
#endif
           size_t I_CV1=0;
#ifdef _USE_INVARIANT_SHEAR_VFIELD_I_
           real_prec CV1 = this->cwclass.Invariant_VS_I_at(ig);
           I_CV1= get_bin(CV1, this->s_mins.prop7, N_CV_BIN1, this->s_deltas.prop7, this->bin_accumulate_borders);
#elif defined _USE_NABLA2DELTA_
           real_prec CV1 = this->cwclass.N2D_at(ig);      // Nabla² ð
           I_CV1= get_bin(CV1, this->s_mins.prop7, N_CV_BIN1, s_deltas.prop7,this->bin_accumulate_borders);
 #elif defined (_USE_INVARIANT_PWEB_I_)
         real_prec CV1 = this->cwclass.Invariant_TF_I_at(ig); // When using PWEB, the iarrays contaning th nvariants of the tidal field are used to allocate the invariats of the Pfield
         I_CV1= get_bin(CV1, this->s_mins.prop7, N_CV_BIN1,this->s_deltas.prop7, this->bin_accumulate_borders);
#endif
           size_t I_CV2=0;
#ifdef _USE_INVARIANT_SHEAR_VFIELD_II_
           real_prec CV2 = this->cwclass.Invariant_VS_II_at(ig);
           I_CV2= get_bin(CV2, this->s_mins.prop8, N_CV_BIN2,this->s_deltas.prop8, this->bin_accumulate_borders);
#elif defined _USE_S2DELTA_
           real_prec CV2 = this->cwclass.S2DELTA_at(ig);         // s²ð
           I_CV2= get_bin(CV2, this->s_mins.prop8, N_CV_BIN2, s_deltas.prop8,this->bin_accumulate_borders);
#elif defined (_USE_INVARIANT_PWEB_II_)
           real_prec CV2 = this->cwclass.Invariant_TF_II_at(ig); // When using PWEB, the iarrays contaning th nvariants of the tidal field are used to allocate the invariats of the Pfield
           I_CV2= get_bin(CV2, this->s_mins.prop8, N_CV_BIN2, s_deltas.prop8,this->bin_accumulate_borders);
#endif
           size_t I_CV3=0;
#ifdef _USE_INVARIANT_SHEAR_VFIELD_III_
           real_prec CV3 = this->cwclass.Invariant_VS_III_at(ig);
           I_CV3= get_bin(CV3, this->s_mins.prop9, N_CV_BIN3,this->s_deltas.prop9, this->bin_accumulate_borders);
#elif defined _USE_S3_
           real_prec CV3 = this->cwclass.S3_at(ig);                                   // s³
           I_CV3= get_bin(CV3, this->s_mins.prop9, N_CV_BIN3, s_deltas.prop9,this->bin_accumulate_borders);
#elif defined (_USE_INVARIANT_PWEB_III_)
           real_prec CV3 = this->cwclass.Invariant_TF_III_at(ig); // When using PWEB, the iarrays contaning th nvariants of the tidal field are used to allocate the invariats of the Pfield
           I_CV3= get_bin(CV3, this->s_mins.prop9, N_CV_BIN3, s_deltas.prop9,this->bin_accumulate_borders);
#endif
#ifndef _BIN_ACCUMULATE_
                     ...
                     if(CV1<= this->s_maxs.prop7 && CV1>= this->s_mins.prop7)
                       ...
                       if(CV2<= this->s_maxs.prop8 && CV2>= this->s_mins.prop8)
                         ...
                         if(CV3<= this->s_maxs.prop9 && CV3>= this->s_mins.prop9)
                           ...
                           {
                           std::vector<size_t>idxa={I_X, I_CWT, I_MK, I_CWV,I_VK, I_C1, I_C2, I_C3, I_CV1, I_CV2, I_CV3};
                           std::vector<size_t>dimsa={0,static_cast<size_t>(this->params._n_cwt()), static_cast<size_t>(this->params._n_sknot_massbin()), static_cast<size_t>(this->params._n_cwv()), static_cast<size_t>(this->params._n_vknot_massbin()),N_C_BIN1,N_C_BIN2,N_C_BIN3, N_CV_BIN1, N_CV_BIN2,N_CV_BIN3};    
                           size_t index_dm=index_nd(idxa,dimsa);
                           size_t Index=index_2d(I_Y, index_dm, LENGHT_BIAS_NCOUNT_aux);
#ifdef _USE_OMP_
#pragma omp atomic update
#endif
    ...
    if(this->iteration>=this->params._Iteration_Kernel_average())
    {
        for(size_t i=0;i<this->AVERAGE_BIAS_NCOUNTS.size();++i)
...
/*
   ...
   for(int j=0; j<this->params._NX(); j++) {
        ...
        for(size_t i=0; i<new_nbins_y; i++)
...
#endif
#ifndef _EXTRAPOLATE_VOLUME_
       size_t ncells_used = static_cast<size_t>(get_nobjects(this->BIAS_NCOUNTS));
...
long aux_b_full;
size_t aux_b_empty=0;
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
 for(size_t i=0;i < LENGHT_BIAS_NCOUNT_aux;++i)// loop sobre bines de dark matter
    {
       long aux_a=-10000;
       long aux_b;
       for(int tr_j = 0; tr_j < nbins_y_temp; ++tr_j)
         {
           size_t inde=index_2d(tr_j, i, LENGHT_BIAS_NCOUNT_aux);
...
#endif
   for(size_t i=0;i < this->BIAS_NCOUNTS.size();++i)// loop sobre bines de dark matter
...
#pragma omp parallel for collapse(2)
   for(size_t i=0;i<LENGHT_BIAS_NCOUNT_aux;++i)
     for(int tr_j = 0; tr_j < nbins_y_temp; ++tr_j)
      {
       size_t inde_l=index_2d(tr_j,i,LENGHT_BIAS_NCOUNT_aux);
#pragma omp atomic update
...
#pragma omp atomic update
     ...
     }  // closes if(this->params._iteration_ini()>0 && this->use_iteration_ini) //NOT WORKING YET, mut be deprecated
...
//#define _assign_again_  // with this, thesame pdf is recycled to assign Nh to missing cells once the available refs are over.
 ...
 void BiasMT::get_mock_grid(string property)
 {
   ...
   size_t counter_orphan=0;
   size_t counter_out_of_theta=0;
   size_t counter_orphan_b=0;
   size_t counter_dyn=0;
   size_t Ntracer_dyn=0;
   size_t Ntracer_glob=0;
   time_t start_mock;
   time(&start_mock);
   ...
   vector<size_t> X_Y_hist_dynamical(size_bias_array);
...
#ifdef _new_counter_
   vector<size_t> new_X_Y_hist_dynamical(size_bias_array);
   ...
   size_t lenght_bias=N_CV_BIN1 * N_CV_BIN2* N_CV_BIN3 * N_C_BIN1 * N_C_BIN2* N_C_BIN3 * this->params._n_sknot_massbin() * this->params._n_cwv() * this->params._n_vknot_massbin() * this->params._n_cwt() * nx;
...
#pragma omp parallel for collapse(2)
#endif
   for(size_t i=0; i< lenght_bias ;++i)
       for(size_t j=0; j< ny ; ++j)
         {
           size_t index_h=index_2d(j,i,lenght_bias);
#ifdef _USE_OMP_
#pragma omp atomic update
#endif
...
#endif
         size_t KK = static_cast<size_t>(get_nobjects(this->BIAS_NCOUNTS));
   ...
   vector<size_t>Ncells_density_bin_new(size_bias_array , 0);
   ...
   vector<size_t> NCELLSperDMBIN_now(lenght_bias, 0);
  size_t nt_partial=0;
  size_t nt_partial_b=0;
#ifdef _GET_BiasMT_REALIZATIONS_
#ifdef _FULL_VERBOSE_
   ...
   std::cout<<endl;
    ...
    vector<size_t> pdf_ref(ny+1,0);
    vector<size_t> pdf_temp(ny+1,0);
    int iaux;
...
*/
   vector<size_t>vseeds(NTHREADS,0);
...
#pragma omp for reduction(+:counter_orphan, counter_out_of_theta,Ntracer_dyn,counter_dyn)
     ...
     for(size_t i=0;i< this->params._NGRID();++i)        // First block, meant to construct halo number counts
       {
#ifdef _assign_again_
        if(false==cell_info[i].assigned)
        {
         ...
         I_X  = static_cast<size_t>(((this->params._iMAS_X() == 0  && !this->params._Convert_Density_to_Delta_X()) ? static_cast<int>(this->delta_X[i]) : get_bin(dm,this->s_mins.prop1,this->params._NX(),this->s_deltas.prop1,this->bin_accumulate_borders)));
#endif
         // **********CWT
         size_t I_CWT=0;
#ifdef _USE_CWC_
         I_CWT=static_cast<size_t>(this->cwclass.get_Tclassification(i));
#endif
         // **********MK
         // Get the corresponding bin on Knot-mass. The current cell is in a cluster with a given mass falling in that particular bin
         size_t I_MK=0;
#ifdef _USE_MASS_KNOTS_
         I_MK= static_cast<size_t>((this->cwclass.cwt_used[this->cwclass.get_Tclassification(i)]== static_cast<int>(WebType::Knot) ? this->cwclass.SKNOT_M_info[i]: 0));
#endif
         // **********CW-V
         size_t I_CWV=0;
#ifdef _USE_CWC_V_
         I_CWV=static_cast<int>(this->cwclass.get_Vclassification(i));
#endif
         // **********MV
         size_t I_VK=0;
#ifdef _USE_VEL_KNOTS_V_
         I_VK= (this->cwclass.cwv_used[I_CWV]== static_cast<int>(WebType::Knot) ? this->cwclass.VDISP_KNOT_info[i]: 0);
#endif
        // **********C1
                   ...
                   if(CV2<= this->s_maxs.prop8 && CV2>= this->s_mins.prop8)
                     ...
                     if(CV3<= this->s_maxs.prop9 && CV3>= this->s_mins.prop9)
...
#endif
                       {
                           ...
                           std::vector<size_t>idxa={I_X,I_CWT,I_MK,I_CWV,I_VK,I_C1,I_C2,I_C3,I_CV1,I_CV2,I_CV3};
                           std::vector<size_t>dimsa={0,static_cast<size_t>(this->params._n_cwt()),static_cast<size_t>(this->params._n_sknot_massbin()),static_cast<size_t>(this->params._n_cwv()), static_cast<size_t>(this->params._n_vknot_massbin()),N_C_BIN1,N_C_BIN2,N_C_BIN3,N_CV_BIN1,N_CV_BIN2,N_CV_BIN3};    
                           size_t index_el=index_nd(idxa,dimsa);
                           ...
                           if(aux_h>0) //This applies in the case of generating mocks. This is likely the reason why parallelization in the mock production is not working, for it works for all cells......
                             {
                               ...
                               size_t N_available_positions_in_denbin_cwt_Mk = this->NCELLSperDMBIN[index_el];
                               // Number of used cells for the current bin of DM and CWT
                               size_t N_used_positions_in_denbin_cwt_Mk = NCELLSperDMBIN_now[index_el];
                                   ...
                                   while (false==cell_accepted)
                                     {
                                       ...
                                       real_prec ran = 10.0;
                                       size_t index=static_cast<size_t>(LARGE_NUMBER);
                                       ...
                                       size_t N_available_positions=this->BIAS_NCOUNTS[index];
                                       size_t N_used_positions = Ncells_density_bin_new[index];
...
#pragma omp atomic update
                                           ...
                                           for(int j=0;j< ny ;++j)
                                           {
                                             size_t iin=index_2d(j,index_el, lenght_bias);
                                             size_t AUX=X_Y_hist_dynamical[iin];
...
#endif
#endif
 this->Nobjects=static_cast<size_t>(get_nobjects(this->delta_Y_new));
...
// get the ratio btween the number of objects assigned and the total number of tracers form the reference
  ...
  size_t Nobjects_ref=static_cast<int>(get_nobjects(this->delta_Y));
/*
...
#endif
 for(size_t i=0;i< this->params._NGRID();++i)      //Start loop over the cells to assign Nh following th eglobal pbias
   {
    if(cell_info[i].auxvar>0)
      if(false==cell_info[i].assigned)
       {
         size_t index=cell_info[i].theta_index;
         int Nhalos_orfan;
         real_prec prob=-1.0;
         real_prec ran=0.0;
   ...
   So.message_screen("Second loop done");
     ...
     size_t Ntracer_new=static_cast<int>(get_nobjects(this->delta_Y_new));
//     size_t Ntracer_ref_poiss = static_cast<size_t>(gsl_ran_poisson(gBaseRand,Nobjects_ref));
     size_t Ntracer_ref_poiss = static_cast<size_t>(Nobjects_ref*(1+gsl_ran_gaussian(gBaseRand,0.002)));

     So.message_screen("Setting threshold at ",Ntracer_ref_poiss );
     So.message_screen("Number of reference tracers",Nobjects_ref);
     //do{
//       gsl_ran_shuffle(gBaseRand,&i_nac[0], i_nac.size(),sizeof(size_t));
...
#pragma omp parallel for reduction(+:AUX_p)
    for(size_t i=0; i<X_Y_hist_dynamical.size();++i)
    ...
    for(int iy = 0; iy < ny; ++iy)
    {
      for(size_t i=0;i < this->params._NX();++i)// loop sobre bines de dark matter
       {
         size_t index=index_2d(iy,i,this->params._NX());
         for(int sua = 0; sua < this->params._n_cwt(); ++sua)
           for(int k = 0; k < this->params._n_sknot_massbin() ;  ++k)
            for(int vua = 0; vua < this->params._n_cwv(); ++vua)
              for(int vk = 0; vk < this->params._n_vknot_massbin(); ++vk)
                   ...
                   for(int l3 = 0; l3 < N_C_BIN3; ++l3)
                     for(int lv1 = 0; lv1 < N_CV_BIN1; ++lv1)
                       for(int lv2 = 0; lv2 < N_CV_BIN2; ++lv2)
                         for(int lv3 = 0; lv3 < N_CV_BIN3; ++lv3)
                          {
                            std::vector<size_t>idxa={iy,i,sua,k,vua,vk,l1,l2,l3,lv1,lv2,lv3};
                            std::vector<size_t>dimsa={0,this->params._NX(),this->params._n_cwt(),this->params._n_sknot_massbin(),this->params._n_cwv(), this->params._n_vknot_massbin(),N_C_BIN1,N_C_BIN2,N_C_BIN3,N_CV_BIN1,N_CV_BIN2,N_CV_BIN3};    
                            size_t index_el=index_nd(idxa,dimsa);
  ...
  // ahora normalizamos en bines de dm
    ...
    counter_orphan=0;
     ...
     for(size_t ic=0;ic<i_nac.size(); ++ic)
       {
         size_t i=i_nac[ic];
         size_t index_el= cell_info[i].delta_index;
         double aux_h=0;
         for(int ih=0;ih<ny;++ih)
           aux_h +=  static_cast<double>(new_bias[index_2d(ih,index_el, ny*this->params._NX())]);
         if(aux_h>0)
          {
            size_t N_available_positions_in_denbin_cwt_Mk = new_NCELLSperDMBIN[index_el];
            size_t N_used_positions_in_denbin_cwt_Mk = new_NCELLSperDMBIN_now[index_el];
            bool flag = true;
            if (N_available_positions_in_denbin_cwt_Mk == N_used_positions_in_denbin_cwt_Mk)
              flag=false;
            if(true==flag)
             {
              ...
              while (false==cell_accepted)
               ...
               size_t N_available_positions=new_bias[index];
               size_t N_used_positions = new_Ncells_density_bin[index];
               if(N_available_positions > N_used_positions)
                {
                  this->delta_Y_new[i]=static_cast<real_prec>(halo_prop);
                  cell_accepted=true;
                 ...
                 for(int j=0;j< ny ;++j)
                  {
                    size_t iin=index_2d(j,index_el, ny*NX);
                    size_t AUX=new_bias_dyn[iin];
...
#else
    for(size_t i=0;i<i_nac.size();++i)
      {
        size_t i_cell = i_nac[i]; // original ID of the not assigned cell
    //    size_t index= cell_info[i_cell].theta_index;
        size_t index= cell_info[i_cell].delta_index;// thiis is to use the bias as a function of delta
        int Nhalos_orfan=0;
        real_prec prob=-1.0;
        real_prec ran=0.0;
        while(prob<ran) // This while aims at chosing the right Nh according to the full propability distribution
         {
           Nhalos_orfan= gsl_rng_uniform_int(gBaseRand,ny); //draw number counts in cells
//            size_t Index_t=index_2d(Nhalos_orfan, index, lenght_bias);
           size_t Index_t=index_2d(Nhalos_orfan, index, NX); // thiis is to use the bias as a function of delta
...
// This  brings problems with clustering, although gioves good number of tracers
    ...
    if(i_nac.size()>0) // if we have cells to distribute. When dealing with the DM use for the calibration, this third part is not used for all cells are assigned.
      ...
      do{
          gsl_ran_shuffle(gBaseRand,&i_nac[0], i_nac.size(),sizeof(size_t));
         //Start loop over the cells that are not yeat assigned
          for(size_t i=0;i<i_nac.size();++i) // loop ove rrandomized not assigned cells
            {
            size_t i_cell = i_nac[i]; // original ID of the not assigned cell
            size_t index= cell_info[i_cell].theta_index;
            int Nhalos_orfan=0;
...
#pragma omp atomic
           ...
           for (size_t i=0;i<DELTA_HIST.size();++i)aja<<i<<"\t"<<DELTA_HIST[i]<<endl;
           aja.close();
           DELTA_HIST.clear();
...
#pragma omp parallel for
       ...
       if(this->params._Write_PDF_number_counts()) 	  // Write the PDF of the outcome */
         {
...
#pragma omp atomic
               ...
               for(size_t i=0;i<this->PDF_NC_Y.size();++i)
       ...
       } //closes if(this->Write_PDF_number_counts)
...
#endif
      if(this->params._Convert_Density_to_Delta_Y())// Here I am also assuming that if this is true, we also took the log10(num_in_log+delta)
       {
        for(size_t i=0;i<  this->params._NGRID();++i) //tbc
              {
...
void BiasMT::get_mock_grid_two(string property)
 ...
 int NTHREADS = _NTHREADS_;  // omp_get_max_threads();
  ...
  struct tracers_dfield{
   ...
   vector<size_t>id_cells;//container for the id of the cells in each box
   string fname;
   bool file_exist;
   size_t Ntracer_dyn;
   size_t Nobjects;
   size_t Ntracer_glob;
   size_t counter_dyn;
   size_t counter_orphan;
   size_t mesh_id_counter;
   ...
   if(_DENSITY_==property)
     case_prop=2;
   size_t ny = this->new_nbins_y;
   size_t size_bias_array= ny*lenght_bias;//    this->BIAS_NCOUNTS.size();
   this->BIAS_NCOUNTS.clear();
   this->BIAS_NCOUNTS.shrink_to_fit();
...
#pragma omp parallel for
#endif
  for(size_t i=0;i < lenght_bias;++i)// loop sobre bines de dark matter
     {
        long aux_a=-10000;
        long aux_b;
        for(int tr_j = 0; tr_j < ny; ++tr_j)
          {
            size_t inde=index_2d(tr_j, i, lenght_bias);
            long AUX=this->BIAS_NCOUNTS[inde];
            if(AUX==0)
              aux_b_empty++;
            aux_b=max(aux_a, AUX);
  ...
  for(size_t i=0;i<n_new_dm;++i)
    {
      ...
      for(size_t j=0;j< this->params._NGRID();++j)
...
//        for(size_t i=0;i<delta_ghost.size();++i)delta_ghost[i]*=1.033;
//
        this->Konvolve(delta_ghost, delta_ghost); //Convolve with the new kernel
//        for(size_t i=0;i<this->Kernel.size();++i)cout<<this->Kernel[i]<<endl;
        ...
        alpha_c=1.0;
#endif
        for(size_t i = 0; i <  this->params._NGRID() ;++i )  //TRANSFORM DELTATO LOG10(NUM_IN_LOG + DELTA)
...
// WE need to do this becuase the get_new_min_mass function expects this->delta_X
  ...
  size_t cell_counter=0;
   for(size_t ii=0;ii<(n_new_dm)* this->params._NGRID();++ii)   // loop over (nrefs-1)*NGRID: we randomly choose mesh1 or mesh2, and in each mesh, we pick-up a random cell. NOte that we need to do the CWC of the second field as well and plage the inner with ifs()
    {
...
//              mesh_id=gsl_rng_uniform_int(gBaseRand,n_new_dm);
      ...
      size_t I_CWT=0;
#ifdef _USE_CWC_
       I_CWT=static_cast<size_t>(dm_new[mesh_id].cwclass_field.get_Tclassification(i));
#endif
        // **********MK
         // Get the corresponding bin on Knot-mass. The current cell is in a cluster with a given mass falling in that particular bin
        size_t I_MK=0;
#ifdef _USE_MASS_KNOTS_
        I_MK= (dm_new[mesh_id].cwclass_field.cwt_used[dm_new[mesh_id].cwclass_field.get_Tclassification(i)]== WebType::Knot ? dm_new[mesh_id].cwclass_field.SKNOT_M_info[i]: 0);
//        cout<<I_MK<<" "<<I_CWT<<"   "<<dm<<endl;
        ...
        size_t I_CWV=0;
         ...
         size_t I_VK=0;
#ifdef _USE_VEL_KNOTS_V_
         I_VK= (dm_new[mesh_id].cwclass_field.cwv_used[I_CWV]== WebType::Knot ? dm_new[mesh_id].cwclass_field.VDISP_KNOT_info[i]: 0);
#endif
         // **********C1
         // Get the corresponding bin in the two invariants of the shear of the tidal field
         size_t I_C1=0;
  ...
  for(int il=0;il<n_new_dm;++il)
    {
      ...
      {
       for(size_t i=0;i< this->params._NGRID();++i)this->delta_Y_new[i]=tracers[il].delta_field[i];
...
void BiasMT::assign_tracer_property(bool initial_assignment, string h_property)
{
  ...
  So.message_screen("Number of tracers in reference = ", this->catalogue_ref._NOBJS());
  So.message_screen("Number of tracers in mock      = ", this->catalogue._NOBJS());
#endif


  size_t cumulative_counter=0; // this counts all
  // *********************************************OMP stuff***********************************************************************
  int NTHREADS=_NTHREADS_;
  gsl_rng_env_setup();
  ...
  gsl_rng *rn = gsl_rng_alloc (Tn);
  // ********************************************************************************************************************
  size_t LENGHTdm=this->dm_properties_bins.size();
  ...
  // ********************************************************************************************************************
   size_t counter_masses_0=0;
#ifndef _MULTISCALE_
  vector<size_t>number_in_theta_ref(LENGHTdm,0);
  // container to track the number of available masses in each theta-bin
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
  for(size_t i=0;i<LENGHTdm; ++i)
    number_in_theta_ref[i]=this->dm_properties_bins[i].tracer_properties.size();
#ifdef _SET_THRESHOLD_FOR_PROP_RANDOM_TRACERS_
  size_t LENGHTdm_for_randoms=this->dm_properties_for_randoms_bins.size(); //In principe this number is equal to this->dm_properties_bins.size();
  vector<size_t>number_in_theta_ref_for_randoms(LENGHTdm_for_randoms,0);
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
  for(size_t i=0;i<LENGHTdm_for_randoms; ++i)
  ...
  if(true==initial_assignment)
    nbins_mf=this->htools_ref->vmax_function.size();
  else 
   {
     if(h_property==_MASS_)
      nbins_mf=this->htools_ref->mass_function.size();
     else if (h_property==_RS_ )
      nbins_mf=this->htools_ref->rs_function.size();
     else if ( h_property == _CONCENTRATION_)
      nbins_mf=this->htools_ref->cvir_function.size();
     else if (h_property==_SPIN_ || h_property==_SPIN_BULLOCK_)
      nbins_mf=this->htools_ref->s_function.size();
}
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
  if(true==initial_assignment)
      nbins_mf=this->htools_ref->mass_function.size();
  else
  {
...
#endif
   this->htools->define_property_bins();
   this->mfunc.resize(nbins_mf,0);
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
     if(true==initial_assignment)
      {
        this->htools_ref->_params().set_i_vmax_g(POSITIVE_INT); //any positive number, allows to define bins in vmax
        this->htools_ref->define_property_bins();
        this->prop_min=this->htools_ref->VMAXBin[0];
        this->prop_max=this->htools_ref->VMAXBin[nbins_mf-1];
     }
     else if(false==initial_assignment)
      {
        if(h_property==_MASS_)
         {
           this->catalogue_ref._params().set_i_mass_g(POSITIVE_INT); //any positive number, allows to define bins in mass
           this->htools_ref->define_property_bins();
           this->prop_min=this->htools_ref->MBin[0];
           this->prop_max=this->htools_ref->MBin[nbins_mf-1];
         }
     }
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
   if(true==initial_assignment)
    {
      this->catalogue_ref._params().set_i_mass_g(POSITIVE_INT); //any positive number, allows to define bins in mass
      this->catalogue_ref._params().set_i_vmax_g(NEGATIVE_INT); //any positive number, allows to define bins in mass
      this->htools_ref->define_property_bins();
      this->prop_min=this->htools_ref->MBin[0];
      this->prop_max=this->htools_ref->MBin[nbins_mf-1];
   }
   else if(false==initial_assignment)
   {
      if(h_property==_VMAX_)
       {
         this->catalogue_ref._params().set_i_vmax_g(POSITIVE_INT); //any positive number, allows to define bins in vmax
         this->catalogue_ref._params().set_i_mass_g(NEGATIVE_INT); //any positive number, allows to define bins in mass
         this->htools_ref->define_property_bins();
         this->prop_min=pow(10,this->htools_ref->VMAXBin[0]);
         this->prop_max=pow(10,this->htools_ref->VMAXBin[nbins_mf-1]);
       }
   }
#endif
   if(false==initial_assignment)
   {
     ...
     if(h_property==_RS_ )
       {
          this->catalogue_ref._params().set_i_rs_g(POSITIVE_INT); //any positive number, allows to define bins in rs
          this->htools_ref->define_property_bins();
          this->prop_min=this->htools_ref->RSBin[0];
          this->prop_max=this->htools_ref->RSBin[nbins_mf-1];
        }
#endif
#ifdef _USE_CONCENTRATION_AS_DERIVED_OBSERVABLE_
     if( h_property == _CONCENTRATION_)
       {
          this->htools_ref->define_property_bins();
          this->prop_min=this->htools_ref->CONCENTRATIONBin[0];
          this->prop_max=this->htools_ref->CONCENTRATIONBin[nbins_mf-1];
       }
#endif
#ifdef _USE_SPIN_AS_DERIVED_OBSERVABLE_
      else if (h_property==_SPIN_ || h_property==_SPIN_BULLOCK_)
        {
          ...
          { 
            this->catalogue_ref._params().set_i_spin_g(POSITIVE_INT);//any positive number, allows to define bins in spin
            this->catalogue_ref._params().set_i_spin_bullock_g (NEGATIVE_INT);//any positive number, allows to define bins in spin
          }
          if (h_property==_SPIN_BULLOCK_)
          {
            this->catalogue_ref._params().set_i_spin_g(NEGATIVE_INT);//any positive number, allows to define bins in spin
            this->catalogue_ref._params().set_i_spin_bullock_g(POSITIVE_INT);//any positive number, allows to define bins in spin
          }
          this->prop_min=this->htools_ref->SPINBin[0];
          this->htools_ref->define_property_bins();
          this->prop_max=this->htools_ref->SPINBin[nbins_mf-1];
//          this->prop_min= this->params._SPINmin();//  this->htools_ref->SPINBin[0];
//          this->prop_max= this->params._SPINmax(); // this->htools_ref->SPINBin[nbins_mf-1];
...
#endif
          for(size_t i=0; i< nbins_mf; ++i)
            this->mfunc[i]=this->htools_ref->vmax_function[i]*delta_prop_aux[i];
        }
        else{
         if(h_property==_MASS_)
#ifdef _USE_OMP_
...
#endif
          for(size_t i=0; i< nbins_mf; ++i)
           this->mfunc[i]=this->htools_ref->mass_function[i]*delta_prop_aux[i];
        }
#ifdef _USE_GNUPLOT_ABUNDANCE_V_PLOT_
   xy_pts_ref_v.clear();xy_pts_ref_v.shrink_to_fit();
   for(size_t i=0; i<this->htools_ref->vmax_function.size(); ++i)
     xy_pts_ref_v.push_back(std::make_pair(this->htools_ref->VMAXBin[i]+(i+0.5)*(this->htools_ref->VMAXBin[i]-this->htools_ref->VMAXBin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools_ref->vmax_function[i])));
#endif
...
#pragma omp parallel for
   ...
   for(size_t i=0; i<this->htools_ref->mass_function.size(); ++i)
       xy_pts_ref_m.push_back(std::make_pair(this->htools_ref->MBmin[i]+(i+0.5)*(this->htools_ref->MBmax[i]-this->htools_ref->MBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools_ref->mass_function[i])));
#endif
...
#pragma omp parallel for
         ...
         for(size_t i=0; i< nbins_mf; ++i)
           {
            this->mfunc[i]=this->htools_ref->rs_function[i]*delta_prop_aux[i];
           }
         }
       if(h_property == _CONCENTRATION_)
            {
...
#pragma omp parallel for
         ...
         for(size_t i=0; i< nbins_mf; ++i)
           {
            this->mfunc[i]=this->htools_ref->cvir_function[i]*delta_prop_aux[i];
           }
         }
...
#endif
       for(size_t i=0; i< nbins_mf; ++i)
          this->mfunc[i]=this->htools_ref->s_function[i]*delta_prop_aux[i];
        }
      }
...
#pragma omp parallel for
    ...
    if(true==initial_assignment) // for true, we initially assign the primary property
      {
        spline    = gsl_spline_alloc (gsl_interp_linear, this->htools_ref->vmax_function.size());
        gsl_spline_init (spline, &(this->htools_ref->VMAXBin[0]), &(this->mfunc[0]), this->htools_ref->vmax_function.size());
        ...
        if(h_property==_MASS_) // if false, we assign the secondary
         {
           spline    = gsl_spline_alloc (gsl_interp_linear, this->htools_ref->mass_function.size());
           gsl_spline_init (spline, &(this->htools_ref->MBin[0]), &(this->mfunc[0]), this->htools_ref->mass_function.size());
    ...
    if(false==initial_assignment)
    {
      if(h_property==_RS_ )
       {
        spline    = gsl_spline_alloc (gsl_interp_linear, this->htools_ref->rs_function.size());
        gsl_spline_init (spline, &(this->htools_ref->RSBin[0]), &(this->mfunc[0]), this->htools_ref->rs_function.size());
        lm_min=log10(this->params._RSmin());
        lm_max=log10(this->params._RSmax());
       }
      if( h_property == _CONCENTRATION_)
       {
        spline    = gsl_spline_alloc (gsl_interp_linear, this->htools_ref->cvir_function.size());
        gsl_spline_init (spline, &(this->htools_ref->CONCENTRATIONBin[0]), &(this->mfunc[0]), this->htools_ref->cvir_function.size());
        lm_min=log10(this->params._CONCENTRATIONmin());
        lm_max=log10(this->params._CONCENTRATIONmax());
       }
      else if(h_property==_SPIN_ || h_property==_SPIN_BULLOCK_ )
       {
        spline    = gsl_spline_alloc (gsl_interp_linear, this->htools_ref->s_function.size());
        gsl_spline_init (spline, &(this->htools_ref->SPINBin[0]), &(this->mfunc[0]), this->htools_ref->s_function.size());
        lm_min=log10(this->params._SPINmin());
        lm_max=log10(this->params._SPINmax());
       }
    }
  // ************************************************RENAME SOME BIN PROPPERTIES*****************************************
  size_t N_a=N_C_BIN1; //THis will be used by std_sep_in cell as well
  if(true==initial_assignment) // This applies for the assignment of Vmax with the different techniques
    {
      ...
      this->catalogue_ref.get_density_field_grid(_MASS_, MOCK_MASS_FIELD); // ojo que estoy cargando la ref, no el mock:esto debería ser el mock con la masa total dada por la calibración
#else
  ...
  if(true==initial_assignment)
    {
#ifdef _USE_NUMBER_OF_NEIGHBOURS_
      this->catalogue.get_neighbour_tracers(this->ncells_info);
      N_b = N_NEIGHBOURS_MAX;
#elif defined (_USE_MIN_DISTANCE_TO_NEIGHBOURS_) || defined (_USE_LOCAL_CLUSTERING_)
      N_b=N_BINS_MIN_DIST_TO_NEI;
#elif defined _USE_MIN_SEPARATIONS_IN_CELLS_
     N_b= N_BINS_MIN_SEP_IN_CELLS;
     this->catalogue.get_stats_separation_in_cell();
     delta_min_sep = (MAX_SEP_IN_CELLS-MIN_SEP_IN_CELLS)/static_cast<real_prec>(N_BINS_MIN_SEP_IN_CELLS);
#elif defined _USE_MEAN_SEPARATIONS_IN_CELLS_
     N_b= N_BINS_MEAN_SEP_IN_CELLS;
     this->catalogue.get_stats_separation_in_cell(); // this also works to get the mean and stedev
...
//  The following loop aims at collecting the values of Vmax in theta bins.
  ...
  size_t count_dm=0;
#ifdef _SET_THRESHOLD_FOR_PROP_RANDOM_TRACERS_  //this is only done for vmax so far, since we shall  use that property as main property
  size_t count_dm_full=0;
  size_t count_ran_full=0;
#endif
  // Note:
  ...
  vector<size_t>vseeds(NTHREADS,0);
  for(size_t i=0;i<vseeds.size();++i)
   vseeds[i]=35+static_cast<size_t>(i+14)*56045;
#ifdef _MULTISCALE_
#ifdef _FULL_VERBOSE_
...
#pragma omp parallel private (jthread, Trn, randa)// this is causing problem
  {
    ...
    // Or, if we are to assign coords to a new reference, we proceed to compute properties from that new target reference
      ...
      if(this->params._Get_tracer_bias())
      {
        ...
        Psb.object_by_object_bias(this->catalogue,DM_DEN_FIELD);
        DM_DEN_FIELD.clear();DM_DEN_FIELD.shrink_to_fit();
      }
#ifdef _USE_MACH_NUMBER_
      this->catalogue.get_local_mach_number_chuncks(this->params._Scale_mach_number());
#endif
#ifdef _WRITE_BIAS_
      ...
      this->So.message_screen("Writing bias in  file ", out_bias);
      for(size_t i=0;i<this->catalogue._NOBJS();++i)
        bout<<this->catalogue.bias[i]<<"\t"<<this->catalogue.mach_number[i]<<"\t"<<this->catalogue.local_overdensity[i]<<endl;
      bout.close();
#endif
#endif
#ifdef _READ_BIAS_
...
//      string out_bias=params._Output_directory()+"individual_bias.txt"; // if assining to the same reference
      ...
      for(size_t i=0;i<this->catalogue._NOBJS();++i)
        bout>>this->catalogue.bias[i]>>this->catalogue.mach_number[i]>>this->catalogue.local_overdensity[i];
      bout.close();
#endif 
#ifdef _USE_TIDAL_ANISOTROPY_SEC_PROP_
      vector<real_prec>tidal(this->params._NGRID(),0);
...
#pragma omp parallel for
      ...
      for(size_t i=0;i<this->params._NGRID() ;++i)
        tidal[i]=tidal_anisotropy(this->cwclass.lambda1[i], this->cwclass.lambda2[i], this->cwclass.lambda3[i]);
      this->catalogue.get_tracer_tidal_anisotropy(tidal);
      So.message_screen("\tMin TA :",this->catalogue._min_tidal_anisotropy());
      So.message_screen("\tMax TA :",this->catalogue._max_tidal_anisotropy());
      tidal.clear();tidal.shrink_to_fit();
#endif
...
#else
    for(size_t id=0; id <  this->params._NGRID();++id)
#endif
#else   //else for ifdef _MULTISCALE_
#ifdef _USE_OMP_TEST_
#pragma omp for reduction(+:counter_fmf, count_dm)
#endif
    for(size_t ig=0; ig < this->catalogue._NOBJS(); ++ig) // Loop enabled if no multiscale is used. WE assign directly here
#endif    // endif for else to def _MULTISCALE_
        {
       // Get the cell ID where the mock tracer is located
         size_t id=this->catalogue.GridID_at(ig);
         size_t I_X=0;
       // Get the bin in the delta dark matter (or log 1+delta) in each cell
#ifdef _USE_DM_DENSITY_AT_HALO_POSITION_
        real_prec xdm=linInterpol(this->params._Nft(),this->params._Lbox(),this->params._d1(),this->catalogue.coord1_at(ig),this->catalogue.coord2_at(ig),this->catalogue.coord3_at(ig),this->delta_X);
        ...
        I_X  = get_bin(xdm,this->s_mins.prop1,this->params._NX(),this->s_deltas.prop1,this->bin_accumulate_borders);
         ...
         size_t I_CWT=0;
#if defined _USE_CWC_ || defined (_USE_MASS_KNOTS_)
         I_CWT=static_cast<size_t>(this->cwclass.get_Tclassification(id));
#if defined (_MULTISCALE_) || defined (_SET_THRESHOLD_FOR_PROP_RANDOM_TRACERS_)
...
// Get the corresponding bin on Knot-mass. The current cell is in a cluster with a given mass falling in that particular bin
         size_t I_MK=0;
#ifdef _USE_MASS_KNOTS_
         I_MK= (this->cwclass.cwt_used[I_CWT]== WebType::Knot ? this->cwclass.SKNOT_M_info[id]: 0);
#endif
         size_t I_CWV=0;
#ifdef _USE_CWC_V_
         I_CWV=static_cast<size_t>(this->cwclass.get_Vclassification(id));
#endif
       // Get the corresponding bin on Knot-mass. The current cell is in a cluster with a given mass falling in that particular bin
        size_t I_VK=0;
#ifdef _USE_VEL_KNOTS_V_
         I_VK= (this->cwclass.cwt_used[I_CWV]== static_cast<size_t>(WebType::Knot) ? this->cwclass.VDISP_KNOT_info[id]: 0);
#endif
       // Get the corresponding bin in the two invariants of the shear of the tidal field
        size_t I_C1=0;
#ifdef _USE_TOTAL_MASS_IN_CELL_
       ...
       I_C1= get_bin(C1, this->s_mins.prop4, N_C_BIN1, s_deltas.prop4,this->bin_accumulate_borders);
        ...
        if(true==initial_assignment)
         {
           real_prec sep_std = this->catalogue.stdv_separation_in_cell[id];
           I_C1 = get_bin(sep_std, MIN_STDV_SEP_IN_CELLS, N_a, delta_stdv_sep , this->bin_accumulate_borders);
        }
#endif
       size_t I_C2=0;
       ...
       if(true==initial_assignment)
         {
           I_C2=this->catalogue.Number_of_neighbours_at(ig);
           if(I_C2>=N_NEIGHBOURS_MAX)
              I_C2=N_NEIGHBOURS_MAX-1;
          }
#elif defined _USE_INVARIANT_TIDAL_FIELD_III_
          ...
          if(true==initial_assignment)
           {
             real_prec min_sep = this->catalogue.min_separation_in_cell[id];
             I_C2 = get_bin(min_sep, MIN_SEP_IN_CELLS, N_b, delta_min_sep , this->bin_accumulate_borders);
           }
#elif defined _USE_MEAN_SEPARATIONS_IN_CELLS_
          if(true==initial_assignment)
           {
             real_prec min_sep = this->catalogue.mean_separation_in_cell[id];
...
#ifdef  _USE_ANISOTROPY_AT_TRACER_POSITION_
          real_prec C3 = linInterpol(this->params._Nft(),this->params._Lbox(),this->params._d1(),this->catalogue.coord1_at(ig),this->catalogue.coord2_at(ig),this->catalogue.coord3_at(ig),this->cwclass.Tidal_Anisotropy);
          I_C3= get_bin(C3, this->s_mins.prop6, N_C_BIN3, s_deltas.prop6,this->bin_accumulate_borders);
#else
          real_prec C3 = this->cwclass.Tidal_Anisotropy[id];
          I_C3= get_bin(C3, this->s_mins.prop6, N_C_BIN3, s_deltas.prop6,this->bin_accumulate_borders);
          ...
          I_C3= get_bin(C3, this->s_mins.prop6, N_C_BIN3, s_deltas.prop6,this->bin_accumulate_borders);
#elif defined _USE_MACH_NUMBER_
          I_C3= get_bin(this->catalogue.mach_number_at(ig), this->htools_ref->_minmach(), N_BINS_MACH, (this->htools_ref->_maxmach()-this->htools_ref->_minmach())/static_cast<real_prec>(N_BINS_MACH),this->bin_accumulate_borders);
#endif
          size_t I_CV1=0;
#ifdef _USE_TRACERS_IN_CELLS_
          if(true==initial_assignment)
...
//#ifdef _ASSIGN_MASS_POST_
         ...
         {
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
             I_CV2=get_bin(log10(this->catalogue.vmax_at(ig)), log10(this->params._VMAXmin()),N_x,log10(this->params._VMAXmax()/this->params._VMAXmin())/static_cast<double>(N_x), true);
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
             I_CV2=get_bin(log10(this->catalogue.mass_at(ig)), this->params._LOGMASSmin(),N_x,(this->params._LOGMASSmax()-this->params._LOGMASSmin())/static_cast<double>(N_x), true);
#endif
         }
         else{
...
//      real_prec CV2 = CROSS_CORR[id];         // cross correlation
      real_prec CV2  =linInterpol(this->params._Nft(),this->params._Lbox(), this->params._d1(),this->catalogue.coord1_at(ig),this->catalogue.coord2_at(ig),this->catalogue.coord3_at(ig),CROSS_CORR);
      I_CV2= get_bin(CV2, this->s_mins.prop8, N_CV_BIN2, s_deltas.prop8,this->bin_accumulate_borders);
      //#endif // endif for _ASSIGN_MASS_POST_
#elif defined _USE_LOCAL_OVERDENSITY_
      I_CV2= get_bin(this->catalogue.local_overdensity_at(ig), this->htools_ref->_minlocal_overdensity(), N_BINS_LO,  (this->htools_ref->_maxlocal_overdensity()-this->htools_ref->_minlocal_overdensity())/static_cast<real_prec>(N_BINS_BIAS) ,this->bin_accumulate_borders);
#endif
         }
        size_t I_CV3=0;
#ifdef _USE_INVARIANT_SHEAR_VFIELD_III_
        ...
        I_CV3 = get_bin(this->catalogue.Halo_at(ig).bias, this->htools_ref->min_bias(),N_BINS_BIAS,(this->htools_ref->max_bias()-this->htools_ref->min_bias())/static_cast<real_prec>(N_BINS_BIAS),this->bin_accumulate_borders);
#endif
#ifndef _BIN_ACCUMULATE_
                  ...
                  if(CV2<= this->s_maxs.prop8 && CV2>= this->s_mins.prop8)
                   ...
                   if(CV3<= this->s_maxs.prop9 && CV3>= this->s_mins.prop9)
#endif
                     {
                       ...
                       std::vector<size_t>idxa={I_X,I_CWT,I_MK,I_CWV,I_VK,I_C1,I_C2,I_C3,I_CV1,I_CV2,I_CV3};
                       std::vector<size_t>dimsa={0,static_cast<size_t>(this->params._n_cwt()), static_cast<size_t>(this->params._n_sknot_massbin()),static_cast<size_t>(this->params._n_cwv()), static_cast<size_t>(this->params._n_vknot_massbin()), N_a, N_b, N_c,N_v,N_x,N_bias};    
                       size_t index_bins=index_nd(idxa,dimsa);
...
#endif
#ifdef _SET_THRESHOLD_FOR_PROP_RANDOM_TRACERS_  //this is only done for vmax so far, since we shall  use that property as main property
                       if(this->catalogue.identity_at(ig) >0 )  // Select tracer associated to a DM particle
                                           {
                           count_dm_full++;
#endif
#ifndef _MULTISCALE_
                                   ...
                                   size_t N_tracers_in_theta_bin = this->dm_properties_bins[index_bins].tracer_properties.size();
                                   size_t N_tracers_left =  number_in_theta_ref[index_bins]; // this container is updated below
                                    ...
                                    if(N_tracers_left>0) //when we use the ref as a mock, this condition will be always satisfied by construction
                                     {
...
#else
                                      size_t i_halo_index= gsl_rng_uniform_int(rn,N_tracers_in_theta_bin);
#endif
                                      bool used_tracer = this->dm_properties_bins[index_bins].used_property[i_halo_index]; //true or false if the mass was already chosen or not
                                       if(false == used_tracer)// if the property of the tracer (e.g., vmax, mass) has not been used before, then assign that property to the current particle i
                                        {
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                                           this->catalogue.vmax_at(ig)= this->dm_properties_bins[index_bins].tracer_properties[i_halo_index]; //prop_to_assign;
#ifdef  _USE_HYBRID_ASSIGNMENT_NEW_              //fill the structure with the new assigment in order to make the swap procedure below
                                           this->dm_properties_bins_mock[index_bins].tracer_properties.push_back(this->catalogue.vmax_at(ig));
                                           // keep track of the id  of the tracers
                                           this->dm_properties_bins_mock[index_bins].GalID_bin_properties.push_back(ig);
#endif
#ifdef _ASSIGN_MASS_LINKED_TO_V_FROM_REF_
                                          this->catalogue.mass_at(ig)= this->dm_properties_bins[index_bins].tracer_properties_secondary[i_halo_index]; //prop_to_assign;
                                          this->catalogue.observed_at(ig)_secondary=true; // Mark this tarcer as already assigned a property. This will be used below.
#endif // end
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
                                          this->catalogue.mass_at(ig)= this->dm_properties_bins[index_bins].tracer_properties[i_halo_index];  //prop_to_assign;
#ifdef  _USE_HYBRID_ASSIGNMENT_NEW_              //fill the structure with the new assigment in order to make the swap procedure below
                                           this->dm_properties_bins_mock[index_bins].tracer_properties.push_back(this->catalogue.mass_at(ig));
                                           // keep track of the id  of the tracers
                                           this->dm_properties_bins_mock[index_bins].GalID_bin_properties.push_back(ig);
#endif // end  _USE_HYBRID_ASSIGNMENT_NEW_
#endif // end _USE_MASS_AS_PRIMARY_OBSERVABLE_

                                           // this->catalogue.GridID_at(ig)=index_bins; // Mark this tarcer as already assigned a property. This will be used below.
                                          this->catalogue.observed_at(ig)=true; // Mark this tarcer as already assigned a property. This will be used below.
#ifndef _USE_STATISTICAL_ASSIGNMENT_
...
#ifdef _USE_STATISTICAL_ASSIGNMENT_
                              real_prec aux_h=0;//
                              for(size_t iy=0;iy< this->params._NPROPbins_bam();++iy) // we only need to go up to the threshold mass
...
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                                   real_prec fraction_p= xr*log10(this->htools_ref->VMAXBmax[i_halo_index]/this->htools_ref->VMAXBmin[i_halo_index]);
                                   real_prec lp_halo = log10(this->htools_ref->VMAXBmin[i_halo_index])+fraction_p ;
                                   this->catalogue.vmax_at(ig) = pow(10,lp_halo);
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
                                   real_prec fraction_p= xr*log10(this->htools_ref->MBmax[i_halo_index]/this->htools_ref->MBmin[i_halo_index]);
                                   real_prec lp_halo = log10(this->htools_ref->MBmin[i_halo_index])+fraction_p ;
...
//                  So.message_screen_flush("Number of tracers with assigned property = ",static_cast<int>(counter_fmf));
    ...
    for(size_t i = 0; i < this->dm_properties_bins.size(); ++i)
      for(size_t j = 0; j < this->dm_properties_bins[i].used_property.size(); ++j)
        if(!this->dm_properties_bins[i].used_property[j])
          {
...
#endif
    for(size_t ig=0; ig < this->catalogue._NOBJS(); ++ig)
      if( this->catalogue.identity_at(ig)>0  && !this->catalogue.observed_at(ig))// proceed if this object is random
        {
          bool flag=false;
          while(false==flag){
            ...
            if(false==left_overs_properties_used[jj])
              {
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
                this->catalogue.mass_at(ig)=left_overs_properties[jj] ;
#elif defined _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                this->catalogue.vmax_at(ig)= left_overs_properties[jj] ;
#endif
                this->catalogue.observed_at(ig)=true;
...
#ifdef _FULL_VERBOSE_
    So.message_screen("Assigning property to random tracers. Expected ", this->catalogue.Ntracers_ran);
#endif
    size_t count_ran=0;
    ...
    for(size_t ig=0; ig < this->catalogue._NOBJS(); ++ig)
      {
        if( this->catalogue.identity_at(ig)<0)// proceed if this object is random
          {
            count_ran_full++;
            size_t id=this->catalogue.GridID_at(ig);
            size_t index_bins = cell_info_tr[id].Theta_bin;
            size_t N_tracers_in_theta_bin_r = this->dm_properties_for_randoms_bins[index_bins].tracer_properties.size();
            size_t N_tracers_left_r =  number_in_theta_ref_for_randoms[index_bins]; // this container is updated below
           // If we have available properties in the theta-bin
        //    std::cout<<ig<<" "<<this->catalogue.identity_at(ig)<<"   "<<this->catalogue.observed_at(ig)<<"   "<<N_tracers_left_r<<endl;
                ...
                while(flag == false) // este while obliga a elegir una masa -no elegida antes- de entre las disponibles
                 {
                   ...
                   if(false == used_tracer)// if the property of the tracer (e.g., vmax, mass) has not been used before, then assign that property to the current particle i
                    {
                      this->catalogue.vmax_at(ig)= this->dm_properties_for_randoms_bins[index_bins].tracer_properties[i_halo_index_r]; //prop_to_assign;
                      this->catalogue.observed_at(ig) = true; // Mark this tarcer as already assigned a property. This will be used below.
...
#endif
        // Left overs:
    for(size_t i = 0; i < this->dm_properties_for_randoms_bins.size(); ++i)
      for(size_t j = 0; j < this->dm_properties_for_randoms_bins[i].used_property.size(); ++j)
        if(!this->dm_properties_for_randoms_bins[i].used_property[j])
          {
...
// See documentation for multiscale:
   ...
   size_t ep_ncounts=this->catalogue_ref._NOBJS()-this->catalogue._NOBJS();
   if(this->catalogue_ref._NOBJS()-this->catalogue._NOBJS()<0)
       ep_ncounts*=-1.0;
...
//           ntr/=static_cast<real_prec>(this->params._Number_of_references());
      ...
      this->params.set_Ntracers_MultiLevels(i,static_cast<size_t>(floor(ntr)));
    }
#else   // if not, read the numbers from the last load reference.
   for(size_t i=0;i<this->params._Number_of_MultiLevels();++i)
          this->params.set_Ntracers_MultiLevels(i,this->catalogue_ref.params.get_Ntracers_MultiLevels(i));
#endif
    size_t N_mlevels=0;
     for(size_t i=0;i<this->params._Number_of_MultiLevels();++i)
        N_mlevels+=this->params.get_Ntracers_MultiLevels(i);
     N_props_particle_level=this->catalogue_ref._NOBJS()-ep_ncounts-N_mlevels;
#ifdef _FULL_VERBOSE_
   So.message_screen("Total to be assigned from grid-approach = ", N_mlevels);
   std::cout<<endl;
#endif
     if(this->catalogue_ref._NOBJS()<N_mlevels)
      {
       So.message_warning("Error in line", __LINE__);
       throw std::invalid_argument( "size_t defined variable was assigned a negative value" );
...
#endif
   for(size_t ig=0;ig < this->catalogue._NOBJS(); ++ig)
       this->catalogue.observed_at(ig)=false;  // this will be used below, so let keep it inzialized like that
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
   for(size_t ig=0;ig < this->catalogue._NOBJS(); ++ig)
      this->catalogue.multi_scale_level_at(ig)=-1;  // this will be used below, so let keep it inzialized like that
...
/*
   ...
   for(size_t it=0; it < LENGHTdm; ++it)  // We now sort all properties in the different theta bins from top-to-bottom
     {
...
#pragma omp parallel for
#endif
   for(size_t ig=0;ig < this->catalogue._NOBJS(); ++ig)// Loop over the mock tracers to which properties are to be assigned
     {
       size_t id=this->catalogue.GridID_at(ig);
       this->catalogue.Halo_at(ig).galThetaID=cell_info_tr[id].Theta_bin;
#ifdef _USE_OMP_
#pragma omp atomic
#endif
...
#endif
   for(size_t it=0; it < LENGHTdm; ++it)
     {
       size_t ll=this->dm_properties_bins[it].tracer_properties.size();
       if(ll>0)
        {
         size_t llp=0;
         ...
         vector<size_t>aux_l;
          ...
          if(aux_l.size()>0)
            {
              gsl_ran_shuffle(rn,&aux_l[0], aux_l.size() ,sizeof(size_t));  // randomize within the level 0
              for(size_t ipnew=0; ipnew< aux_l.size(); ++ipnew )  // reassign to dm_prop at this theta bin up tp the size of aux_level4
                {
          ...
          if(this->params._Number_of_MultiLevels()>1)
            {
              for(int iml=1;iml<this->params._Number_of_MultiLevels();iml++)// Now randomize for the other levels if there are
              {
                ...
                if(aux_l.size()>0)
                 {
                  gsl_ran_shuffle(rn,&aux_l[0], aux_l.size() ,sizeof(size_t));  // randomize within the level 4
...
//---------------------------------------------------------------------------------------------------------------------------------------
 ...
 size_t cumulative_counter=0;
 vector<size_t>count_aux_cell(LENGHTdm,0);  // This must be outside of the do-while loop
 cout<<endl;
 for(int iml=0; iml<this->params._Number_of_MultiLevels(); ++iml)   // Loop over the multiscale levels >1, split to avoid if's inside loops
  {
    ...
    size_t Ntracers_ml=this->params.get_Ntracers_MultiLevels(iml);
    size_t NFT_ml=this->params.get_Nft_MultiLevels(iml);
    size_t NGRID_MS=NFT_ml*NFT_ml*NFT_ml;// Grid size for the current level
    size_t counter_av=0;
    size_t counter_masses_a_tolerance=static_cast<size_t>(floor(Ntracers_ml*static_cast<real_prec>(this->params.get_Props_Tolerance_MultiLevels(iml))));
#ifdef _USE_OMP_
#pragma omp parallel for reduction(+:counter_av)
    ...
    size_t counter_multiscale=0;
    do
     {
       size_t N_cells=NGRID_MS;// cells_id_still_to_assign.size();
       size_t counter_internal=0;
       size_t id_ini;
#ifdef _USE_OMP_LX_
#pragma omp parallel for default(shared) private(id_ini) reduction(+:counter_internal)
       ...
       for(id_ini=0; id_ini< N_cells; ++id_ini) // loop over cells with resolution according to current level
        {
          size_t N_cells_in_cells= cell_info_cell[id_ini].gal_index.size();  // Get the Number of high-res cells (i.e, the original ones) in one low-res cell
          ...
          size_t id=cell_info_cell[id_ini].gal_index[index_cell_in_low_res_cell];    //this chosen id will be used below to assign mass
          if(number_in_cells_aux[id]>0)// If there are tracers in the spatial cell id with no property assigned (this is reduced by one after every assignment), go
            {
             size_t index_theta_bins = cell_info_tr[id].Theta_bin;      //Get the theta-bin in which this high-res cell has been classified. This has been calculated in the step PRop<Prop above in this function
             size_t i_mass_halo_label=count_aux_cell[index_theta_bins]; //Get the -updated in the previous label- the stage at which, in the sorted list of prop, the assignemt is located
             size_t N_mocks_in_cell=cell_info_tr[id].gal_index.size();  //Number of mock tracers in the cell id
             size_t jk= gsl_rng_uniform_int(rn,N_mocks_in_cell);        //Choose a random integer in  [0,N_mocks_in_cell).
             size_t ig=cell_info_tr[id].gal_index[jk];                  //Get the Galaxy ID of the mock tracer chosen from the cell id in the position jk of tracers without mass
             if(!this->catalogue.observed_at(ig))                 //If this mock tracer has not yet a property, proceed
               {
...
#endif
                       this->catalogue.mass_at(ig)=assigned_property ;
#ifdef _ASSIGN_MASS_POST_
                     else
                       this->catalogue.vmax_at(ig)=assigned_property ;
#endif
#elif defined _USE_VMAX_AS_PRIMARY_OBSERVABLE_
...
#pragma omp atomic
                      ...
                      this->catalogue.set_gal_cwt(cell_info_tr[id].cwt, ig);
                      this->catalogue.set_local_dm(cell_info_tr[id].local_dm,ig);
                      this->catalogue.set_observed(1,ig);
                      this->catalogue.set_multi_scale_level_at(iml+1, ig);
         ...
         } // closes if(!this->catalogue.observed_at(ig))
   ...
   //      break;
    ...
    size_t counter_masses_bb=0; // Number of tracers without assigned property below the current minimum of this level
#ifdef _USE_OMP_
#pragma omp parallel for reduction(+:counter_masses_bb)
#endif
    for(size_t i = 0; i < this->dm_properties_bins.size(); ++i)
        for(size_t j = 0; j < this->dm_properties_bins[i].used_property.size(); ++j)
            if(!this->dm_properties_bins[i].used_property[j] && (this->dm_properties_bins[i].tracer_properties[j]<=min_prop_level))//count what is below the minimum of the current level
              counter_masses_bb++;
#ifdef _FULL_VERBOSE_
    So.message_screen(". Partial number of properties assigned =",cumulative_counter);
    So.message_screen("Pending for assignment =",this->catalogue.coord1.size()-cumulative_counter);
...
#endif
   size_t counter_from_ref=0; // Number of tracers read from the reference
   vector<size_t>id_to_assign;
   size_t counter_masses_b=0; // Number of tracers without assigned property
#ifdef _USE_OMP_
#pragma omp parallel for reduction(+:counter_masses_b)
#endif
   for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
     if(this->catalogue.observed_at(ig)==false)
       counter_masses_b++;
#ifdef _FULL_VERBOSE_
   So.message_screen("Requested(obtained from non-observed in catalogue.Halo) =", counter_masses_b);
   So.message_screen("Requested(obtained from N_props_particle_level) =",N_props_particle_level);
   So.message_screen("Requested(check Nobs - cumulative) =", this->catalogue._NOBJS()-cumulative_counter);
#endif
   size_t counter_masses_b_tolerance=static_cast<size_t>(floor(counter_masses_b*static_cast<real_prec>(TOLERANCE_FACTOR_L0)));
#ifdef _FULL_VERBOSE_
...
#pragma omp parallel for reduction(+:counter_from_ref)
   ...
   for(size_t i = 0; i < this->dm_properties_bins.size(); ++i)
       for(size_t j = 0; j < this->dm_properties_bins[i].used_property.size(); ++j)
           if(!this->dm_properties_bins[i].used_property[j] && this->dm_properties_bins[i].tracer_properties[j]< this->minimum_multiscale_property)
             counter_from_ref++;
   ...
   size_t ntracers_this_level=min(counter_masses_b_tolerance, counter_from_ref);
#ifdef _FULL_VERBOSE_
   ...
   So.message_screen("Number of properties to assign ",ntracers_this_level);
    ...
    size_t counter_masses_0=0; // NUmber of tracers with property assigned: This will be updated inside the loop
#ifdef _MASS_ASSIGNMENT_TO_REFERENCE_
...
#pragma omp parallel for default(shared) private(ig) reduction(+:counter_internal)
#endif
          for(ig=0; ig <this->catalogue._NOBJS(); ++ig)// loop over the MOCK tracers
           {
             if(!this->catalogue.observed_at(ig))// if this tracer has not been assigned a property, proceed
                {
                  size_t id=this->catalogue.GridID_at(ig); // Retrieve ID of spatial cell where the tracer is located
                  size_t index_bins = cell_info_tr[id].Theta_bin; // Retrieve the bin in theta where that cells has been identified
                  if(counter_from_ref>counter_masses_0) // counter_masses_0 is updated inside this loop. This "if" statment ensures that we try to use all refernece properties
                   {
                    size_t N_props_in_bin = this->dm_properties_bins[index_bins].tracer_properties.size();// Get the number of tracers in that theta bin
                    if(N_props_in_bin>0)
                      {
...
#endif
                           this->catalogue.observed_at(ig)=true;
                           this->catalogue.multi_scale_level_at(ig)=0;
                           this->dm_properties_bins[index_bins].used_property[i_mass_halo_label] = true; //mark this mass as already assigned
                           this->catalogue.gal_cwt_at(ig)=cell_info_tr[id].cwt;
                           this->catalogue.local_dm_at(ig)=cell_info_tr[id].local_dm;
                           counter_internal++;
                         }
                        else
                         {
                           this->catalogue.observed_at(ig)=false;
                           this->catalogue.multi_scale_level_at(ig)=0;
                        }
   ...
   std::cout<<endl;
   So.message_screen("Partial number of properties assigned = ",cumulative_counter);
   So.message_screen("Number of properties to assign = ",this->catalogue._NOBJS()-cumulative_counter);
   std::cout<<endl;
#endif
   size_t nc_test=0;
#ifdef _USE_OMP_
...
#endif
   for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
      if(!this->catalogue.observed_at(ig))
         nc_test++;
#ifdef _FULL_VERBOSE_
   So.message_screen("Number of 'un-observed' (not assigned) tracers = ", nc_test);
   So.message_screen("Number of properties to assign from unobserved tracers= ",this->catalogue._NOBJS()-nc_test);
#endif
    // IF THERE RE STILL PARTICLES TO BE ASSIGNED PROPERTIES:
   if(this->catalogue._NOBJS()-cumulative_counter>0)
    {
...
//---------------------------------------------------------------------------------------------
        ...
        So.message_screen("Number of tracers to assign with tolerance L0b = ", static_cast<size_t>(nc_test*TOLERANCE_FACTOR_L0b));
#endif
        ...
        size_t counter_masses_left=0;
        do{
         for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
           {
            if(0==this->catalogue.observed_at(ig))
              {
               int jj = gsl_rng_uniform_int(rn,left_overs_properties.size());
               if(false==left_overs_properties_used[jj])
                 {
...
#endif
                   this->catalogue.observed_at(ig)=true;
                   this->catalogue.multi_scale_level_at(ig)=8;
                   left_overs_properties_used[jj]=true;
                   counter_masses_left++;
                   counter_masses_0++;
                 }
...
#endif
          if(counter_masses_left==static_cast<size_t>(TOLERANCE_FACTOR_L0b*nc_test))
               break;
        }while(counter_masses_left < static_cast<size_t>(TOLERANCE_FACTOR_L0b*nc_test));
#ifdef _FULL_VERBOSE_
      So.DONE();
  ...
  }//  close if(this->catalogue._NOBJS()-cumulative_counter<0)
...
#ifdef _FULL_VERBOSE_
      if(h_property==_MASS_)
        So.message_screen("Assigning Mvir.  Expected number of properties to assign = ", this->catalogue._NOBJS());
      if(h_property==_VMAX_)
        So.message_screen("Assigning Vmax.  Expected number of properties to assign = ", this->catalogue._NOBJS());
      else if(h_property==_RS_)
        So.message_screen("Assigning RS.  Expected number of properties to assign = ", this->catalogue._NOBJS());
      else if(h_property==_CONCENTRATION_)
       So.message_screen("Assigning Cvir.  Expected number of properties to assign = ", this->catalogue._NOBJS());
      else if(h_property==_SPIN_ || h_property==_SPIN_BULLOCK_ )
        So.message_screen("Assigning SPIN.  Expected number of properties to assign = ", this->catalogue._NOBJS());
#endif
      size_t N_props_particle_level=this->catalogue._NOBJS();
      size_t Ntot=1;
      // For X1 (see table in documentation):
      Ntot*=this->params._NPROPbins_bam();
      // For X2: 
...
#endif
//*****************************************
      size_t Ntot_partial=Ntot;// This must be after the last property advocated
//*****************************************
...
#ifdef _CHECK_ASSIGN_
      Halo_mass_original.resize(this->catalogue._NOBJS(),0);
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
       for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
       Halo_mass_original[ig]=this->catalogue.mass_at(ig);
#endif
...
#pragma omp parallel for
     ...
     for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
        {
          this->catalogue.observed_at(ig)=false;
          this->catalogue.Halo_at(ig).rs=0;
        }
#endif
#ifdef _USE_CONCENTRATION_AS_DERIVED_OBSERVABLE_
  if(h_property == _CONCENTRATION_)
...
#pragma omp parallel for
     ...
     for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
        {
          this->catalogue.set_observed(0,ig);
          this->catalogue.set_concentration(0,ig);
        }
#endif
#ifdef _USE_SPIN_AS_DERIVED_OBSERVABLE_
  if(h_property==_SPIN_ || h_property==_SPIN_BULLOCK_ )
...
#pragma omp parallel for
     ...
     for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
        {
          this->catalogue.set_observed(0,ig);
          this->catalogue.set_spin(0,ig);
          this->catalogue.set_spin_bullock(0,ig);
        }
#endif
...
#pragma omp parallel for
#endif
  for(size_t i=0;i<Ntot_partial;++i)
...
#pragma omp atomic
    ...
    size_t Ng_f=0;
    size_t Ng_v=0;
    size_t counter_masses_or=0;
    size_t counter_masses_global=0;
    size_t counter_masses_test=0;
#ifdef _USE_OMP_TEST2_
  vector<size_t>vseeds(NTHREADS,0);
     for(int  i=0;i<vseeds.size();++i)
         vseeds[i]=565+static_cast<size_t>(i)*565;
     int jthread=0;
     const gsl_rng_type *Trn;
     gsl_rng *randa;
...
#pragma omp parallel for reduction(+:counter_masses_0, counter_masses_or, counter_masses_global,Ng_k,Ng_f,Ng_s,Ng_v, counter_masses_test)
      ...
      // IF we neeed to assign RS, CVIR, Spin, put Mass in the X1 slot
         if(h_property==_RS_ || h_property == _CONCENTRATION_ || h_property==_SPIN_  ||  h_property==_SPIN_BULLOCK_ ) // Using DELTA only allowed so far when assigning mass. FOr Rs and Spin we use so far P(Rs|M,Vmax) or P(S|M,Vmax)
            I_X1 =  get_bin(log10(this->catalogue.mass_at(ig)),this->params._LOGMASSmin(),this->params._NPROPbins_bam(), (this->params._LOGMASSmax()-this->params._LOGMASSmin())/static_cast<double>(this->params._NPROPbins_bam())  ,this->bin_accumulate_borders);
        // Use this if we want to use Vmax (even if primary) to assign seconday, passing over Mvir assigned
//        I_X1 =  get_bin(log10(this->catalogue.vmax_at(ig)),log10(this->params._VMAXmin()),this->params._NPROPbins_bam(), (log10(this->params._VMAXmax())-log10(this->params._VMAXmin()))/static_cast<double>(this->params._NPROPbins_bam())  ,this->bin_accumulate_borders);
#endif
        else
         { // Unless we want to assign M(or Vmax) in which case we put Vmax (or M)
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_  // Here this means that Vmax is already available and we can learn from it
          if(h_property==_MASS_)
            I_X1=get_bin(log10(this->catalogue.vmax_at(ig)), log10(this->params._VMAXmin()),this->params._NPROPbins_bam(),log10(this->params._VMAXmax()/this->params._VMAXmin())/static_cast<double>(this->params._NPROPbins_bam()), true);
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_ // If mvir is primary  we can now assign vmax from P(Vmax|M...)
            I_X1=get_bin(log10(this->catalogue.mass_at(ig)), this->params._LOGMASSmin(),this->params._NPROPbins_bam(),(this->params._LOGMASSmax()-this->params._LOGMASSmin())/static_cast<double>(this->params._NPROPbins_bam()), true);
#endif
         }
        size_t I_X2=0;
        size_t NbinsX2=1;
        if(h_property==_MASS_ || h_property==_VMAX_ || h_property==_CONCENTRATION_ || h_property==_RS_)
           ...
           {
#if defined  _USE_RS_AS_DERIVED_OBSERVABLE_  // Recal that for assignment, Rs and Cvir cannot coexist
            I_X2  = get_bin(log10(this->catalogue.Halo_at(ig).rs),log10(this->params._RSmin()),this->params._NPROPbins_bam(),this->catalogue.logdeltaRS,this->bin_accumulate_borders);
#elif defined _USE_CONCENTRATION_AS_DERIVED_OBSERVABLE_
            I_X2  = get_bin(log10(this->catalogue.concentration_at(ig)),log10(this->params._CONCENTRATIONmin()),this->params._NPROPbins_bam(),log10(this->params._CONCENTRATIONmax()/this->params._CONCENTRATIONmin())/static_cast<double>(this->params._NPROPbins_bam()),this->bin_accumulate_borders);
#endif
            NbinsX2=this->params._NPROPbins_bam();
          }
#endif
         size_t I_X3=0;
#ifdef _USE_CWC_mass_
          I_X3=static_cast<size_t>(this->cwclass.get_Tclassification(id));
          if(true==initial_assignment)
            {
              if(I_X3 == static_cast<int>(WebType::Knot))
                  Ng_k++;
              if(I_X3 == static_cast<int>(WebType::Filament))
                  Ng_f++;
              if(I_X3 == static_cast<int>(WebType::Sheet))
                  Ng_s++;
              if(I_X3 == static_cast<int>(WebType::Void))
                  Ng_v++;
            }
#endif
          size_t I_X4=0;
#ifdef _USE_MACH_NUMBER_
          I_X4= get_bin(this->catalogue.mach_number_at(ig),this->htools_ref->_minmach(), N_BINS_MACH, (this->htools_ref->_maxmach()-this->catalogue._min_mach())/static_cast<real_prec>(N_BINS_MACH), this->bin_accumulate_borders);
#endif
          size_t I_X5=0;
#ifdef _USE_LOCAL_OVERDENSITY_
          I_X5= get_bin(this->catalogue.local_overdensity_at(ig), this->htools_ref->_minlocal_overdensity(), N_BINS_LO,  (this->htools_ref->_maxlocal_overdensity()-this->catalogue._min_local_overdensity())/static_cast<real_prec>(N_BINS_LO) ,this->bin_accumulate_borders);
#endif
         size_t index_dm=0; 
         size_t I_X6=0;
#ifdef _USE_TIDAL_ANISOTROPY_SEC_PROP_ // if defined, onlyu applies to cvir, spin,. not to mass
         I_X6= get_bin(this->catalogue.Halo_at(ig).tidal_anisotropy, this->htools_ref->_mintidal_anisotropy(), N_BINS_TA,  (this->htools_ref->_maxtidal_anisotropy()-this->htools_ref->_mintidal_anisotropy())/static_cast<real_prec>(N_BINS_TA) ,this->bin_accumulate_borders);
         std::vector<size_t>idxa={I_X1,I_X2,I_X3,I_X4,I_X5,I_X6};
         std::vector<size_t>dimsa={0,NbinsX2,n_cwt,N_BINS_MACH,N_BINS_LO,N_BINS_TA};    
         size_t index_dm=index_nd(idxa,dimsa);
#else
                ...
                while(prob<ran)
                 {
...
#endif
                      size_t index_or=index_2d(i_halo_index,index_dm,Ntot_partial);
                  ...
                  if(_VMAX_ == h_property)
                    {
                      real_prec fraction_mass= xr*log10(this->htools_ref->VMAXBmax[i_halo_index]/this->htools_ref->VMAXBmin[i_halo_index]);
                      real_prec lmass_halo = log10(this->htools_ref->VMAXBmin[i_halo_index])+fraction_mass ;
                      this->catalogue.vmax_at(ig) = pow(10,lmass_halo);
                  }
#elif defined (_USE_VMAX_AS_PRIMARY_OBSERVABLE_)
                  if(_MASS_ == h_property)
                    {
                      real_prec fraction_mass= xr*log10(this->htools_ref->MBmax[i_halo_index]/this->htools_ref->MBmin[i_halo_index]);
                      real_prec lprop_halo = log10(this->htools_ref->MBmin[i_halo_index])+fraction_mass ;
                      this->catalogue.mass_at(ig) = pow(10,lprop_halo);
                    }
#endif
#ifdef _USE_RS_AS_DERIVED_OBSERVABLE_
                   else if(_RS_== h_property)
                    {
                      real_prec fraction_mass= xr*log10(this->htools_ref->RSBmax[i_halo_index]/this->htools_ref->RSBmin[i_halo_index]);
                      real_prec lprop_halo = log10(this->htools_ref->RSBmin[i_halo_index])+fraction_mass ;
                      this->catalogue.Halo_at(ig).rs = pow(10,lprop_halo);
                   }
#endif
#ifdef _USE_CONCENTRATION_AS_DERIVED_OBSERVABLE_
                   else if(_CONCENTRATION_== h_property)
                    {
                      real_prec fraction_mass= xr*log10(this->htools_ref->CONCENTRATIONBmax[i_halo_index]/this->htools_ref->CONCENTRATIONBmin[i_halo_index]);
                      real_prec lprop_halo = log10(this->htools_ref->CONCENTRATIONBmin[i_halo_index])+fraction_mass ;
                      this->catalogue.concentration_at(ig) = pow(10,lprop_halo);
                    }
#endif
#ifdef _USE_SPIN_AS_DERIVED_OBSERVABLE_
                    else if(_SPIN_== h_property)
                      {
                        real_prec fraction_mass= xr*log10(this->htools_ref->SPINBmax[i_halo_index]/this->htools_ref->SPINBmin[i_halo_index]);
                        real_prec lprop_halo = log10(this->htools_ref->SPINBmin[i_halo_index])+fraction_mass ;
                        this->catalogue.spin_at(ig) = pow(10,lprop_halo);
                      }
                    else if(h_property==_SPIN_BULLOCK_)
                      {
                        real_prec fraction_mass= xr*log10(this->htools_ref->SPINBmax[i_halo_index]/this->htools_ref->SPINBmin[i_halo_index]);
                        real_prec lprop_halo = log10(this->htools_ref->SPINBmin[i_halo_index])+fraction_mass ;
                        this->catalogue.spin_bullock_at(ig) = pow(10,lprop_halo);
                      }
#endif
                     this->catalogue.observed_at(ig)=true;
                     counter_masses_0++;
                      ...
                      while(prob<ran)
                       {
                        ...
                        mass_tracer=pow(10,log10(limit_vmax_halos(this->catalogue.mass_at(ig)))+fraction_mass);
#endif
                        ...
                        if(h_property==_MASS_)
                            this->catalogue.mass_at(ig)=mass_tracer;
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_ // if this applies, we now assign vmax
                        if(h_property==_VMAX_)
                            this->catalogue.set_vmax(mass_tracer, ig);
#endif
#ifdef _USE_RS_AS_DERIVED_OBSERVABLE_
                        else if(h_property==_RS_)
                            this->catalogue.set_rs(mass_tracer, ig);
#endif
#ifdef _USE_CONCENTRATION_AS_DERIVED_OBSERVABLE_
                        else if(h_property==_CONCENTRATION_)
                          this->catalogue.set_concentration(mass_tracer, ig);
#endif
#ifdef _USE_SPIN_AS_DERIVED_OBSERVABLE_
                        else if(h_property==_SPIN_)
                          this->catalogue.set_spin(mass_tracer,ig);
                        else if(h_property==_SPIN_BULLOCK_)
                          this->catalogue.set_spin_bullock(mass_tracer,ig);
#endif
...
#endif
#endif
     } // end loop over tracers for(size_t ig=0; ig < this->catalogue._NOBJS();++ig)
#ifdef _USE_OMP_TEST2_
     gsl_rng_free(randa);
    } // closes parallel region
#endif
      So.message_screen("Number of properties assigned (test) = ",static_cast<size_t>(counter_masses_test));
...
#elif defined _USE_VMAX_AS_PRIMARY_OBSERVABLE_
        So.message_screen("Number of properties assigned = ",static_cast<size_t>(counter_masses_0));
#endif
#endif
    if(h_property==_MASS_)
      So.message_screen("Number of Mvir values assigned (using joint prob-dist) = ",static_cast<size_t>(counter_masses_or));
    else if(h_property==_RS_)
       So.message_screen("Number of Rs values assigned (using joint prob-dist) = ",static_cast<size_t>(counter_masses_or));
    else if(h_property==_CONCENTRATION_)
       So.message_screen("Number of Cvir values assigned (using joint prob-dist) = ",static_cast<size_t>(counter_masses_or));
    else if(h_property==_SPIN_ || h_property==_SPIN_BULLOCK_)
      So.message_screen("Number of Spin values assigned (using joint prob-dist) = ",static_cast<size_t>(counter_masses_or));
    So.message_screen("Number of properties assigned with global abundance = ",static_cast<size_t>(counter_masses_global));
#ifdef _USE_CWC_mass_
    if(true==initial_assignment)
    {
      So.message_screen("Fraction of tracers in knots:",  Ng_k*100./static_cast<real_prec>(this->catalogue._NOBJS()), "%");
      So.message_screen("Fraction of tracers in filaments:", Ng_f*100./static_cast<real_prec>(this->catalogue._NOBJS()), "%");
      So.message_screen("Fraction of tracers in sheets:",  Ng_s*100./static_cast<real_prec>(this->catalogue._NOBJS()), "%");
      So.message_screen("Fraction of tracers in voids:", Ng_v*100./static_cast<real_prec>(this->catalogue._NOBJS()), "%");
    }
#endif
    So.DONE();
#endif
...
#endif
  So.DONE();
  size_t assigned_properties_mf=0;
...
#pragma omp parallel for reduction(+:assigned_properties_mf)// ask, after all the multilevel or other assignmetn, how many are there left without property
  ...
  if(true==initial_assignment)  //this applies for vmax assignment when the global vmax function  is needed
    {
      if(assigned_properties_mf != this->catalogue._NOBJS())
        {
#ifdef _FULL_VERBOSE_
          So.message_screen("Number of tracers with *no property* assigned so far = ", this->catalogue._NOBJS()-assigned_properties_mf);
#endif
          size_t nc_test=0;
...
#pragma omp for reduction(+:Ncount_ab) // this parallelization was causing a problem
            ...
            for(size_t ig=0; ig < this->catalogue._NOBJS(); ++ig)
              {
                if(!this->catalogue.observed_at(ig) || this->catalogue.vmax_at(ig)<=0)
                  {
                     real_prec prob=-10.0;
                     real_prec ran=10.0;
...
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                           if(aux_prop<this->htools_ref->VMAXBin[0] || aux_prop>this->htools_ref->VMAXBin[nbins_mf-1])
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
                           if(aux_prop<=this->htools_ref->MBmin[0] || aux_prop>=this->htools_ref->MBmin[nbins_mf-1])
#endif
                             prob=0.0;
                           else
                             {
...
#ifdef _USE_MASS_AS_PRIMARY_OBSERVABLE_
                         this->catalogue.mass_at(ig)=property_tracer;
#elif defined _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                            this->catalogue.vmax_at(ig)=property_tracer;
#endif
                          this->catalogue.observed_at(ig)=true;
                          this->catalogue.multi_scale_level_at(ig)=-1;
                          Ncount_ab++;
                      }//closes if
...
//  this->catalogue.aux_flag=false;

  if(true==initial_assignment)
    {
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
      this->htools->params.set_i_vmax_g(POSITIVE_INT); //this allows the tracer to get the vmax function
      this->htools->params.set_i_mass_g(NEGATIVE_INT); //no mass information here
      this->htools->params.set_i_spin_bullock_g(NEGATIVE_INT); //no mass information here
      this->htools->params.set_i_rs_g(NEGATIVE_INT); //no mass information here
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
      this->htools->params.set_i_vmax_g(-5); //no mass information here
      this->htools->params.set_i_mass_g(5); //this allows the tracer to get the mass function
#endif
      this->htools->get_property_function(fname_mass_function_Y);
      this->htools_ref->get_property_function(fname_mass_function_Y_ref);
#ifdef _USE_GNUPLOT_ABUNDANCE_V_PLOT_
...
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
      for(size_t i=0; i<this->htools->vmax_function.size(); ++i)
          xy_pts_v.push_back(std::make_pair(this->catalogue.VMAXBmin[i]+(i+0.5)*(this->catalogue.VMAXBmax[i]-this->catalogue.VMAXBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools->vmax_function[i])));
...
#elif defined  _USE_MASS_AS_PRIMARY_OBSERVABLE_
      for(size_t i=0; i<this->htools->mass_function.size(); ++i)
          xy_pts_v.push_back(std::make_pair(this->catalogue.MBmin[i]+(i+0.5)*(this->catalogue.MBmax[i]-this->catalogue.MBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools->mass_function[i])));
...
#pragma omp parallel for reduction(+:count_b,residuals)
...
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
        if(this->htools->vmax_function[i]>0)
#elif defined  _USE_MASS_AS_PRIMARY_OBSERVABLE_
          if(this->htools->mass_function[i]>0)
#endif
        {
          count_b++;
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
          residuals+= fabs(this->htools_ref->vmax_function[i]/this->htools->vmax_function[i] -1.0 );
#elif defined  _USE_MASS_AS_PRIMARY_OBSERVABLE_
          residuals+= fabs(this->htools_ref->mass_function[i]/this->htools->mass_function[i] -1.0 );
#endif
        }//closes if(this->htools->mass_function[i]>0)
  ...
  if(h_property==_VMAX_)
      ...
      {
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
        this->htools->params.set_i_vmax_g(NEGATIVE_INT); //this avoids the measurement of the vmax function again
        this->htools->params.set_i_mass_g(POSITIVE_INT); //this allow the tracer to get the mass function
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
        this->htools->params.set_i_vmax_g(POSITIVE_INT); //this allowsthe measurement of the vmax function
        this->htools->params.set_i_mass_g(NEGATIVE_INT); //this avoids the tracer to get the mass function
#endif
        this->htools->params.set_i_rs_g(NEGATIVE_INT); //no rs information here
        this->htools->params.set_i_spin_bullock_g(NEGATIVE_INT); //this allow the tracer to get the mass function
        this->htools->get_property_function(fname_mass_function_Y);
        this->htools_ref->get_property_function(fname_mass_function_Y_ref);
#ifdef _USE_GNUPLOT_ABUNDANCE_PLOT_
      ...
      this->gp_abundance<<"set ylabel 'log n(M) ' font 'Times-Roman,15' textcolor rgb '"<<FG_COLOR<<"' \n";
        ...
        for(size_t i=0; i<this->htools->vmax_function.size(); ++i)
          xy_pts_v.push_back(std::make_pair(this->catalogue.VMAXBmin[i]+(i+0.5)*(this->catalogue.VMAXBmax[i]-this->catalogue.VMAXBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools->vmax_function[i])));
        for(size_t i=0; i<this->htools->vmax_function.size(); ++i)
          xy_pts_ref_v.push_back(std::make_pair(this->htools_ref->VMAXBmin[i]+(i+0.5)*(this->htools_ref->VMAXBmax[i]-this->htools_ref->VMAXBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools_ref->vmax_function[i])));
        ...
        for(size_t i=0; i<this->htools->mass_function.size(); ++i)
          xy_pts_v.push_back(std::make_pair(this->catalogue.MBmin[i]+(i+0.5)*(this->catalogue.MBmax[i]-this->catalogue.MBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools->mass_function[i])));
        for(size_t i=0; i<this->htools->mass_function.size(); ++i)
          xy_pts_ref_v.push_back(std::make_pair(this->htools_ref->MBmin[i]+(i+0.5)*(this->htools_ref->MBmax[i]-this->htools_ref->MBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools_ref->mass_function[i])));
        this->gp_abundance<<"plot"<<this->gp_abundance.file1d(xy_pts_ref_v) << "w l lw 3 lt 2 title 'Reference', "<<this->gp_abundance.file1d(xy_pts_v) << "w l lw 3 lt 3 title 'Mock'"<<endl;
#endif
        xy_pts_v.clear(); xy_pts_v.shrink_to_fit();
        xy_pts_ref_v.clear(); xy_pts_ref_v.shrink_to_fit();
...
#pragma omp parallel for reduction(+:count_b,residuals)
      ...
      {
        this->htools->params.set_i_vmax_g(NEGATIVE_INT); //this allows the avoid the measurement of the vmax function again
        this->htools->params.set_i_mass_g(NEGATIVE_INT); //this allow aviud the tracer to get the mass function
        this->htools->params.set_i_rs_g(POSITIVE_INT); //this allow the tracer to get the mass function
        this->htools->params.set_i_spin_g(NEGATIVE_INT); //this allow the tracer to get the mass function
        this->htools->get_property_function(fname_mass_function_Y);
        ...
        xy_pts_ref_v.clear(); xy_pts_ref_v.shrink_to_fit();
        vector<pair<real_prec, real_prec> > xy_pts_v;
        for(size_t i=0; i<this->catalogue.rs_function.size(); ++i)
          xy_pts_v.push_back(std::make_pair(this->catalogue.RSBmin[i]+(i+0.5)*(this->catalogue.RSBmax[i]-this->catalogue.RSBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->catalogue.rs_function[i])));
        for(size_t i=0; i<this->htools_ref->rs_function.size(); ++i)
          xy_pts_ref_v.push_back(std::make_pair(this->htools_ref->RSBmin[i]+(i+0.5)*(this->htools_ref->RSBmax[i]-this->htools_ref->RSBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools_ref->rs_function[i])));
...
#pragma omp parallel for reduction(+:count_b,residuals)
        for(size_t i=0;i<this->params._NMASSbins_mf() ;++i)
          if(this->catalogue.rs_function[i]>0)
            {
              count_b++;
              residuals+= fabs(this->htools_ref->rs_function[i]/this->catalogue.rs_function[i]-1.0);
            }
        residuals/=static_cast<real_prec>(count_b)/100.0;
#ifdef _FULL_VERBOSE_
    ...
    else if(h_property==_CONCENTRATION_)
      {
        ...
        vector<pair<real_prec, real_prec> > xy_pts_v;
        for(size_t i=0; i<this->catalogue.rs_function.size(); ++i)
          xy_pts_v.push_back(std::make_pair(this->catalogue.CONCENTRATIONBmin[i]+(i+0.5)*(this->catalogue.CONCENTRATIONBmax[i]-this->catalogue.CONCENTRATIONBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->catalogue.rs_function[i])));
        for(size_t i=0; i<this->htools_ref->rs_function.size(); ++i)
          xy_pts_ref_v.push_back(std::make_pair(this->htools_ref->CONCENTRATIONBmin[i]+(i+0.5)*(this->htools_ref->CONCENTRATIONBmax[i]-this->htools_ref->RSBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools_ref->rs_function[i])));
        this->gp_abundance_rs<<"plot"<<this->gp_abundance_rs.file1d(xy_pts_ref_v) << "w l lw 3 lt 2 title 'Reference', "<<this->gp_abundance_rs.file1d(xy_pts_v) << "w l lw 3 lt 3 title 'Mock' textcolor rgb '"<<FG_COLOR<<"'"<<endl;
        xy_pts_v.clear(); xy_pts_v.shrink_to_fit();
        xy_pts_ref_v.clear(); xy_pts_ref_v.shrink_to_fit();
#endif
...
#pragma omp parallel for reduction(+:count_b,residuals)
        for(size_t i=0;i<this->params._NMASSbins_mf() ;++i)
          if(this->htools->cvir_function[i]>0)
            {
              count_b++;
              residuals+= fabs(this->htools_ref->cvir_function[i]/this->htools->cvir_function[i]-1.0);
            }
        residuals/=static_cast<real_prec>(count_b)/100.0;
#ifdef _FULL_VERBOSE_
      ...
      else if(h_property==_SPIN_ || h_property==_SPIN_BULLOCK_)
       {
        this->htools->params.set_i_vmax_g(NEGATIVE_INT); //this allows the avoid the measurement of the vmax function again
        this->htools->params.set_i_mass_g(NEGATIVE_INT); //this allow aviud the tracer to get the mass function
        this->htools->params.set_i_rs_g(NEGATIVE_INT); //this allow the tracer to get the mass function
        if(h_property==_SPIN_)
          this->htools->params.set_i_spin_g(POSITIVE_INT); //this allow the tracer to get the mass function
        if(h_property==_SPIN_BULLOCK_)
          this->htools->params.set_i_spin_bullock_g(POSITIVE_INT); //this allow the tracer to get the mass function
        this->htools->get_property_function(fname_mass_function_Y);
        ...
        xy_pts_ref_v.clear(); xy_pts_ref_v.shrink_to_fit();
        vector<pair<real_prec, real_prec> > xy_pts_v;
        for(size_t i=0; i<this->catalogue.s_function.size(); ++i)
          xy_pts_v.push_back(std::make_pair(this->htools->SPINBmin[i]+(i+0.5)*(this->htools->SPINBmax[i]-this->htools->SPINBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->catalogue.s_function[i])));
        for(size_t i=0; i<this->htools_ref->s_function.size(); ++i)
          xy_pts_ref_v.push_back(std::make_pair(this->htools_ref->SPINBmin[i]+(i+0.5)*(this->htools_ref->SPINBmax[i]-this->htools_ref->SPINBmin[i])/static_cast<double>(this->params._NMASSbins_mf()), log10(this->htools_ref->s_function[i])));
#endif            
...
#pragma omp parallel for reduction(+:count_b,residuals)
        for(size_t i=0;i<this->params._NMASSbins_mf() ;++i)
          if(this->htools->s_function[i]>0)
            {
              count_b++;
              residuals+= fabs(this->htools_ref->s_function[i]/this->htools->s_function[i]-1.0);
            }
        residuals/=static_cast<real_prec>(count_b)/100.0;
...
//  this->catalogue_ref.select_random_subsample(0.2, rfile);
...
//      this->catalogue.select_random_subsample(0.2, bfile);
  ...
  // aca nos toca hacer un ranomd selection para escribir las masas originales y las asignadas en el caso en que estemos asignando al mismo catalogo
    // copio esto de la clase cataalog y lo hago directamente aca, donde tengo acceso a tracer_ref y a tracer
      ...
      So.message_screen("\tSelecting a fraction: ",100*fraction, "%");
        do
       {
        int i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
        rcat<<log10(Halo_mass_original[i])<<"\t"<<log10(this->catalogue.mass_at(i))<<endl;
        counter++;
...
void BiasMT::sample_mock()
   {
     ...
     for(size_t i=0;i<this->delta_Y_new.size();++i)
       {
         int Ngal=this->delta_Y_new[i];
         int counter=0;
         size_t x_min, y_min, z_min;
...
#pragma omp parallel for reduction(+:new_nobjects)
                 ...
                 for(size_t i = 0; i <  this->params._NGRID(); ++i)
#ifdef _USE_CWC_
                   if(this->cwclass.get_cell_classified(sua, i))
#endif
...
#pragma omp parallel for
#endif
         for(size_t i = 0; i < new_nbins_x; ++i)
           X_bins[i]= (this->params._iMAS_X() == 0 && !this->params._Convert_Density_to_Delta_X() && type_X==LINEAR) ? static_cast<real_prec>(i) :  this->Xmin+(i+0.5)*(this->Xmax-this->Xmin)/static_cast<real_prec>(new_nbins_x);
...
#endif
//         for(size_t i = 0; i < new_nbins_y; ++i)
 //          Y_bins[i]= (iMAS_Y == 0  && !this->params._Convert_Density_to_Delta_Y()  && type_Y==LINEAR) ? static_cast<real_prec>(i) :  this->Ymin+(i+0.5)*(this->Ymax-this->Ymin)/static_cast<real_prec>(new_nbins_y);

         for(size_t i = 0; i < new_nbins_y; ++i)
           Y_bins[i]=   this->Ymin+(i+0.5)*(this->Ymax-this->Ymin)/static_cast<real_prec>(new_nbins_y);
         ofstream sy; sy.open("ybins.txt");
         for(size_t i = 0; i < new_nbins_y; ++i)sy<<i<<"  "<<Y_bins[i]<<endl;
         sy.close();
         // ************************RESIZE VECTORS FOR HISTOGRAMS*************************
...
#pragma omp parallel for
         ...
         for(size_t ig = 0; ig<  this->params._NGRID() ; ++ig)
           {
             size_t redshift_bin=0;
             real_prec redshift_mask_cell=0;
#ifdef _USE_REDSHIFT_MASK_
#ifdef _USE_BINARY_MASK_
             if(static_cast<real_prec>(binary_mask_at(ig))>0)
               {
#endif
                 redshift_mask_cell=redshift_mask_at(ig);
...
#pragma omp atomic update
                     ...
                     size_t ict=0;
#ifdef _USE_MASS_KNOTS_
                     ict=this->cwclass.SKNOT_M_info_at(ig);
#endif
#ifdef _USE_CWC_
                     if(this->cwclass.get_cell_classified(sua, ig))
                       {
#endif
                         if(delta_X[ig]!= NOCELL && delta_Y[ig]!= NOCELL)   // This means, use cells being classified.
                           if((delta_X[ig]>=this->Xmin && delta_X[ig]<=this->Xmax) && (delta_Y[ig]>=this->Ymin && delta_Y[ig]<=this->Ymax))
                             {
                               size_t IX= get_bin(delta_X[ig], this->Xmin, this->params._NX(),this->DELTAX, false);
                               size_t IY= get_bin(delta_Y[ig], this->Ymin, this->params._NY(),this->DELTAY, false);
                               std::vector<size_t>idxa={IX, IY, sua, ict, redshift_bin};
                               std::vector<size_t>dimsa={0,new_nbins_y, this->params._n_cwt(), this->params._n_sknot_massbin(), N_REDSHIFT_BINS};  
#ifdef _USE_OMP_
#pragma omp atomic update
#endif
                              this->BIAS_NCOUNTS[index_nd(idxa,dimsa)]++;
                           }
#ifdef _USE_CWC_
                       }
#endif
...
#ifdef _USE_REDSHIFT_MASK_
         size_t NCELLS=0;
         for(size_t i=0;i<ncells_zbin.size();++i)
             ...
             for(mk=0;mk<this->params._n_sknot_massbin();++mk)
               {
                 ...
                 for(size_t lx=0;lx<new_nbins_x;++lx)
                   for(size_t ly=0;ly<new_nbins_y;++ly){
                    std::vector<size_t>idxa={lx,ly,sua,mk,iz};
                    std::vector<size_t>dimsa={0,new_nbins_y, this->params._n_cwt(), this->params._n_sknot_massbin(), N_REDSHIFT_BINS};   
                    aux_v[index_2d(lx,ly,new_nbins_y)]=this->BIAS_NCOUNTS[index_nd(idxa,dimsa)];
                   }
                 size_t lkk=get_max<LONG>(aux_v);
                 vector<real_prec>aux_n(new_nbins_x*new_nbins_y, 0);
                 for(size_t i=0;i< aux_v.size() ;++i)
                 ...
                 for(size_t i=0;i<new_nbins_x;++i)
                   for(size_t j=0;j<new_nbins_y;++j)
         ...
         // if(this->cwt_used[sua]==WebType::Knot)
         //   {
#ifdef _nomas_
#ifdef _USE_REDSHIFT_MASK_
             ...
             for(int mk=0;mk<this->params._n_sknot_massbin();++mk)
               {
                 ...
                 for(size_t i=0;i<new_nbins_x;++i)// loop sobre bines de dark matter
                   {
...
//                     for(int j=0;j<vEPSILON.size();++j)
 //                       std::cout<<vEPSILON[j]<<endl;
                     ...
                     if (this->params._write_files_for_histograms() && vEPSILON.size()>0)
                       {
...
{
  vector<size_t>global_id_candidate;
  vector<size_t>id_dist_candidate;
  size_t sort_candidates(){
    size_t rank=0;
    sort_1d_vectors<size_t>(this->id_dist_candidate,rank);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   void BiasMT::collapse_randoms()
   {
     ...
     for(size_t i=0; i< Ntracers; ++i)
       {
         size_t id=grid_ID(&box_collps, this->catalogue.coord1_at(i),this->catalogue.coord2_at(i),this->catalogue.coord3_at(i));
         if(this->catalogue.identity_at(i)>0)
           {
             x_dm_pos.push_back(this->catalogue.coord1_at(i));
             y_dm_pos.push_back(this->catalogue.coord2_at(i));
             z_dm_pos.push_back(this->catalogue.coord3_at(i));
             mass_dm.push_back(this->catalogue.mass_at(i));
         ...
         else if(this->catalogue.identity_at(i)<0)
           {
             x_random_pos.push_back(this->catalogue.coord1_at(i));
             y_random_pos.push_back(this->catalogue.coord2_at(i));
             z_random_pos.push_back(this->catalogue.coord3_at(i));
             mass_random.push_back(this->catalogue.mass_at(i));
...
#ifdef _COLLAPSE_RANDOMS_VELS_
             x_random_v.push_back(this->catalogue.vel1_at(i));
             y_random_v.push_back(this->catalogue.vel2_at(i));
             z_random_v.push_back(this->catalogue.vel3_at(i));
#endif
           }
       }
     this->So.DONE();
...
#define NO_NUM -999
     size_t Mdm_index_cell=box_collps.NGRID*max_count;
     vector<size_t> dm_index_cell(Mdm_index_cell, NO_NUM);
#ifdef _SEARCH_CLOSEST_DM_IN_NEIGH_CELLS_
     vector<size_t>dm_particle_id(Mdm_index_cell,0);
#endif
#ifdef _FULL_VERBOSE_
     ...
     for(size_t i=0;i<Ntracers;++i) // Loop over the DM particles
     {
       if(this->catalogue.identity_at(i)>0) // Loop over the DM particles
         {
           size_t idc=dm_id[i];         // Get the ID of the cell where this DM particle is located
           size_t dmc=dm_count[idc];    // Retrieve the current number of dm particles (local id) in this IDcell, 0, 1, 2, ..., Ndm(cell) in the cell
     ...
     dm_id.shrink_to_fit();
     this->So.DONE();
     vector<size_t>dm_index_closer_tot(N_rand,0);
#ifdef _SEARCH_CLOSEST_DM_IN_NEIGH_CELLS_
...
#pragma omp parallel for
#endif
     for(size_t i=0; i< N_rand; ++i)//loop over randoms
       {
         size_t id_tracer=ran_id[i]; // identify the ID (low-res cell) where this random lives.
         real_prec xra=x_random_pos[i];
         real_prec yra=y_random_pos[i];
         real_prec zra=z_random_pos[i];
         vector<size_t>i_r_to_dm_dist; // define array to allocate distance in form of index
         vector<size_t>n_index_tot;    // define array to allocate the label of the dm particle in the cell
         size_t N_dm_cell=dm_count[id_tracer];// Get the number of dm particles in this cell
         if(N_dm_cell>0)// if there are dm particles in the cell where this random is located, then
           for(size_t jc = 0; jc < N_dm_cell; ++jc)//loop over the dm particles in the cell where the current random is
              {
                size_t jdm=dm_index_cell[index_2d(id_tracer,jc,max_count)]; //indice entre (0,N_dm-1) que tiene cada particula de dm dentro de la celda id_ran
                if(jdm!=NO_NUM)
                   {
                     real_prec xda=xra-x_dm_pos[jdm];// x_random - x_dm
                     real_prec yda=yra-y_dm_pos[jdm];
                     real_prec zda=zra-z_dm_pos[jdm];
                     size_t i_dist=_get_modulo_squared(xda,yda,zda);  //distance² between rand and each dm particle, converted to floor
         ...
         size_t rank=0;
         sort_1d_vectors<size_t>(i_r_to_dm_dist, rank); // sort the container with distance indices
         size_t s_ind=n_index_tot[rank];                // get the if dm in (0,.Ndm-1) particle associated to the first element of the sorted distance
#ifdef _SEARCH_CLOSEST_DM_IN_NEIGH_CELLS_
         size_t s_val=i_r_to_dm_dist[rank];             // get the distance index
     ...
     for(size_t i=0; i< N_rand; ++i)//loop over randoms
       {
         ...
         for (int ic=0; ic < N_neighbour_cells; ++ic)  // Loop over neighbour cells
           {
             size_t id_neighbour=nearest_cells[id_tracer].close_cell[ic]; // ID of the current neighbout cell
             size_t N_dm_cell=dm_count[id_neighbour];                     // Get the number of dm particles in this cell
             if(N_dm_cell > 0)// for this "if" I do not write an else, for if there are no dm in neighbourr cells, we do not care
              {
                for(size_t jc = 0; jc < N_dm_cell; ++jc)//loop over the dm particles in the neighbour cell id_n
                  {
                    size_t jdm  = dm_index_cell[index_2d(id_neighbour, jc, max_count)]; //index jdm in(0,N_dm-1) for each dm particle in the cell id_n
                    if(jdm!=NO_NUM)
                     {
                       real_prec xda = xra-x_dm_pos[jdm];
                       real_prec yda = yra-y_dm_pos[jdm];
                       real_prec zda = zra-z_dm_pos[jdm];
                       size_t i_dist= _get_modulo_squared(xda,yda,zda); //100000 * distance² between rand and each dm particle, converted to floor
                       size_t id_dm = dm_particle_id[index_2d(id_neighbour, jc, max_count)]; // global index in (0,N_tracer-1) for each dm particle in the cell id_n
          ...
          sort_1d_vectors<size_t>(i_r_to_dm_dist, rank);
...
//#pragma omp parallel for
#endif
     for(size_t counter=0; counter< N_rand; ++counter)
      {
         ...
         size_t index_dm_closer_a = dm_index_closer_tot[counter];
...
#ifdef _SEARCH_CLOSEST_DM_IN_NEIGH_CELLS_
         real_prec xdm = this->catalogue.Halo[index_dm_closer_a].coord1;
         real_prec ydm = this->catalogue.Halo[index_dm_closer_a].coord2;
         real_prec zdm = this->catalogue.Halo[index_dm_closer_a].coord3;
#else
        ...
        // redefine ran coords to the ref sistem of its closest dm particle:
         ...
         this->catalogue.set_coord1(sep_with_exclusion*sin(theta)*cos(phi)+xdm, ran_id_global[counter]);
         this->catalogue.set_coord2(sep_with_exclusion*sin(theta)*sin(phi)+ydm, ran_id_global[counter]);
         this->catalogue.set_coord3(sep_with_exclusion*cos(theta)+zdm, ran_id_global[counter]);
#ifdef _COLLAPSE_RANDOMS_VELS_
...
/*         real_prec alpha_max=alpha<M_PI ? (M_PI-alpha): alpha;
        ...
        // real_prec uup=_get_modulo(ux,uy,uz); // modulo, just co cross-check with v
         ...
         this->catalogue.Halo[ran_id_global[counter]].vel1=nux;
         this->catalogue.Halo[ran_id_global[counter]].vel2=nuy;
         this->catalogue.Halo[ran_id_global[counter]].vel3=nuz;
#endif
       }
     this->So.DONE();
     ...
     vector<size_t>vseeds(NTHREADS,0);
     for(size_t i=0;i<vseeds.size();++i)
         vseeds[i]=35+static_cast<size_t>(i)*565;
      const gsl_rng_type *  rng_t;
      gsl_rng * gBaseRand;
      int jthread=0;
      gsl_rng_env_setup();
...
#pragma omp for
     ...
     for(size_t i=0; i< this->catalogue._NOBJS(); ++i)
      {
         if(this->catalogue.mass_at(i)>this->params._M_exclusion())
        {
          real_prec x = this->catalogue.coord1_at(i);
          real_prec y = this->catalogue.coord2_at(i);
          real_prec z = this->catalogue.coord3_at(i);
          real_prec mass = this->catalogue.mass_at(i);              // Mass of dm particle
          real_prec radius = pow(3.0*mass/(4.*M_PI*density), 1./3.);
//          this->catalogue.coord1_at(i)= x + gsl_ran_gaussian(gBaseRand, radius);
//          this->catalogue.coord2_at(i)= y + gsl_ran_gaussian(gBaseRand, radius);
//          this->catalogue.coord3_at(i)= z + gsl_ran_gaussian(gBaseRand, radius);
          this->catalogue.coord1_at(i)= x + radius*(gsl_rng_uniform(gBaseRand)-0.5);
          this->catalogue.coord2_at(i)= y + radius*(gsl_rng_uniform(gBaseRand)-0.5);
          this->catalogue.coord3_at(i)= z + radius*(gsl_rng_uniform(gBaseRand)-0.5);
         }
      }
#ifdef _USE_OMP_
}
 ...
 void BiasMT::execute()
  {
    ...
    this->warnings();
     ...
     this->catalogue.read_catalog_new(this->params._Input_dir_cat()+this->params._file_catalogue());
     htools = new HaloTools(params, catalogue);

this->catalogue.get_density_field_grid(_COUNTS_, file_density_field_tracer);
#ifdef _USE_MASS_FIELD_
     htools->get_density_field_grid(_MASS_, file_Y_mass);
     string fname_mass_function_Y = this->params._Output_directory()+"tracer_ref_abundance.txt";
     htools->get_property_function(fname_mass_function_Y);
#endif
#endif
#endif
#ifdef _GET_INTERPOLATED_FIELDS_FROM_BIN_FILES_
     this->catalogue.read_catalog_bin( this->params._NGRID(),"A","A","A");
     exit(0);
#endif
...
#pragma omp parallel for reduction(+:residuals,ncounts)
               for(size_t i=0;i<this->Power_REF.size();++i)
...
#endif
{
    ...
    // In this function we asign coordinates from DM + random particles based on the mock number counts
     ...
     // ****************************DM POSITIONS *******************************************
     size_t N_dm= this->params._NGRID();     // Note that here the number of dm particles is that of the Ngrid
    ...
    //  Cointainers for the velocities of the dm particles evaluated at Lagrangian positions.
     ...
     So.DONE();
#ifdef _USE_NP_VEL_
     vector<size_t>id_empty_cells;  // COntainer to allocate the index ID of empty cells
#endif
#ifdef _FULL_VERBOSE_
...
// COnvolve the velocities with a kernel of the form 1/(1+s² k²). This has to be applin to the intepolated vel-field.
    if(this->params._use_vel_kernel())
    {
...
#pragma omp parallel for
#endif
      for(size_t i=0;i<aux_vx.size();++i)
       {
       int I_CWT=static_cast<int>(this->cwclass.get_Tclassification(i));
       if(this->cwclass.cwt_used[I_CWT]== static_cast<int>(WebType::Knot) ||  this->cwclass.cwt_used[I_CWT]== static_cast<int>(WebType::Filament))
         {
           aux_vx[i]=velx[i];
           aux_vy[i]=vely[i];
           aux_vz[i]=velz[i];
...
// call the _USE_TRACERS_IN_CELLS_
     ...
     size_t Nobjects_mock=get_nobjects(MOCK_DEN_FIELD);
     size_t Ntracers = Nobjects_mock;  //reduntant, but still
     this->catalogue.set_NOBJS(Nobjects_mock);
#endif
...
#endif
         size_t id   =  index[i];    //identify the cell where this particle lives
#ifdef _COLLAPSE_RANDOMS_AUX_
         size_t id_collapse = grid_ID(&box_collps, x,y,z);
#endif
     ...
     if(Nrandoms1>0)
       {
         ...
         for(size_t i=0; i<this->params._Nft(); ++i)
           for(size_t j=0; j<this->params._Nft(); ++j)
             for(size_t k=0; k<this->params._Nft(); ++k)
               {
                 size_t id= index_3d(i,j,k,this->params._Nft(),this->params._Nft());
...
#pragma omp parallel for
#endif
     for(size_t idm=0;idm<N_dms;++idm)
       {
         size_t id=dm_id[idm];
...
#pragma omp parallel for
     ...
     for(size_t i=0; i< N_rand; ++i)//loop over randoms
       {
         ...
         vector<size_t>i_r_to_dm_dist;
         vector<size_t>n_index_tot;
         if(N_dm_cell>0)// if there are dm particles in the low res cell where this random is located, then
           {
             for(size_t jc = 0; jc < N_dm_cell; ++jc)//loop over the dm in that cell where the random is
               {
                 size_t jdm=dm_index_cell[id_ran][jc]; //indice entre (0,Ndm-1) que identifica a cada particula DM dentro de la celda id_ran
                 if(jdm!=NO_NUM)
                   {
                     ...
                     size_t i_dist=_get_modulo_squared(xdd,ydd,zdd);  //distance² between rand and each dm particle, converted to floor
                     i_r_to_dm_dist.push_back(i_dist);  // allocate the index for the distance
                     n_index_tot.push_back(jdm);        // allocat the id of the dm particle
                     count++;
                   }
         ...
         size_t rank=0;  // OJO ACA: 0 es el mas cercano, 1 es el segundo más cercano
         sort_1d_vectors<size_t>(i_r_to_dm_dist, rank); // sort the container with distance indices
     ...
     size_t Nobjects_mock_rc=0;

     this->catalogue.resize_positions(this->catalogue._NOBJS());    
     this->catalogue.resize_velocities(this->catalogue._NOBJS());    
     this->catalogue.identity.resize(this->catalogue._NOBJS());    
     this->catalogue.GridID.resize(this->catalogue._NOBJS());    
...
// Problems with this parallelization. LEAVE COMMENTED!!!!!!!!!1
         ...
         if(MOCK_DEN_FIELD[id]>0)
           if(true==dm_used[id])
             for(int in=0;in < cell_inf_dm[id].posx_p.size(); ++in)
               {
                 this->catalogue.coord1[Nobjects_mock_rc] = cell_inf_dm[id].posx_p[in];
                 this->catalogue.coord2[Nobjects_mock_rc] = cell_inf_dm[id].posy_p[in];
                 this->catalogue.coord3[Nobjects_mock_rc] = cell_inf_dm[id].posz_p[in];
#ifndef _FCOL_TESTS_
                 this->catalogue.vel1[Nobjects_mock_rc] = cell_inf_dm[id].velx_p[in];
                 this->catalogue.vel2[Nobjects_mock_rc] = cell_inf_dm[id].vely_p[in];
                 this->catalogue.vel3[Nobjects_mock_rc] = cell_inf_dm[id].velz_p[in];
#endif
                 this->catalogue.identity[Nobjects_mock_rc] = ONE ;
                 this->catalogue.GridID[Nobjects_mock_rc] = cell_inf_dm[id].id_p[in];
                 Nobjects_mock_rc++;
               }
       }
     So.DONE();
...
#endif
     this->catalogue.Ntracers_dm=Nobjects_mock_rc;
     this->catalogue_ref.Ntracers_dm=Nobjects_mock_rc;
     this->catalogue.fraction_tracer_from_dm=static_cast<real_prec>(Nobjects_mock_rc)/(static_cast<real_prec>(Nobjects_mock_rc)+static_cast<real_prec>(Nrandoms1)) ;
     this->catalogue_ref.fraction_tracer_from_dm=this->catalogue.fraction_tracer_from_dm;

#ifdef _FULL_VERBOSE_
     So.message_screen("(", 100.0*this->catalogue.fraction_tracer_from_dm, "%)");
#ifdef _VERBOSE_FREEMEM_
     So.message_screen("Freeing memory");
#endif
#endif
     ...
     So.DONE();
     size_t N_dms_b= Nobjects_mock_rc;
     if(Nrandoms1>0)
       {
#ifdef _FULL_VERBOSE_
...
//#endif
         for(size_t id=0;id< this->params._NGRID();++id)
           {
             if(MOCK_DEN_FIELD[id]>0)
               if(true==random_used[id])
                 for(size_t in=0;in<cell_inf_ran[id].posx_p.size();++in)
                   {
                     this->catalogue.Halo[Nobjects_mock_rc].coord1=cell_inf_ran[id].posx_p[in];
                     this->catalogue.Halo[Nobjects_mock_rc].coord2=cell_inf_ran[id].posy_p[in];
                     this->catalogue.Halo[Nobjects_mock_rc].coord3=cell_inf_ran[id].posz_p[in];
#ifndef _FCOL_TESTS_
                     this->catalogue.Halo[Nobjects_mock_rc].vel1=cell_inf_ran[id].velx_p[in];
                     this->catalogue.Halo[Nobjects_mock_rc].vel2=cell_inf_ran[id].vely_p[in];
                     this->catalogue.Halo[Nobjects_mock_rc].vel3=cell_inf_ran[id].velz_p[in];
#endif
                     this->catalogue.Halo[Nobjects_mock_rc].identity=-1 ;
                     this->catalogue.Halo[Nobjects_mock_rc].GridID=cell_inf_ran[id].id_p[in];
         ...
         this->catalogue.Ntracers_ran=Nrandoms1;
         this->catalogue_ref.Ntracers_ran=Nrandoms1;
         this->catalogue.fraction_tracer_from_random=static_cast<real_prec>(Nrandoms1)/(static_cast<real_prec>(N_dms_b)+static_cast<real_prec>(Nrandoms1));
         this->catalogue_ref.fraction_tracer_from_random=this->catalogue.fraction_tracer_from_random;
         So.message_screen("(", 100.0*this->catalogue.fraction_tracer_from_random, "%)");
#ifdef _VERBOSE_FREEMEM_
         So.message_screen("Freeing memory");
#endif
#endif
...
//#endif
     for(size_t i=0; i<Ntracers; ++i)
       {
         if(this->catalogue.identity_at(i)<0)  //This means, use the randoms
           {
             ran_id_global.push_back(i);
             //total index (i.e, from 0 to N_dms) of the closest dm particle to the ramdom i. Useful to retrieve coordinates
             size_t index_dm_closer_a=dm_index_closer_tot[counter];
             ...
             this->catalogue.coord1_at(i)=new_distance*sin(theta)*cos(phi)+xdm;
             this->catalogue.coord2_at(i)=new_distance*sin(theta)*sin(phi)+ydm;
             this->catalogue.coord3_at(i)=new_distance*cos(theta)+zdm;
             counter++;
             }
       }
...
/*         real_prec alpha_max=alpha<M_PI ? (M_PI-alpha): alpha;
     ...
     // This space was meant to allocate the vector of struturures catalogue_ref.Halo[] with the same elements of the catalogue.Halo
...
#endif
#ifdef _ASSIGN_TO_CALIBRATION_
     this->catalogue.type_of_object="TRACER_MOCK_ONLY_COORDS";
#else
     this->catalogue.set_type_of_object("TRACER_MOCK");
#endif
     string primary_prop;
...
#pragma omp simd
    ...
    for(size_t i=0; i<this->catalogue_ref._NOBJS();++i)
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
      this->htools_ref->VMAX_parent[i]=this->htools_ref->vmax_at(i);
#else
        this->catalogue_ref.mass_parent[i]=this->catalogue_ref.mass_at(i);
#endif
#endif
...
#pragma omp simd
    ...
    for(size_t i=0; i<this->catalogue_ref._NOBJS();++i)
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
      this->catalogue.vmax_assigned[i]=this->catalogue.vmax_at(i);
#else
      this->catalogue.mass_assigned[i]=this->catalogue.mass_at(i);
#endif
...
#pragma omp parallel for reduction (+:ideal_corr)
    for(size_t i=0; i<this->catalogue._NOBJS();++i)
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
        ideal_corr+=pow(log10(this->catalogue.vmax_at(i)) - log10(this->htools_ref->VMAX_parent[i]),2);
#else
        ideal_corr+=pow(log10(this->catalogue.mass_at(i)) - log10(this->catalogue_ref.mass_parent[i]),2);
#endif
    ideal_corr=sqrt(ideal_corr/static_cast<real_prec>(this->catalogue._NOBJS()));
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
...
#pragma omp parallel for reduction (+:ideal_corr)
    ...
    size_t Nreal = 1;
    for (size_t ir=0;ir<Nreal;++ir)
      {
#ifdef _USE_OMP_re
        vector<size_t>vseeds(_NTHREADS_,0);
        for(size_t i=0;i<vseeds.size();++i)
          vseeds[i]=353*ir+static_cast<size_t>(i)*53215*pow(ir,3);
#endif
        counter_swap=0;
...
#pragma omp for reduction(+:counter_swap)
#endif
              for(size_t idm=0; idm<this->dm_properties_bins_mock.size();++idm)
                {
                  size_t Number_of_props_in_Theta_bin=this->dm_properties_bins_mock[idm].tracer_properties.size();
                  if(Number_of_props_in_Theta_bin>0)
                    {
                      vector<size_t>prop_bin_counter(pNbins);//* for histograms of prop within each theta bin *//
                      vector<vector<size_t>> id_in_propbin(pNbins*pNbins);//* to allocate gals ID in bins of main propery *//
                      for(size_t idm_tracer=0;idm_tracer<Number_of_props_in_Theta_bin;++idm_tracer)
                        {
                          size_t ig=this->dm_properties_bins_mock[idm].GalID_bin_properties[idm_tracer];
                          //* Get the value of vmax of the tracer: *//
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                       //* Get bin of primary_property from reference or parent *//
                          size_t ibin_prop=get_bin(log10(this->catalogue_ref.vmax_at(ig)),log10(this->params._VMAXmin()),pNbins,delta_PROP,true);
                  //* Get the value of vmax of the tracer_ref:  *//
                  //* Get bin of vmax *//
                         size_t jbin_prop=get_bin(log10(this->catalogue.vmax_at(ig)),log10(this->params._VMAXmin()),pNbins,delta_PROP,true);
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
                      //* Get bin of vmax *//
                          size_t ibin_prop = get_bin(log10(this->catalogue_ref.mass_at(ig)),this->params._LOGMASSmin(),pNbins,delta_PROP,true);
                      //* Get bin of vmax *//
                          size_t jbin_prop = get_bin(log10(this->catalogue.mass_at(ig)),this->params._LOGMASSmin(),pNbins,delta_PROP,true);
#endif
			  ...
			  //* Read the id of the chosen tracer *//
                      size_t ig=this->dm_properties_bins_mock[idm].GalID_bin_properties[idm_tracer];
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                      real_prec vmax_tracer     =  this->catalogue.vmax_at(ig);//* get the value of vmax assigned to the tracer, renewed below swaped from another bin *//
                      real_prec vmax_tracer_ref =  this->catalogue_ref.vmax_at(ig); //* get the value of the original vmax of the tracer *//
                      size_t ibin_prop = get_bin(log10(vmax_tracer_ref),log10(this->params._VMAXmin()),pNbins,delta_PROP,true); //* get i bin of vmax *//
                      size_t jbin_prop = get_bin(log10(vmax_tracer),log10(this->params._VMAXmin()),pNbins,delta_PROP,true); //* get j bin of vmax *//
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
                      real_prec mass_tracer     =  this->catalogue.mass_at(ig);//* get the value of vmax assigned to the tracer, renewed below swaped from another bin *//
                      real_prec mass_tracer_ref =  this->catalogue_ref.mass_at(ig); //* get the value of the original vmax of the tracer *//
                      size_t ibin_prop = get_bin(log10(mass_tracer_ref),this->params._LOGMASSmin(),pNbins,delta_PROP,true); //* get i bin of prop in reference *//
                      size_t jbin_prop = get_bin(log10(mass_tracer),this->params._LOGMASSmin(),pNbins,delta_PROP,true); //* get j bin of prop in  *//
#endif
              //* This defines the diagonal bins as exlusion zone, i.e, we do not swap from those bins *//
                  ...
                  // elige el número de bins en prop a saltar hacia arriba. Este va desde 1 hasta el i-bin:(ver dibujo)
                      size_t diff = jbin_prop>ibin_prop ? jbin_prop-ibin_prop : ibin_prop-jbin_prop;
                      size_t ncells_jumps= diff>1 ? 2 + gsl_rng_uniform_int(gBaseRands, diff)  : 0 ;
                  if(ncells_jumps>0)
                    {
                      ...
                      if(new_jbin_prop<0)//* do not go beyond the numbre of bins  *//
                        new_jbin_prop=0;
                      size_t Ntracers_in_new_prop_jbin=prop_bin_counter[new_jbin_prop];//* Get the number of objects in the new j-bin *//
				      ...
				      //* Starting bin of new i array: *//
                          size_t baux= (dir_jump> 0 ? 0   : new_jbin_prop-(Exclusion_bins-1));
				      //* Number of bins in the i-dir to use: this is aiming at selecting a new i bin such that when swaping, we do not go above (dir>0) oe below the diagonal*//
                          size_t naux= (dir_jump> 0 ? ibin_prop-(Exclusion_bins-1) : pNbins-baux); //* asegurarse de qe del lado de la diagonal tenemos celdas con tracers: //
                          vector<size_t>noeb;
                          for(size_t ip=baux;ip<naux;++ip)
                            if(id_in_propbin[index_2d(ip,new_jbin_prop,pNbins)].size()>0)
                              noeb.push_back(ip);
                          if(noeb.size()>0)
                            {
                               size_t new_ibin_prop=noeb[gsl_rng_uniform_int(gBaseRands,noeb.size())];
                               size_t N_props_in_new_ijbin=id_in_propbin[index_2d(new_ibin_prop,new_jbin_prop,pNbins)].size();
                               if(N_props_in_new_ijbin>0)
                                {
                                  size_t idprop=gsl_rng_uniform_int(gBaseRands,N_props_in_new_ijbin);
                                  size_t new_ig = id_in_propbin[index_2d(new_ibin_prop,new_jbin_prop,pNbins)][idprop];
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                                  real_prec vmax_tracer_new= this->catalogue.Halo[new_ig].vmax;
                                  this->catalogue.vmax_at(ig)=vmax_tracer_new; //* SWAP *//
                                  this->catalogue.Halo[new_ig].vmax=vmax_tracer; //* SWAP *//
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
                                  real_prec mass_tracer_new= this->catalogue.Halo[new_ig].mass;
                                  this->catalogue.mass_at(ig)=mass_tracer_new; //* SWAP *//
                                  this->catalogue.Halo[new_ig].mass=mass_tracer; //* SWAP *//
#endif
                                  counter_swap++;
			...
			}//* close for(size_t idm_tracer=0 *//
		    }//* close  if(Number_of_props_in_Theta_bin>0) *//
		}//* close  for(size_t idm=0.. *//
#ifdef _USE_OMP_re
...
#pragma omp parallel for reduction(+:ecorr)
	    for(size_t i=0;i<this->catalogue._NOBJS(); ++i)
	      {
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
        ecorr+=pow(log10(this->catalogue.vmax_at(i))-log10(this->htools_ref->VMAX_parent[i]),2);
#else
         ecorr+=pow(log10(this->catalogue.mass_at(i))-log10(this->catalogue_ref.mass_parent[i]),2);
#endif
        }
        corr=sqrt(ecorr/static_cast<real_prec>(this->catalogue._NOBJS()));
        real_prec ebias=0;
#pragma omp parallel for reduction(+:ebias)
	    for(size_t i=0;i<this->catalogue._NOBJS(); ++i)
	      {
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
            ebias+=this->catalogue.vmax_at(i)/this->htools_ref->VMAX_parent[i];
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
            ebias+=this->catalogue.mass_at(i)/this->catalogue_ref.mass_parent[i];
#endif
         }
        ebias=sqrt(ebias/static_cast<real_prec>(this->catalogue._NOBJS()));
        real_prec sbias=0;
#pragma omp parallel for reduction(+:ebias)
	    for(size_t i=0;i<this->catalogue._NOBJS(); ++i)
	      {
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
            sbias+=pow(this->catalogue.vmax_at(i)/this->htools_ref->VMAX_parent[i]-1,2);
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
            sbias+=pow(this->catalogue.mass_at(i)/this->catalogue_ref.mass_parent[i]-1,2);
#endif
	      }
        sbias=sqrt(sbias/static_cast<real_prec>(this->catalogue._NOBJS()));
	...
	for(size_t idm=0; idm<this->dm_properties_bins_mock.size();++idm)
	  {
	    size_t Number_of_props_in_Theta_bin=this->dm_properties_bins_mock[idm].tracer_properties.size();
            ...
            vector<size_t> prop_bin_counter(pNbins,0);
	    if(Number_of_props_in_Theta_bin>0)
	      {
		for(size_t idm_tracer=0;idm_tracer<Number_of_props_in_Theta_bin;++idm_tracer)
		  {
		    size_t ig=this->dm_properties_bins_mock[idm].GalID_bin_properties[idm_tracer];
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                    prop_bin_counter[get_bin(log10(this->catalogue.Halo_at(ig).vmax_assigned),log10(this->params._VMAXmin()),pNbins,delta_PROP,true)]++;
#else
                    prop_bin_counter[get_bin(log10(this->catalogue.Halo_at(ig).mass_assigned),this->params._LOGMASSmin(),pNbins,delta_PROP,true)]++;
#endif
                }
		for(size_t idm_tracer=0;idm_tracer<Number_of_props_in_Theta_bin;++idm_tracer)
		  {
...
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
                    size_t jbin_prop_assigned=get_bin(log10(this->catalogue.Halo_at(ig).vmax_assigned),log10(this->params._VMAXmin()),pNbins,delta_PROP,true);
                    size_t jbin_prop_new=get_bin(log10(this->catalogue.vmax_at(ig)),log10(this->params._VMAXmin()),pNbins,delta_PROP,true);
#else
                    size_t jbin_prop_assigned=get_bin(log10(this->catalogue.Halo_at(ig).mass_assigned),this->params._LOGMASSmin(),pNbins,delta_PROP,true);
                    size_t jbin_prop_new=get_bin(log10(this->catalogue.mass_at(ig)),this->params._LOGMASSmin(),pNbins,delta_PROP,true);
#endif
...
#pragma omp parallel for
	for(size_t i=0; i<this->catalogue._NOBJS();++i)
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
          this->catalogue.vmax_at(i)=this->catalogue.vmax_assigned[i];
#else
            this->catalogue.mass_at(i)=this->catalogue.mass_assigned[i];
#endif
...
#pragma omp parallel private (jthread, gBaseRands, rng_s)
	{
	  ...
	  rng_s = gsl_rng_mt19937;//_default;
	  gBaseRands = gsl_rng_alloc (rng_s);
      vector<bool> swap(this->catalogue._NOBJS(), false);
#ifdef _USE_OMP_rea
#pragma omp for
#endif
	  for(size_t idm=0; idm<this->dm_properties_bins_mock.size();++idm)
	    {
         ...
         if(Number_of_props_in_Theta_bin>0)
		{
          vector<vector<size_t>> id_in_propbin(pNbins);
           vector<size_t> prop_bin_counter(pNbins,0);
          for(size_t idm_tracer=0;idm_tracer<Number_of_props_in_Theta_bin;++idm_tracer)
		    {
		      size_t ig=this->dm_properties_bins_mock[idm].GalID_bin_properties[idm_tracer];
		      real_prec vmax_tracer = this->catalogue.Halo_at(ig).vmax_assigned;//get the value of vmax assigned to  the tracer, renewed below swaped from another bin
                      size_t jbin_prop = get_bin(log10(vmax_tracer),log10(this->params._VMAXmin()),pNbins,delta_PROP,true); // get j bin of vmax
		  ...
		  vector<vector<size_t>>ig_up(pNbins*pNbins);
                  //* Container to allocate ID os galaxies which in each vmax bin went down*/
		  vector<vector<size_t>>ig_down(pNbins*pNbins);
          //* Do loop over bins of vmax *//
		  for(size_t iv=0;iv<pNbins;++iv)
		    {
		      //* In this bin: *//
		      //* Identify the number of tracers *//
                      size_t Ntracers_vbin=id_in_propbin[iv].size();
		      if(Ntracers_vbin>0)
			{
           ...
           //* Choose the tracers that in the bin iv went up to different bins .//
              ...
              size_t Nup_m=0;
			  for(size_t iav=0;iav<pNbins;++iav)
			    Nup_m+=this->dm_properties_bins_mock[idm].Pfj_upward[index_2d(iv,iav,pNbins)];	  
              if(Nup_m>0)
			    {
			      for(size_t iv_up=iv+1;iv_up<pNbins;++iv_up)
				{
				  size_t index=index_2d(iv,iv_up,pNbins);
                                  // Number of tracers going up
				  size_t Ntracers_bin_bin_up=static_cast<size_t>(this->dm_properties_bins_mock[idm].Pfj_upward[index]);
                                  //Number of tracers in th bin j j'
                 size_t Ntracers_bin_bin_all=static_cast<size_t>(this->dm_properties_bins_mock[idm].Pjj[index]);
                  if(Ntracers_bin_bin_up>0)
				      ...
				      while(counter_du<Ntracers_bin_bin_up)
					{
					  ...
					  //* And here is where we can mix things.*//
                                          size_t igr = gsl_rng_uniform_int(gBaseRands,Ntracers_vbin);
                                          size_t gal_id=id_in_propbin[iv][igr];
					  ...
					  counter_du++;
                          ...
                          for(size_t ip=0;ip<sel.size();++ip)
                            if(false==sel[ip])
                              counter_partial++;
                          ...
                          if(Ndown_m>0)
			    {
                              for(size_t iv_down=0;iv_down<iv ;++iv_down)
				{
				  size_t index=index_2d(iv,iv_down,pNbins);
				  size_t Ntracers_bin_bin_down=static_cast<size_t>(this->dm_properties_bins_mock[idm].Pfj_downward[index]);
                                  size_t Ntracers_bin_bin_all=static_cast<size_t>(this->dm_properties_bins_mock[idm].Pjj[index]);
				  if(Ntracers_bin_bin_down>0)
					...
					{
                                          size_t igr = gsl_rng_uniform_int(gBaseRands,Ntracers_vbin);//Ntracers_bin_bin_all
                                          size_t gal_id=id_in_propbin[iv][igr];
//                                          if(false==sel[gal_id])
					    {
		  ...
		  for(size_t iv_parent=0;iv_parent<pNbins-1;++iv_parent)
		    {
                      size_t Ntracers_vbin=id_in_propbin[iv_parent].size();
		      if(Ntracers_vbin>0)
			{
			  for(size_t iv_final=0;iv_final<pNbins-1;++iv_final)
			    {
			      size_t index_parent_final=index_2d(iv_parent, iv_final, pNbins);
			      //* Number of tracers going up in from bin iv_parent to bin iv_final*//
			      size_t Ntracers_up = ig_up[index_parent_final].size();
			      //* Loop over the tracers ready to go-up from the iv_parent to the bin iv_final: *//
			      for(size_t i_up = 0; i_up< Ntracers_up;++i_up)
				{
				  //* For the current tracer to be pushed-up: *//
				  size_t Ig_parent=ig_up[index_parent_final][i_up];
				  //if(false==swap[Ig_parent])
				    {
				      //* Read its vmax *//
				      real_prec vmax_parent = this->catalogue.Halo[Ig_parent].vmax_assigned;
				      ...
				      if(ig_down[index_parent_final].size()>0)
					{
					  ...
					  size_t Ig_new = ig_down[index_parent_final][gsl_rng_uniform_int(gBaseRands,ig_down[index_parent_final].size())];
					  //* Read its vmax *//
					  real_prec vmax_new=this->catalogue.Halo[Ig_new].vmax_assigned;
					  //swap only if these have not been swaped before
					  //if(false==swap[Ig_new])
					    {
					      //					      cout<<iv_parent<<"  "<<iv_final<<endl;
					      this->catalogue.Halo[Ig_new].vmax_assigned=vmax_parent;
					      this->catalogue.Halo[Ig_parent].vmax_assigned=vmax_new;
					      swap[Ig_new]=true;
					      swap[Ig_parent]=true;
					      counter_swap++;
					    }//* closes if*/
		  ...
		  for(size_t iv_parent=pNbins-1;iv_parent--> 1;)
		    {
                      size_t Ntracers_vbin=id_in_propbin[iv_parent].size();
		      if(Ntracers_vbin>0)
			{
			  for(size_t iv_final=0;iv_final<iv_parent;++iv_final)
			    {
			      size_t index_parent_final=index_2d(iv_parent, iv_final, pNbins);
			      
			      size_t Ntracers_down=ig_down[index_parent_final].size();
			      //* Loop over the tracers ready to go-up
			      for(size_t i_down=0; i_down< Ntracers_down;++i_down)
				{
				  //* Select one tracer to be pushed-up:
				  size_t Ig_parent=ig_down[index_parent_final][i_down];
				  if(false==swap[Ig_parent])
				    {
				      real_prec vmax_parent=this->catalogue.Halo[Ig_parent].vmax_assigned;
				      //* New bin down: *//
					...
					{
					  size_t Ig_new = ig_up[index_parent_final][gsl_rng_uniform_int(gBaseRands,ig_up[index_parent_final].size())];
					  real_prec vmax_new = this->catalogue.Halo[Ig_new].vmax_assigned;
					  //swap:
					  if(false==swap[Ig_new])
					    {
...
#pragma omp parallel for reduction(+:ecorr)
    for(size_t i=0;i<this->catalogue._NOBJS(); ++i)
      ecorr+=pow(this->catalogue.vmax_at(i)-this->htools_ref->VMAX_parent[i],2);
    real_prec corr=sqrt(ecorr/static_cast<real_prec>(this->catalogue._NOBJS()));
...
#ifdef _USE_GNUPLOT_
    this->gp_abundance_v<<"set log x \n";
    this->htools->params.set_i_spin_g(NEGATIVE_INT); //no spin information here
    this->htools->params.set_i_rs_g(NEGATIVE_INT); //no rs information here
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
    this->htools->params.set_i_vmax_g(POSITIVE_INT); //this allows the tracer to get the vmax function
    this->htools->params.set_i_mass_g(NEGATIVE_INT); //no mass information here
#elif defined _USE_MASS_AS_PRIMARY_OBSERVABLE_
    this->htools->params.set_i_vmax_g(NEGATIVE_INT); //no vmax  information here
    this->htools->params.set_i_mass_g(POSITIVE_INT); //this allows the tracer to get the mass function
#endif
    this->htools->get_property_function(this->params._Output_directory()+"test");
    this->htools_ref->get_property_function(this->params._Output_directory()+"test");
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
    this->gp_abundance_v<<"set xlabel 'Vrms [km/s]' font 'Times-Roman,15' textcolor rgb '"<<FG_COLOR<<"' \n";
    this->gp_abundance_v<<"set ylabel 'log n(Vrms)' font 'Times-Roman,15' textcolor rgb '"<<FG_COLOR<<"' \n";
    vector<pair<real_prec, real_prec> > xy_pts_v;
...
//    exit(0);
    ...
    this->catalogue_ref.set_type_of_object("TRACER_REF"); // THis name is important.
    this->catalogue.set_type_of_object("TRACER_MOCK"); // THis name is important.
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
#ifdef _ASSIGN_MASS_LINKED_TO_V_FROM_REF_
    ...
    this->catalogue.get_distribution_min_separations(this->ncells_info);
#endif
#ifdef _ASSIGN_PROPERTIES_TO_REFERENCE_
#ifdef _USE_GNUPLOT_SREL_
...
//------------RS-----------------------------
   ...
   if(this->catalogue_ref._NOBJS()>0)// We kept tracer_ref in memory for comparisons of bias. Here we clar it and read  reference catalog below if we need it again
   {
...
//------------Cvir-----------------------------
   ...
   if(this->catalogue_ref.coord1.size()>0)// We kept tracer_ref in memory for comparisons of bias. Here we clar it and read  reference catalog below if we need it again
   {
     So.message_screen("**Freeing memmory from tracer_ref, line", __LINE__);
     this->catalogue_ref.clear_mem();
   }
...
// ******** here we correct for mean vels and boundary conditions
    ...
    for(size_t i = 0; i< this->catalogue._NOBJS(); ++i)
    {
      if(this->catalogue.coord1_at(i)>this->params._Lbox())
        this->catalogue.coord1_at(i)-=this->params._Lbox();
      if(this->catalogue.coord2_at(i)>this->params._Lbox())
        this->catalogue.coord2_at(i)-=this->params._Lbox();
      if(this->catalogue.coord3_at(i)>this->params._Lbox())
        this->catalogue.coord3_at(i)-=this->params._Lbox();
      if(this->catalogue.coord1_at(i)<0)
        this->catalogue.coord1_at(i)+=this->params._Lbox();
      if(this->catalogue.coord2_at(i)<0)
        this->catalogue.coord2_at(i)+=this->params._Lbox();
      if(this->catalogue.coord3_at(i)<0)
        this->catalogue.coord3_at(i)+=this->params._Lbox();
    }
    this->So.DONE();
#endif
...
#pragma omp parallel for reduction(+:meanvel)
    for(size_t i = 0; i< this->catalogue._NOBJS(); ++i)
        meanvel+=this->catalogue.vel1_at(i);
    meanvel/=static_cast<real_prec>(this->catalogue._NOBJS());
#pragma omp parallel for
    for(size_t i = 0; i< this->catalogue._NOBJS(); ++i)
        this->catalogue.vel1_at(i)-=meanvel;
    meanvel=0;
#pragma omp parallel for reduction(+:meanvel)
    for(size_t i = 0; i< this->catalogue._NOBJS(); ++i)
        meanvel+=this->catalogue.vel2_at(i);
    meanvel/=static_cast<real_prec>(this->catalogue._NOBJS());
#pragma omp parallel for
    for(size_t i = 0; i< this->catalogue._NOBJS(); ++i)
        this->catalogue.vel2_at(i)-=meanvel;
    meanvel=0;
#pragma omp parallel for reduction(+:meanvel)
    for(size_t i = 0; i< this->catalogue._NOBJS(); ++i)
        meanvel+=this->catalogue.vel3_at(i);
    meanvel/=static_cast<real_prec>(this->catalogue._NOBJS());
#pragma omp parallel for
    for(size_t i = 0; i< this->catalogue._NOBJS(); ++i)
        this->catalogue.vel3_at(i)-=meanvel;
...
// Herew we get the power spectrum from the catalog, before actually writting it as an output.
    ...
    this->catalogue.set_type_of_object("TRACER_MOCK"); // THis name is important.
#ifdef _FCOL_TESTS_
    size_t ifcol=static_cast<size_t>(floor(1000.*this->params._Distance_fraction()));
...
#pragma omp parallel for
	...
	if (this->catalogue.mass_at(i) >1e12  && this->catalogue.mass_at(i) <1e13)
	  ibvel=0.9;
	else if (this->catalogue.mass_at(i) >=1e13)
	  ibvel=0.83;
#endif //* end of _CORRECT_VEL_HALOMASS_
	int cwc_exponent=this->catalogue.Halo[i].gal_cwt;
	real_prec density=pow(abs(1.00+this->catalogue.Halo[i].local_dm),corr_den[cwc_exponent]);
	this->catalogue.vel1_at(i)*=density*ibvel;
	this->catalogue.vel2_at(i)*=density*ibvel;
	this->catalogue.vel3_at(i)*=density*ibvel;
      }
#endif //* end of _CORRECT_VEL_DENSITY_
    //* If no properties are assigned, we might still want to use the cwt of each catalogue. THis is done in get_scaling_relations_primary_property and _assign_property_new_new *//
...
#pragma omp parallel for
#endif
    for(size_t i=0;i<this->catalogue._NOBJS();++i)
        this->catalogue.Halo[i].gal_cwt=this->cwclass.CWClass[this->catalogue.Halo[i].GridID];
#endif
    ...
    for(size_t i=0;i<this->catalogue._NOBJS();++i)
      {
	if(this->catalogue.Halo[i].gal_cwt==WebType::Knot)
	  {
	    //	    real_prec epsilon=
	    // This otion adds scaltter to individual vel components
	    this->catalogue.vel1_at(i)+=gsl_ran_gaussian(gBaseRand_p,this->params._slengthv());
	    this->catalogue.vel2_at(i)+=gsl_ran_gaussian(gBaseRand_p,this->params._slengthv());
	    this->catalogue.vel3_at(i)+=gsl_ran_gaussian(gBaseRand_p,this->params._slengthv());
	    // this option adds scatte to the mvelocity without changing the direction
	    /*
	      real_prec vmod=sqrt(pow(this->catalogue.vel1_at(i),2)+pow(this->catalogue.vel2_at(i),2)+pow(this->catalogue.vel3_at(i),2));
	      real_prec vmod_ep=(vmod+epsilon)/static_cast<real_prec>(vmod);
	      this->catalogue.vel1_at(i)*=vmod_ep;
	      this->catalogue.vel2_at(i)*=vmod_ep;
	      this->catalogue.vel3_at(i)*=vmod_ep;
	    */
	  }
      }
#endif
...
#ifdef _COMPUTE_REF_POWER_
    Catalog acatalogue_ref;
#ifdef _SLICS_
      if(this->params._IC_index()<1000)
        {
#endif
        acatalogue_ref.set_params(this->params_original);
        acatalogue_ref.read_catalog(this->params_original._Input_dir_cat()+this->params_original._file_catalogue(),pow(10,this->params._LOGMASSmin())*this->params._MASS_units());
#ifdef _CAT_POWER_ONLY_COORDS_
        Power_BiasMT_ref.measure_power_spectrum(acatalogue_ref,"real","_NONE_"); //*Get power*//
#else
        Power_BiasMT_ref.measure_power_spectrum(acatalogue_ref,"real",_MASS_); //* Get the reference power in mass bins*//
#endif
#ifdef _SLICS_
	}
#endif
#endif //* end for _COMPUTE_REF_POWER_ *//
#ifdef _CAT_POWER_ONLY_COORDS_
    Power_BiasMT.measure_power_spectrum(this->catalogue,"real","_NONE_"); //*Get power*//
#else
    Power_BiasMT.measure_power_spectrum(this->catalogue,"real",_MASS_); //* Get the REFERENCE power in mass bins*//
#endif
#ifdef _USE_GNUPLOT_
#ifdef _COMPUTE_REF_POWER_
#ifdef _SLICS_
...
#ifdef _COMPUTE_REF_POWER_
      Power_BiasMT_ref.measure_power_spectrum(acatalogue_ref.Halo,"real",_VMAX_); //* Get the reference power in vmax bins*//
#endif
      Power_BiasMT.measure_power_spectrum(this->catalogue.Halo,"real",_VMAX_); //* Get the bam power in vmax bins*//
#ifdef _USE_GNUPLOT_
...
#ifndef _CAT_POWER_ONLY_COORDS_
      // *******************
      Power_BiasMT.measure_power_spectrum(this->catalogue.Halo,"redshift",_MASS_);
#ifdef _COMPUTE_REF_POWER_
      Power_BiasMT_ref.measure_power_spectrum(acatalogue_ref.Halo,"redshift",_MASS_);
#endif
#ifdef _USE_GNUPLOT_
      this->plot_power(Power_BiasMT_ref.power_in_bins,Power_BiasMT.power_in_bins, "redshift",_MASS_);
#endif

      Power_BiasMT.measure_power_spectrum(this->catalogue.Halo,"redshift",_VMAX_);
#ifdef _COMPUTE_REF_POWER_
      Power_BiasMT_ref.measure_power_spectrum(acatalogue_ref.Halo,"redshift",_VMAX_);
#endif
#ifdef _USE_GNUPLOT_
      ...
      Power_BiasMT.measure_power_spectrum(this->catalogue.Halo,"redshift","_NONE_");
#ifdef _COMPUTE_REF_POWER_
      Power_BiasMT_ref.measure_power_spectrum(acatalogue_ref.Halo,"redshift","_NONE_");
#endif
  ...
  this->catalogue.write_catalog(outputFileName.c_str());
#ifdef _VERBOSE_FREEMEM_
         So.message_screen("Freeing memmory from tracer");
#endif
     this->catalogue.clear_mem();
     So.DONE();
...
void BiasMT::correct_for_exclusion(size_t LENGHTdm){
  So.enter(__PRETTY_FUNCTION__);
  ...
  Tn = gsl_rng_default;
  rn = gsl_rng_alloc (Tn);
  this->catalogue.masses_in_cells_min_sep.clear();
  this->catalogue.masses_in_cells_min_sep.shrink_to_fit();
  this->catalogue.masses_in_cells_min_sep.resize(LENGHTdm);
  vector<int>number_in_theta_mock(LENGHTdm,0);
#pragma omp parallel for
  for(size_t i=0;i<LENGHTdm; ++i)
    number_in_theta_mock[i]=this->dm_properties_bins_mock[i].tracer_properties.size();
  So.DONE();
  // Obtain the list of pairs of masses allocated in a bin of theta that are within a distance min_halo_sep, EXCLUSION_SCALE. output in  this->catalogue.masses_in_cells_min_sep
  this->catalogue.get_masses_of_pairs_in_min_separation_bin_in_theta_bin(this->min_halo_separation, this->dm_properties_bins_mock);
  for(int ih=0;ih< LENGHTdm;++ih) //Loop over the bins in Theta
    {
      ...
      for(int j=0; j< this->catalogue_ref.masses_in_cells_min_sep[ih].M1.size();++j)
        {
          M1_max_ref=max(this->catalogue_ref.masses_in_cells_min_sep[ih].M1[j], M1_max_aux);
          M1_max_aux=M1_max_ref;
          M2_max_ref=max(this->catalogue_ref.masses_in_cells_min_sep[ih].M2[j], M1_max_aux);
      ...
      for(int j=0; j< this->catalogue.masses_in_cells_min_sep[ih].M1.size();++j)
        {
          real_prec M1=this->catalogue.masses_in_cells_min_sep[ih].M1[j];
          if(M1>M1_max_ref) //allocate the galID of this mass
            this->catalogue.masses_in_cells_min_sep[ih].mass_to_swap.push_back(this->dm_properties_bins_mock[ih].GalID_bin_properties[j]);
        }
    }
  ...
  for(int ih=0;ih< LENGHTdm;++ih)//Loop over the bins in Theta
    {
      ...
      for(int j=0; j< this->catalogue.masses_in_cells_min_sep[ih].mass_to_swap.size();++j)//loopover the masses to be swaped
        {
          size_t old_ig=this->catalogue.masses_in_cells_min_sep[ih].mass_to_swap[j]; // galaxy index IDg [0,NOBJS) of the mass requested to be swaped
          real_prec old_mass=this->catalogue.Halo[old_ig].mass; //mass requested to be swaped
          bool baux=false;
          while(baux==false)
            {
              int i_mass_halo_label= gsl_rng_uniform_int(rn,N_props_in_bin); // pick-up randomly some other mass in the same bin-theta
              size_t new_ig=this->dm_properties_bins_mock[ih].
                      [i_mass_halo_label];  // galaxy index IDg [0,NOBJS) of an randomly selected object
              real_prec new_mass=this->catalogue.Halo[new_ig].mass; //mass of the randomly selected object
              //swap masses:
              if(new_ig!=this->catalogue.masses_in_cells_min_sep[ih].mass_to_swap[j])
                {
                  this->catalogue.Halo[old_ig].mass=new_mass; // assign mass
                  this->catalogue.Halo[new_ig].mass=old_mass;
...
void BiasMT::v_konvolve(vector<real_prec> &vel)
{
   // theta = nabla V. Getting V given theta is the main goal.
    ...
    //
   So.enter(__PRETTY_FUNCTION__ );
   size_t N= this->params._NGRID();
   real_prec Lzp1=this->params._Lbox();
   real_prec Lzp2=this->params._Lbox();
   real_prec Lzp3=this->params._Lbox();
   size_t Nzp1=this->params._Nft();
   size_t Nzp2=this->params._Nft();
   size_t Nzp3=this->params._Nft();
   size_t Nhalf=this->NTT;
#ifdef DOUBLE_PREC
       ...
       for (size_t k=0;k<this->params._Nft()/2+1;k++)
         {
           ...
           size_t index=index_3d(i,j,k,this->params._Nft(),this->params._Nft()/2+1);
...
#pragma omp parallel for
#endif
    for(size_t it=0; it <this->dm_properties_bins.size(); ++it)
    {
...
#pragma omp parallel for
#endif
   for(size_t it=0; it < this->dm_properties_bins.size(); ++it)  // We now sort all properties in the different theta bins from top-to-bottom
     {
       size_t ll=this->dm_properties_bins[it].tracer_properties.size();
       if(ll>0)
        {
...
#pragma omp parallel for
#endif
   for(size_t it=0; it < this->dm_properties_bins_mock.size(); ++it)  // We now sort all properties in the different theta bins from top-to-bottom
     {
       size_t ll=this->dm_properties_bins_mock[it].tracer_properties.size();
       if(ll>0)
        {
          gsl_vector *aux_properties=gsl_vector_alloc(ll);
          gsl_vector *aux_properties_id=gsl_vector_alloc(ll);
           for(size_t im=0; im< ll; ++im )
           {
           ...
           for(size_t im_new=0; im_new< ll; ++im_new )
            {
...
void BiasMT::swap_mock_properties_in_theta_bins(int asy){
  ...
  for(size_t ih=0;ih<this->dm_properties_bins_mock.size();++ih)
       {
         size_t size_dmp=this->dm_properties_bins_mock[ih].tracer_properties.size();
         if(size_dmp>1){
         int extra_n=0;
         if(size_dmp%2 !=0)extra_n=1;
         size_t nswaps= static_cast<size_t>(floor(size_dmp/2));
         for (size_t is=0;is<nswaps;++is){ // loop over the possible swaps in this theta bin
             real_prec prop_max=this->dm_properties_bins_mock[ih].tracer_properties[is]; // this is thye largest vmax value in this bin
             size_t ig_max=this->dm_properties_bins_mock[ih].GalID_bin_properties[is]; // this is the ID of the tracer with largest vmax value in this bin
             size_t is_low=nswaps*2-is+extra_n-1;
             real_prec prop_min=this->dm_properties_bins_mock[ih].tracer_properties[is_low]; // this is the lowest vmax value in this bin
             size_t ig_min=this->dm_properties_bins_mock[ih].GalID_bin_properties[is_low]; // this is the ID of the tracer with lowest vmax value in this bin
             //swap:
             this->catalogue.set_vmax(prop_max,ig_min);
             this->catalogue.set_vmax(prop_min,ig_max);
          }
        }
    }
    So.DONE();
...
void BiasMT::extrapolate_kernel(vector<real_prec>&kernel_old, vector<real_prec>&kernel_new){
  ...
  size_t Nft_low=this->params._Nft_low();
  vector<real_prec> coords_low(Nft_low,0);
#ifdef _USE_OMP_
#pragma omp parallel for
#endif
  for(size_t i=0;i<coords_low.size() ;++i)
  ...
  for(size_t i=0;i< Nft_low; ++i)
    {
      for(size_t j=0;j< Nft_low; ++j)
        {
          for(size_t k=0;k< Nft_low/2+1; ++k)
            {
              size_t ind=index_3d(i,j,k,Nft_low, Nft_low/2+1);
              ...
              size_t kmod=static_cast<int>(floor((kv-this->params._d_kmin_low())/this->params._d_DeltaK_data_low()));
  ...
  for(size_t i=0;i<kernel_1d.size(); ++i)
  ...
  for(size_t i=0;i< kernel_1d_new.size(); ++i)
    {
      size_t jlim=Nft/2-Nft_low/2;
  ...
  for(size_t i=0;i< Nft; ++i)
    for(size_t j=0;j< Nft; ++j)
      for(size_t k=0;k< Nft/2+1; ++k)
        {
          size_t ind=index_3d(i,j,k, Nft, Nft/2+1);
          real_prec kv=_get_modulo(coords[i]*this->params._d_deltak_x(),coords[j]*this->params._d_deltak_y(),coords[k]*this->params._d_deltak_z());
          size_t kmod=static_cast<size_t>(floor( (kv-this->params._d_kmin())/this->params._d_DeltaK_data()));
...
{
  size_t n=this->params._N_lines_binary();
#ifdef _FULL_VERBOSE_
  ...
  if(this->params._use_low_pass_filter())
  {
    ...
    So.message_screen("Mean from LR =", mean_field_lr);
        ...
        for(size_t i=1; i<cPSFa._kvector_data_size(); ++i)
        ...
        for(size_t i=1; i<cPSFb._kvector_data_size(); ++i)
...
void BiasMT::plot_power(vector<s_info_in_bins> power_in_bins_ref, vector<s_info_in_bins> power_in_bins, string prop, string space)
{
  ...
  if("real"==space)
	...
	{
	  std::vector<std::pair<double, double> > xy_pts;
	  for(size_t i=1; i< power_in_bins[im_bins].vbin.size(); ++i)
	  ...
	  for(size_t i=1; i< power_in_bins_ref[im_bins].vbin.size(); ++i)
...
void BiasMT::plot_scaling_relation_assignment(string prop){
  ...
  size_t Nobjs_fraction=static_cast<size_t>(floor(fraction_tracers_to_file*this->catalogue._NOBJS()));
  vector<pair<real_prec, real_prec> > xy_pts_ref;
  ...
  if(prop==_MASS_)
      ...
      do
	{
	  ...
	  xy_pts_ref.push_back(std::make_pair(this->catalogue_ref.mass_at(i), this->catalogue.mass_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  xy_pts_ref.push_back(std::make_pair(this->catalogue_ref.mass_at(i), this->catalogue.mass_at(i)));
	  xy_sr_ref.push_back(std::make_pair(this->catalogue_ref.mass_at(i), this->htools_ref->vmax_at(i)));
	  xy_sr_rec.push_back(std::make_pair(this->catalogue.mass_at(i), this->catalogue.vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Knot)
	    xy_pts_ref_k.push_back(std::make_pair(this->catalogue_ref.mass_at(i), this->catalogue.mass_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
	...
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Sheet)
	    xy_pts_ref_s.push_back(std::make_pair(this->catalogue_ref.mass_at(i), this->catalogue.mass_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
	...
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Filament)
	    xy_pts_ref_f.push_back(std::make_pair(this->catalogue_ref.mass_at(i), this->catalogue.mass_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);    
    }
  else if(_VMAX_==prop)
    {
      /*               for(size_t i=0;i<10000;++i)
		       xy_pts_ref.push_back(std::make_pair(this->htools_ref->vmax_at(i), this->catalogue.vmax_at(i)));
      */
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  // if(this->catalogue.Halo[i].GridID==393708){
	  xy_pts_ref.push_back(std::make_pair(this->htools_ref->vmax_at(i), this->catalogue.vmax_at(i)));
	  //  }
	  counter++;
	}while(counter<Nobjs_fraction);
      ...
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Knot)
	    xy_pts_ref_k.push_back(std::make_pair(this->htools_ref->vmax_at(i), this->catalogue.vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Sheet)
	    xy_pts_ref_s.push_back(std::make_pair(this->htools_ref->vmax_at(i), this->catalogue.vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Filament)
	    xy_pts_ref_f.push_back(std::make_pair(this->htools_ref->vmax_at(i), this->catalogue.vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
    }
  else if(_RS_==prop)
	...
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  xy_pts_ref.push_back(std::make_pair(this->htools_ref->RS[i], this->catalogue.Halo[i].rs));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Knot)
	    xy_pts_ref_k.push_back(std::make_pair(this->htools_ref->RS[i], this->catalogue.Halo[i].rs));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Sheet)
	    xy_pts_ref_s.push_back(std::make_pair(this->htools_ref->RS[i], this->catalogue.Halo[i].rs));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Filament)
	    xy_pts_ref_f.push_back(std::make_pair(this->htools_ref->RS[i], this->catalogue.Halo[i].rs));
	  counter++;
	}while(counter<Nobjs_fraction);
    ...
    {
      size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
      xy_pts_ref.push_back(std::make_pair(this->htools_ref->SPIN[i], this->catalogue.Halo[i].spin));
      counter++;
    }while(counter<Nobjs_fraction);
  }  else if(_SPIN_BULLOCK_==prop  )
   {
      ...
      {
      size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
      xy_pts_ref.push_back(std::make_pair(this->htools_ref->SPIN_bullock[i], this->catalogue.Halo[i].spin_bullock ));
      counter++;
    }while(counter<Nobjs_fraction);
   }
	...
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  xy_pts_ref.push_back(std::make_pair(this->catalogue_ref.Halo[i].local_dm, this->htools_ref->vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Knot)
	    xy_pts_ref_k.push_back(std::make_pair(this->catalogue_ref.Halo[i].local_dm, this->htools_ref->vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Sheet)
	    xy_pts_ref_s.push_back(std::make_pair(this->catalogue_ref.Halo[i].local_dm, this->htools_ref->vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
      counter=0;
      do
	{
	  size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
	  if(this->catalogue_ref.Halo[i].gal_cwt==WebType::Filament)
	    xy_pts_ref_f.push_back(std::make_pair(this->catalogue_ref.Halo[i].local_dm, this->htools_ref->vmax_at(i)));
	  counter++;
	}while(counter<Nobjs_fraction);
    }
...
void BiasMT::plot_scaling_relation_assignment_bias(){
   ...
   do{
     size_t i= gsl_rng_uniform_int(gBaseRando,this->catalogue._NOBJS());
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
      xy_pts_ref.push_back(std::make_pair(log10(this->catalogue.vmax_at(i)), this->catalogue.bias[i]));
#else
     xy_pts_ref.push_back(std::make_pair(log10(this->catalogue.mass_at(i)), this->catalogue.bias[i]));
#endif
     counter++;
  }while(counter<Nobjs_fraction);
#ifdef _USE_VMAX_AS_PRIMARY_OBSERVABLE_
...
void BiasMT::get_mean_scaling_relation_assignment_bias(string prop){  //esto hay que pedirselo a Catalog or split loops for ref and mock separately
  ...
  this->htools->get_mean_bias_relation(bias_info);                // Get bias_prop relation from mock
  s_info_in_bins bias_info_ref;
  bias_info_ref.allocate_all(Nbins);
  bias_info_ref.name_info=prop;
  this->htools_ref->get_mean_bias_relation(bias_info_ref); // Get bias_prop relation from reference
...
// -------------------------------------------------------
  ...
  for(size_t i=0;i<secondary_bias_info_ref.size();++i)
  ...
  this->htools_ref->get_mean_secondary_bias_relation(secondary_bias_info_ref); // Get bias_prop relation from mock
// -------------------------------------------------------
  vector<s_info_in_bins>secondary_bias_info(Nqrts);  //container for seconday bias: 2  the number of quartiles to use 
  for(size_t i=0;i<secondary_bias_info.size();++i)
  ...
  this->htools->get_mean_secondary_bias_relation(secondary_bias_info); // Get bias_prop relation from reference
// -------------------------------------------------------
