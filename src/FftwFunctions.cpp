...
#endif
#include<vector>
#include "../include/FftwFunctions.hpp"
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if(this->params._use_random_catalog())
    {
      ...
      this->shot_noise= (alpha+1)*(this->s_r/normal_p);  // originally is alpha*(alpha+1)*(s_r/ normal_power);
          ...
          {  // loop over octant ky>0. Half of what FFT gives
            ...
            for(int k=0;k<=this->params._Nft()/2;++k)
              {  // loop over octant kz>0: this is all FFTW gives.
                ...
                real_prec corr=(this->params._MAS_correction() ? correction_MAS(static_cast<int>(this->params._mass_assignment()), xx_MAS, yy_MAS, zz_MAS): 1.0);
          ...
          {  // loop over octant ky>0. Half of what FFT gives
            ...
            for(int k=0;k<=this->params._Nft()/2;++k)
              {  // loop over octant kz>0: this is all FFTW gives.
                ...
                real_prec corr=(this->params._MAS_correction() ? correction_MAS(static_cast<int>(this->params._mass_assignment()), xx_MAS, yy_MAS, zz_MAS): 1.0);
                real_prec icorr2 = 1.0/(corr*corr);
                real_prec sn_corr=SN_aux*SN_correction_MAS(static_cast<int>(this->params._mass_assignment()), sn_xx_MAS, sn_yy_MAS, sn_zz_MAS);
                // Compute index in c-order
                ULONG lp=index_3d(i,j,k,this->params._Nft(),this->params._Nft()/2+1);
                // Compute thisgs related to 2d power spectrum and multipole
                // decomposition for the first octant
                ...
                switch(this->params._direction_for_rsd_plane_parallel()){
                  case(LineOfSight::X):  mu = i*this->params._d_deltak_x()/kv; break;
                  case(LineOfSight::Y):  mu = j*this->params._d_deltak_y()/kv; break;
                  case(LineOfSight::Z):  mu = k*this->params._d_deltak_z()/kv; break;
                }
                ...
                switch(this->params._direction_for_rsd_plane_parallel())
                  {
                  case(LineOfSight::X):i_par = static_cast<int>(floor((float)(this->params._d_deltak_x()*i * rDeltaK_data))); break;
                  case(LineOfSight::Y):i_par = static_cast<int>(floor((float)(this->params._d_deltak_y()*j * rDeltaK_data))); break;
                  case(LineOfSight::Z):i_par = static_cast<int>(floor((float)(this->params._d_deltak_z()*k * rDeltaK_data))); break;
                  }
                ...
                if(lp!=0)
                  {
                    ...
                    real_prec Pk2=0, Pk4=0;
                    this->aux_func_yamamoto(lp,kx,ky, kz,icorr2,Pk2,Pk4);
                    // 1d Spherical average
                    if(kmod_g<powerk_g0.size())
                      {
#ifdef _USE_OMP_
            ...
            for(int k=0;k<=this->params._Nft();++k)
              {
                ...
                if(lp!=0)
                  { //Exclude zero-mode
                    ...
                    real_prec fluc1=0;
                    this->aux_func_yamamoto(lp,kx,ky,kz,icorr2, fluc1,F2);
...
void FftwFunctions::do_fkp_error_bars_veff(s_data_structure_direct_sum *s_data, vector<real_prec> &P,vector<real_prec> &Veff)
{
  ...
  size_t n_objects = s_data->cat._NOBJS();
#ifdef _USE_OMP_
  int NTHREADS=_NTHREADS_;
  omp_set_num_threads(NTHREADS);
#pragma omp parallel
...
#pragma omp for collapse(2)
    ...
    for(int i = 0; i < n_objects; ++i)
      for(int k=0;k<Veff.size(); ++k)
        {
          real_prec nbar;
          nbar=s_data->cat.mean_density_at(i);
...
void FftwFunctions::get_power_moments_fourier_grid_ds_yam(s_data_structure_direct_sum *s_data){
  ...
  int i_weight1      = (s_data->cat._type_of_object()=="TRACER"? this->params._i_weight1_g() : this->params._i_weight1_r());
  int i_weight2      = (s_data->cat._type_of_object()=="TRACER"? this->params._i_weight2_g() : this->params._i_weight2_r());
  int i_weight3      = (s_data->cat._type_of_object()=="TRACER"? this->params._i_weight3_g() : this->params._i_weight3_r());
  int i_weight4      = (s_data->cat._type_of_object()=="TRACER"? this->params._i_weight4_g() : this->params._i_weight4_r());
  bool use_weight1   = (s_data->cat._type_of_object()=="TRACER"? this->params._use_weight1_g() : this->params._use_weight1_r());
  bool use_weight2   = (s_data->cat._type_of_object()=="TRACER"? this->params._use_weight2_g() : this->params._use_weight2_r());
  bool use_weight3   = (s_data->cat._type_of_object()=="TRACER"? this->params._use_weight3_g() : this->params._use_weight3_r());
  bool use_weight4   = (s_data->cat._type_of_object()=="TRACER"? this->params._use_weight4_g() : this->params._use_weight4_r());
  // Arrays used in the interpolation of mean number density                     *
  
  vector<gsl_real> zzv =  (s_data->zz_v);
  vector<gsl_real> dndz = (s_data->dndz_v);
  
  int i_mean_density= (s_data->cat._type_of_object()=="TRACER"? this->params._i_mean_density_g() : this->params._i_mean_density_r());
  string angles_units = (s_data->cat._type_of_object()=="TRACER" ? this->params._angles_units_g() : this->params._angles_units_r());

  size_t nlines=  s_data->cat._NOBJS();
  int n_columns= s_data->cat._NCOLS();
...
#pragma omp for
    ...
    for(int j=0;j<this->params._Nft()/2;j++){
      ...
      for(int k=0;k<=this->params._Nft()/2;k++){
        ...
        for(ULONG Ig=0;Ig<nlines;++Ig){
          real_prec we;
          real_prec x=s_data->cat.coord1_at(i);
          real_prec y=s_data->cat.coord2_at(i);
          real_prec z=s_data->cat.coord3_at(i);
          real_prec nbar=s_data->cat.mean_density_at(i);
          real_prec rr= sqrt(x*x+y*y+z*z);
          // ***************************************************************
          // Compute weights for each galaxy
          // Compute the weights for each galaxy
          ow[0] = (use_weight1 && (i_weight1<n_columns))? s_data->cat.weight1_at(i) : 1.0;
          ow[1] = (use_weight2 && (i_weight2<n_columns))? s_data->cat.weight2_at(i) : 1.0;
          ow[2] = (use_weight3 && (i_weight3<n_columns))? s_data->cat.weight3_at(i) : 1.0;
          ow[3] = (use_weight4 && (i_weight4<n_columns))? s_data->cat.weight4_at(i) : 1.0;
            ...
            if(kk1<=this->params._kmax_y_ds())
              {
                ...
                a15+=cc*Leg_4;
                a16+=ss*Leg_4;
                if(s_data->cat._type_of_object()=="TRACER"){
  ...
  if(s_data->cat._type_of_object()=="TRACER"){
    ...
    if(s_data->cat._type_of_object() =="RANDOM"){
  ...
  for(int i=0;i<=this->params._Nft();++i){
    ...
    for(int j=0;j<=this->params._Nft();++j){
      ...
      for(int k=0;k<=this->params._Nft();++k){
        ...
        real_prec corr1 = (this->params._MAS_correction() ? correction_MAS(static_cast<int>(this->params._mass_assignment()), xx_MAS, yy_MAS, zz_MAS): 1.00);
        int o_i, o_j, o_k;
        real_prec factor1;
          ...
          for(int jj=0;jj<=this->params._Nft();++jj){
            ...
            for(int kk=0;kk<=this->params._Nft();++kk){
             ...
             if(my_s_box_ave==s_box_linear){
              ...
              real_prec corr2 = (this->params._MAS_correction() ? correction_MAS(static_cast<int>(this->params._mass_assignment()), xxx_MAS, yyy_MAS, zzz_MAS): 1.000);
              ...
              if(fabs(q_iii)<=this->params._Nft()/2 && fabs(q_jjj)<=this->params._Nft()/2 && fabs(q_kkk)<=this->params._Nft()/2){
                ...
                if(lp!=0 &&  lp2!=0 && lp3!=0){			  // Exclude the zero-mode
                  real_prec corr3=(this->params._MAS_correction() ? correction_MAS(this->params._Nft(), static_cast<int>(this->params._mass_assignment()),q_iii,q_jjj,q_kkk): 1.000);
      ...
      for(int k=j;k<this->sgrid;++k){
        ...
        if(kk>0){
         // Assign the MAS correction for this vector
          this->Array_corr[id]=(this->params._MAS_correction() ? correction_MAS(static_cast<int>(this->params._mass_assignment()), xx_MAS, yy_MAS, zz_MAS): 1.000000);
  ...
  if(this->params._type_of_binning()=="linear")
    {
      ...
      So.message_screen("eMAS              =",  static_cast<int>(this->params._mass_assignment()));
