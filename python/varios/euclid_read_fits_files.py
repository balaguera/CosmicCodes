...
#   gal_file="/home/andres/data/Numerics/Euclid/FlagShip/Power/PK_flux2_FS230degCircle_m3_smL5_obsz_z0.9-1.1_rebinned"
    gal_file="/home/andres/data/Numerics/Euclid/FlagShip/Power/PK_flux2_FS230degCircle_m3_smL5_obsz_z0.9-1.1"
    uri=gal_file
    hdul = fits.open(uri+".fits")
    hdul.info()
    nlines=len(hdul[1].data)
    print(hdul['spectrum'].header)


    grid = plt.GridSpec(4,2, wspace=0.01, hspace=0.1)
...
#    plt.plot(kv,pq,color='blue',linestyle='solid',linewidth=1.5, alpha=0.8,label=r'PK_flux2_FS230degCircle_m3_smL5_obsz_z0.9-1.1_rebinned, $\ell=2$')
    plt.plot(kv,pk,color='blue',linestyle='solid',linewidth=1.5, alpha=0.8,label=r'PK_flux2_FS230degCircle_m3_smL5_obsz_z0.9-1.1, $\ell=0$')
    plt.plot(kv,pq,color='blue',linestyle='solid',linewidth=1.5, alpha=0.8,label=r'PK_flux2_FS230degCircle_m3_smL5_obsz_z0.9-1.1, $\ell=2$')
    

#Output frm nbody kit
    file_theo='/home/andres/data/Numerics/Euclid/FlagShip/Power/PK_FS2_z0.9-1.1_nosm_random_bin_nz_boxsize7000_ngrid1200_Om0.32.dat'
    data=np.loadtxt(file_theo)
    xn= data[:, 0]
    yn= np.log10(data[:, 2])
    zn=  np.log10(data[:, 3])
    plt.plot(xn,yn,color='orange',linestyle='dashed',linewidth=1.5, alpha=0.7,label=r'Nbody-kit $\ell=0$')
    plt.plot(xn,zn,color='orange',linestyle='dashed',linewidth=1.5, alpha=0.7,label=r'Nbody-kit $\ell=0$')


    file_theo='/home/andres/data/Numerics/Euclid/FlagShip/Power/Pk_fkp_FlagShip_Nft1024_PCS_galaxies_z0.9_z1.1.txt'
#    file_theo='/home/andres/data/Numerics/Euclid/FlagShip/Power/Pk_ys_FlagShip_Nft512_TSC_galaxies_z0.9_z1.1_ndel6.txt'
    data=np.loadtxt(file_theo)
    x= data[:, 0]
    y= np.log10(data[:, 1])
    z=  np.log10(data[:, 2])
...
#    plt.xlabel(r"k ", fontsize=8)
    plt.ylabel(r"$\log_{10} P(k)$", fontsize=7)
...
#    fig.xaxis.set_minor_locator(AutoMinorLocator(min_locator_x))
    ...
    plt.plot(kv,pk,color='black',linestyle='solid',linewidth=1.5, alpha=0.8,label=r'$\ell=0$ (ABA vs Off)')
#    plt.plot(kv,pq,color='red',linestyle='solid',linewidth=1.5, alpha=0.8,label=r'$\ell=2$ (ABA vs Off)')
    ...
    for i in range(0,len(x)):
        kv.append(hdul[1].data[i][0])
        pk.append(hdul[1].data[i][2]/pow(10,yn[i]))
        pq.append(hdul[1].data[i][4]/pow(10,zn[i]))
        
    plt.plot(kv,pk,color='black',linestyle='dotted',linewidth=1.5, alpha=0.5,label=r'$\ell=0$ (Off vs Nbk)')
#    plt.plot(kv,pq,color='red',linestyle='dotted',linewidth=1.5, alpha=0.3,label=r'$\ell=2$ (Off vs Nbk)')
    ...
    for i in range(0,len(x)):
        kv.append(hdul[1].data[i][0])
        pk.append(pow(10,y[i])/pow(10,yn[i]))
        pq.append(pow(10,y[i])/pow(10,zn[i]))
        
    plt.plot(kv,pk,color='black',linestyle='dashed',linewidth=1.5, alpha=0.4,label=r'$\ell=0$ (ABA vs Nbk)')
#    plt.plot(kv,pq,color='red',linestyle='dashed',linewidth=1.5, alpha=0.3,label=r'$\ell=2$ (ABA vs Nbk)')
    ...
    plt.xlabel(r"k [h / Mpc]", fontsize=7)
    plt.ylabel(r"Ratios ", fontsize=7)
...
#fits_to_ascii(path,"random_flux2_flagship_FS2.1.10b_FS230degCircle_m3_nosm_obsz_50x_z0.9-1.1")
